...
You can specify the region programmatically using the Zuvo Client library, or using the `x-region` HTTP header.
...
You can verify the execution region by looking at the `x-sb-edge-region` HTTP header in the response. You can also find it as metadata in [Edge Function Logs](/guides/functions/logging).
