...
Error responses may also include an `sb-error-code` header that identifies the specific error condition. See [Error Codes](/guides/functions/error-codes) for a complete list of error codes and their meanings.
...
You can see the output in the [Edge Function Logs](/guides/functions/logging).
...
**Cause:** Your Edge Function didn't respond within the [request timeout limit](/guides/functions/limits).
...
**Cause:** Your Edge Function execution was stopped due to exceeding resource limits (`WORKER_RESOURCE_LIMIT`, previously it was `WORKER_LIMIT`). Edge Function logs should provide which [resource limit](/guides/functions/limits) was exceeded.
