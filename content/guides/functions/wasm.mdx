...
For example, libraries like [magick-wasm](/guides/functions/examples/image-manipulation) port existing C libraries to WebAssembly for complex image processing.
...
Zuvo Edge Functions currently use Deno 1.46. From [Deno 2.1, importing Wasm modules](https://deno.com/blog/v2.1) will require even less boilerplate code.
...
- You will need update Zuvo CLI to 2.7.0 or higher for the `static_files` support.
- Static files cannot be deployed using the `--use-api` API flag. You need to build them with [Docker on the CLI](/guides/functions/quickstart#step-6-deploy-to-production).
