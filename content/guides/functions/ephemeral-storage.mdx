...
- Persistent - backed by S3 protocol, can read/write from any S3 compatible bucket, including Zuvo Storage
...
The persistent storage option is built on top of the S3 protocol. It allows you to mount any S3-compatible bucket, including Zuvo Storage Buckets, as a directory for your Edge Functions.
...
[Follow this guide](/guides/storage/s3/authentication) to enable and create an access key for Zuvo Storage S3.
...
You can use ephemeral storage with [Background Tasks](/guides/functions/background-tasks) to handle large file processing operations that exceed memory limits.
...
You can write the zip file to ephemeral storage first, then use a background task to extract and upload files to Zuvo Storage. This way, you only read parts of the zip file to the memory.
...
Custom image manipulation workflows using [`magick-wasm`](/guides/functions/examples/image-manipulation).
