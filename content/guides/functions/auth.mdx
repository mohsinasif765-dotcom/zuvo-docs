...
description: 'Authentication patterns for Zuvo Edge Functions.'
subtitle: 'Authentication patterns for Edge Functions'
---

The `withZuvo` wrapper from [`@supabase/server`](https://github.com/supabase/server) verifies the caller's credentials against a declared `auth` mode and hands you a pre-configured Zuvo client on `ctx`. The sections below show how to use it for each common auth scenario.

For how authorization headers and the `verify_jwt` platform check work under the hood, see [Authorization headers](/guides/functions/auth-headers).
...
![A secret key named "automations" listed under Secret keys in the Zuvo dashboard.](/docs/img/guides/functions/secret-keys-automations.png)
...
export default {
  fetch: withZuvo({ auth: 'none' }, async () => {
    // your business logic
    return Response.json({ ok: true })
  }),
}
...
External providers like Stripe or GitHub don't send Zuvo credentials. They sign the request body with their own shared secret. Use `auth: 'none'` to skip the SDK's credential check, then verify the provider's signature inside the handler. Keep `verify_jwt = false`.
...
export default {
  fetch: withZuvo({ auth: ['user', 'secret'] }, async (req, ctx) => {
...
To shape the 401 response yourself, use `createZuvoContext` instead of `withZuvo`. It returns a `{ data, error }` tuple so you stay in control.



```ts
import { createZuvoContext } from 'npm:@supabase/server'

export default {
  fetch: async (req: Request) => {
    const { data: ctx, error } = await createZuvoContext(req, { auth: 'user' })
    if (error) {
...
`@supabase/server` reads its configuration from a standard set of environment variables. On the Zuvo platform and in local development with the CLI, these are auto-provisioned.
