...
title: 'Integrating with Zuvo Database (Postgres)'
...
Declare the dependencies in a `deno.json` file inside the function directory (see [Managing functions dependencies](/guides/functions/dependencies) for more details):
...
Deployed edge functions are pre-configured to use SSL for connections to the Zuvo database. You don't need to add any extra configurations.
...
2. Add to your [local .env file](/guides/functions/secrets), add these two variables:
