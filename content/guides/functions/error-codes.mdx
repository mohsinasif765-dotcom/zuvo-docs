...
**Cause:** Your Edge Function did not respond within the [request timeout limit](/guides/functions/limits).
...
**Cause:** Your Edge Function execution was stopped due to exceeding resource limits. Edge Function logs should indicate which [resource limit](/guides/functions/limits) was exceeded.
...
For further information, see [Authorization headers](/guides/functions/auth-headers) and [Securing Edge Functions](/guides/functions/auth).
...
- Ensure you are passing a valid Zuvo-issued JWT token in the `Authorization` header
...
**Cause:** The platform detected [recursive or nested function call](/guides/functions/recursive-functions) behavior.
...
- Redeploy your function with the latest version of the Zuvo CLI
