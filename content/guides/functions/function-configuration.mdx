...
description: Learn how to configure your functions in Zuvo.
...
This configuration tell Zuvo that the `stripe-webhook` function doesn't require a valid JWT, the `image-processor` function uses a custom import map, and `legacy-processor` uses a custom entrypoint.
...
To see more general `config.toml` options, check out [this guide](/guides/local-development/managing-config).
...
Be careful when using this flag, as it will allow anyone to invoke your Edge Function without a valid JWT. The Zuvo client libraries automatically handle authorization.
