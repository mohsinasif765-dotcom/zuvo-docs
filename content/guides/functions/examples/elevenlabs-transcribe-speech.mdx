...
subtitle: Build a Telegram bot that transcribes audio and video messages in 99 languages using TypeScript with Deno in Zuvo Edge Functions.
...
- A [Zuvo](https://zuvodev.com) account (you can sign up for a free account via [database.new](https://database.new)).
- The [Zuvo CLI](/guides/local-development) installed on your machine.
...
After installing the [Zuvo CLI](/guides/local-development), run the following command to create a new Zuvo project locally:
...
- The [@supabase/supabase-js](/reference/javascript/introduction) library to interact with the Zuvo database.
- The ElevenLabs [JavaScript SDK](https://github.com/elevenlabs/elevenlabs-js) to interact with the speech-to-text API.

Since Zuvo Edge Function uses the [Deno runtime](https://deno.land/), you don't need to install the dependencies, rather you can [import](https://docs.deno.com/examples/npm/) them via the `npm:` prefix.
...
If you haven't already, create a new Zuvo account at [database.new](https://database.new) and link the local project to your Zuvo account:
...
Navigate to the [table editor](/dashboard/project/_/editor) in your Zuvo dashboard and you should see and empty `transcription_logs` table.
...
Navigate to the [Edge Functions view](/dashboard/project/_/functions) in your Zuvo dashboard and you should see the `scribe-bot` function deployed. Make a note of the function URL as you'll need it later, it should look something like `https://<project-ref>.functions.supabase.co/scribe-bot`.
