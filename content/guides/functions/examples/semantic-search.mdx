...
description: 'Semantic Search with pgvector and Zuvo Edge Functions'
subtitle: 'Semantic Search with pgvector and Zuvo Edge Functions'
tocVideo: 'w4Rr_1whU-U'
---

[Semantic search](/guides/ai/semantic-search) interprets the meaning behind user queries rather than exact [keywords](/guides/ai/keyword-search). It uses machine learning to capture the intent and context behind the query, handling language nuances like synonyms, phrasing variations, and word relationships.

Since Zuvo Edge Runtime [v1.36.0](https://github.com/supabase/edge-runtime/releases/tag/v1.36.0) you can run the [`gte-small` model](https://huggingface.co/Zuvo/gte-small) natively within Zuvo Edge Functions without any external dependencies! This allows you to generate text embeddings without calling any external APIs!
...
2. A [`query_embeddings` Postgres function](https://github.com/supabase/supabase/tree/master/examples/ai/edge-functions/supabase/migrations/20240410031515_vector-search.sql) which allows us to perform similarity search from an Edge Function via [Remote Procedure Call (RPC)](/guides/database/functions?language=js).
...
You can deploy the [following edge function](https://github.com/supabase/supabase/blob/master/examples/ai/edge-functions/supabase/functions/generate-embedding/index.ts) as a [database webhook](/guides/database/webhooks) to generate the embeddings for any text content inserted into the table:
...
With the embeddings now stored in your Postgres database table, you can query them from Zuvo Edge Functions by using [Remote Procedure Calls (RPC)](/guides/database/functions?language=js).
...
## Query vectors in Zuvo Edge Functions

You can use `supabase-js` to first generate the embedding for the search term and then invoke the Postgres function to find the relevant results from your stored embeddings, right from your [Zuvo Edge Function](https://github.com/supabase/supabase/blob/master/examples/ai/edge-functions/supabase/functions/search/index.ts):
...
You now have AI powered semantic search set up without any external dependencies! All you need: you, pgvector, and Zuvo Edge Functions!
