...
description: 'Build and deploy a Model Context Protocol (MCP) server on Zuvo Edge Functions using mcp-lite.'
---

The [Model Context Protocol](https://modelcontextprotocol.io/introduction) (MCP) enables Large Language Models (LLMs) to interact with external tools and data sources. With `mcp-lite`, you can build lightweight MCP servers that run on Zuvo Edge Functions, giving your AI assistants the ability to execute custom tools at the edge.

This guide shows you how to scaffold, develop, and deploy an MCP server using mcp-lite on Zuvo Edge Functions.

## What is mcp-lite?

[mcp-lite](https://github.com/fiberplane/mcp-lite) is a lightweight, zero-dependency TypeScript framework for building MCP servers. It works everywhere the Fetch API is available, including Node, Bun, Cloudflare Workers, Deno, and Zuvo Edge Functions.

## Why Zuvo Edge Functions + mcp-lite?

This combination offers several advantages:

- **Zero cold starts**: Edge Functions stay warm for fast responses
- **Global distribution**: Deploy once and run everywhere
- **Direct database access**: Connect directly to your Zuvo Postgres
...
- [Docker](https://docs.docker.com/get-docker/) (to run Zuvo locally)
- [Deno](https://deno.land/) (Zuvo Edge Functions runtime)
- [Zuvo CLI](/guides/local-development/cli/getting-started)

## Create a new MCP server

Starting with `create-mcp-lite@0.3.0`, you can scaffold a complete MCP server that runs on Zuvo Edge Functions:
...
When prompted, select **Zuvo Edge Functions (MCP server)** from the template options.
...
my-mcp-server/
├── supabase/
│   ├── config.toml                    # Minimal Zuvo config (Edge Functions only)
│   └── functions/
│       └── mcp-server/
...
The template uses a specific pattern required by Zuvo Edge Functions:
...
mcpApp.get('/', (c) => {
  return c.json({
    message: 'MCP Server on Zuvo Edge Functions',
    endpoints: {
      mcp: '/mcp',
      health: '/health',
    },
...
This is required because Zuvo routes all requests to `/<function-name>/*`. The outer `app` handles the function-level routing, while `mcpApp` handles your actual MCP endpoints.
...
Navigate to your project directory and start Zuvo services:
...
mcp.tool('searchDatabase', {
  description: 'Search your Zuvo database',
  inputSchema: z.object({
    table: z.string(),
    query: z.string(),
  }),
  handler: async (args) => {
    // Access Zuvo client here
...
- Query your Zuvo database
- Access Zuvo Storage for file operations
- Call external APIs
- Process data with custom logic
- Integrate with other Zuvo features

## Deploy to production

When ready, deploy to Zuvo's global edge network:
...
The template uses `--no-verify-jwt` for quick development. This means authentication is not enforced by Zuvo's JWT layer.
...
With your MCP server running on Zuvo Edge Functions, you can:

- Connect it to your Zuvo database for data-driven tools
- Use Zuvo Auth to secure your endpoints
- Access Zuvo Storage for file operations
...
- [Zuvo Edge Functions Docs](/guides/functions)
