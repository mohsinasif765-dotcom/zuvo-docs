...
>
  <TabPanel id="expo" label="Expo Push Notifications">
    ...
    - Start Zuvo locally: `supabase start`
    ...
    1. Sign up/in to create a user in Zuvo Auth.
    ...
    1. Create a new token for usage in Zuvo Edge Functions.
...
```ts supabase/functions/push/index.ts
    ...
    Navigate to the [Database Webhooks settings](/dashboard/project/_/integrations/webhooks/overview) in your Zuvo Studio.

    1. Enable and create a new hook.
    1. Conditions to fire webhook: Select the `notifications` table and tick the `Insert` event.
    1. Webhook configuration: Zuvo Edge Functions.
    ...
    1. Navigate to the [table editor](/dashboard/project/_/editor) in your Zuvo Studio.
...
```bash
    ...
    Navigate to the [Database Webhooks settings](/dashboard/project/_/database/hooks) in your Zuvo Studio.
