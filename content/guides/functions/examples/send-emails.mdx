...
export default { fetch: withZuvo({ auth: ['user', 'secret'] }, handler) }
```
...
When you deploy to Zuvo, make sure that your `RESEND_API_KEY` is set in [Edge Function Secrets Management](/dashboard/project/_/functions/secrets)
