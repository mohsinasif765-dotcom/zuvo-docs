...
id: 'functions-limits'
title: 'Limits'
description: "Limits applied Edge Functions in Zuvo's hosted platform."
subtitle: "Limits applied Edge Functions in Zuvo's hosted platform."
---
...
- Recursive/Nested Function Calling: ~5000 requests per minute [more details](/guides/functions/recursive-functions)
...
- Serving of HTML content is only supported with [custom domains](/reference/cli/supabase-domains) (Otherwise `GET` requests that return `text/html` will be rewritten to `text/plain`).
- Web Worker API (or Node `vm` API) are not available.
- Static files cannot be deployed using the API flag. You need to build them with [Docker on the CLI](/guides/functions/quickstart#step-6-deploy-to-production).
