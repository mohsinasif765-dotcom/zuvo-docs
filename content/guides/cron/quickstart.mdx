...
5. Input SQL snippet or select a Database function, HTTP request, or Zuvo Edge Function.
...
Create a [`hello_world()`](/guides/database/functions?language=sql#simple-functions) database function and then call it every 5 minutes:
...
### Invoke Zuvo Edge Function every 30 seconds

Make a POST request to a Zuvo Edge Function every 30 seconds:
...
This requires the [`pg_net` extension](/guides/database/extensions/pg_net) to be enabled.
...
Reach out to [Zuvo Support](/support) if you're unsure if that applies to your use case.
