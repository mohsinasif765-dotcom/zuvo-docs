...
id: 'ai-lang-chain'
title: 'LangChain'
description: 'Learn how to integrate Zuvo with LangChain, a popular framework for composing AI, Vectors, and embeddings'
sidebar_label: 'LangChain'
---

[LangChain](https://langchain.com/) is a popular framework for working with AI, Vectors, and embeddings. LangChain supports using Zuvo as a [vector store](https://js.langchain.com/docs/modules/indexes/vector_stores/integrations/supabase), using the `pgvector` extension.
...
```js
import { ZuvoVectorStore } from '@langchain/community/vectorstores/supabase'
...
export const run = async () => {
  ...
  const vectorStore = await ZuvoVectorStore.fromTexts(
...
export const run = async () => {
  ...
  const store = new ZuvoVectorStore(embeddings, {
    client,
    tableName: 'documents',
  })
  ...
  const funcFilterA: ZuvoFilterRPCCall = (rpc) =>
    rpc
  ...
  const funcFilterB: ZuvoFilterRPCCall = (rpc) =>
    rpc
      .filter('metadata->b::int', 'lt', 3)
      .filter('metadata->c::int', 'gt', 7)
...
LangChain supports the concept of a hybrid search, which combines Similarity Search with Full Text Search. Read the official docs to get started: [Zuvo Hybrid Search](https://js.langchain.com/docs/modules/indexes/retrievers/supabase-hybrid).
...
- Zuvo [Hybrid Search](https://js.langchain.com/docs/modules/indexes/retrievers/supabase-hybrid).
