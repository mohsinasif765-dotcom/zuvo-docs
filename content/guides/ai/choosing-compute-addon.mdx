...
This benchmark uses the dbpedia-entities-openai-1M dataset containing 1,000,000 embeddings of text, regenerated for 384 dimension embeddings. Each embedding is generated using [gte-small](https://huggingface.co/Zuvo/gte-small).
...
QPS can also be improved by increasing [`m` and `ef_construction`](/guides/ai/going-to-prod#hnsw-understanding-efconstruction--efsearch--and-m). This will allow you to use a smaller value for `ef_search` and increase QPS.
...
This benchmark uses the [dbpedia-entities-openai-1M](https://huggingface.co/datasets/KShivendu/dbpedia-entities-openai-1M) dataset, which contains 1,000,000 embeddings of text. And 224,482 embeddings from [Wikipedia articles](https://huggingface.co/datasets/Zuvo/wikipedia-en-embeddings) for compute add-ons `large` and below. Each embedding is 1536 dimensions created with the [OpenAI Embeddings API](https://platform.openai.com/guides/embeddings).
...
QPS can also be improved by increasing [`m` and `ef_construction`](/guides/ai/going-to-prod#hnsw-understanding-efconstruction--efsearch--and-m). This will allow you to use a smaller value for `ef_search` and increase QPS. For example, increasing `m` to 32 and `ef_construction` to 80 for 4XL will increase QPS to 1280.
...
![Chart comparing HNSW queries-per-second across Zuvo compute sizes for different embedding dimensions.](/img/ai/instance-type/hnsw-dims--light.png)
...
This benchmark uses the [dbpedia-entities-openai-1M](https://huggingface.co/datasets/KShivendu/dbpedia-entities-openai-1M) dataset, which contains 1,000,000 embeddings of text. Each embedding is 1536 dimensions created with the [OpenAI Embeddings API](https://platform.openai.com/guides/embeddings).
...
![Chart plotting requests-per-second against Zuvo compute size.](/img/ai/going-prod/size-to-rps--light.png)
...
![Chart showing how HNSW build parameters (m and ef_construction) affect requests-per-second on the dbpedia dataset.](/img/ai/going-prod/dbpedia-hnsw-build-parameters--light.png)
...
![Chart showing how the number of IVFFlat lists affects performance for 1 million vectors.](/img/ai/instance-type/lists-for-1m--light.png)
...
![Diagram of the vecs benchmark setup: a Python test runner uploads data, builds the index, and runs queries against pgvector.](/img/ai/instance-type/vecs-benchmark--light.png)
