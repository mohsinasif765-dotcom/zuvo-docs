...
description: 'Implement video search with the Mixpeek Multimodal Embed API and Zuvo Vector.'
subtitle: 'Implement video search with the Mixpeek Multimodal Embed API and Zuvo Vector.'
---
...
This guide demonstrates how to implement video search using Mixpeek Embed for video processing and embedding, and Zuvo Vector for storing and querying embeddings.
...
If you haven't already, [install the Zuvo CLI](/guides/local-development), then initialize Zuvo in the root of your newly created poetry project:
...
Next, start your local Zuvo stack:
...
This will start up the Zuvo stack locally and print out a bunch of environment details, including your local `DB URL`. Make a note of that for later use.

## Install the dependencies

Add the following dependencies to your project:

- [`supabase`](https://github.com/supabase-community/supabase-py): Zuvo Python Client
...
Next, create a `seed` method, which will create a new Zuvo table, generate embeddings for your video chunks, and insert the embeddings into your database:



```python
def seed():
    # Initialize Zuvo and Mixpeek clients
...
After activating the virtual environment with `poetry shell`, you can now run your seed script via `poetry run seed`. You can inspect the generated embeddings in your local database by visiting the local Zuvo dashboard at [localhost:54323](http://localhost:54323/project/default/editor).

## Perform a video search from a text query

With Zuvo Vector, you can query your embeddings. You can use either a video clip as search input or alternatively, you can generate an embedding from a string input and use that as the query input:
