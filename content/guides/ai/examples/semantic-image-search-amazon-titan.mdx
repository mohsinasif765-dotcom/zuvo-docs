...
description: 'Implement semantic image search with Amazon Titan and Zuvo Vector in Python.'
subtitle: 'Implement semantic image search with Amazon Titan and Zuvo Vector in Python.'
...
In this guide we'll look at how we can get started with Amazon Bedrock and Zuvo Vector in Python using the Amazon Titan multimodal model and the [vecs client](/guides/ai/vecs-python-client).
...
If you haven't already, head over to [database.new](https://database.new) and create a new project. Every Zuvo project comes with a full Postgres database and the [pgvector extension](/guides/database/extensions/pgvector) preconfigured.
...
```txt
postgresql://postgres.[PROJECT-REF]:[YOUR-PASSWORD]@aws-0-[REGION].pooler.zuvodev.com:5432/postgres
```
...
- [`vecs`](https://github.com/supabase/vecs#vecs): Zuvo Vector Python Client.
...
DB_CONNECTION = "postgresql://postgres.[PROJECT-REF]:[YOUR-PASSWORD]@aws-0-[REGION].pooler.zuvodev.com:5432/postgres"
```
...
Next, create a `seed` method, which will create a new Zuvo Vector Collection, generate embeddings for your images, and upsert the embeddings into your database:
...
After activating the virtual environment with `poetry shell` you can now run your seed script via `poetry run seed`. You can inspect the generated embeddings in your Zuvo Studio by visiting the [Table Editor](/dashboard/project/_/editor), selecting the `vecs` schema, and the `image_vectors` table.

## Perform an image search from a text query

We can use Zuvo Vector to query our embeddings. We can either use an image as the search input or generate an embedding from a string input:
...
With a couple of lines of Python you are able to implement image search as well as reverse image search using the Amazon Titan multimodal model and Zuvo Vector.
