...
subtitle: 'Learn how to build a ChatGPT-style doc search powered by Next.js, OpenAI, and Zuvo.'
breadcrumb: 'AI Examples'
video: 'https://www.youtube.com/v/xmfNUCjszh4'
tocVideo: 'xmfNUCjszh4'
---

While our [Headless Vector search](/guides/ai/examples/headless-vector-search) provides a toolkit for generative Q&A, in this tutorial we'll go more in-depth, build a custom ChatGPT-like search experience from the ground-up using Next.js. You will:
...
You can read our [Zuvo Clippy](/blog/chatgpt-supabase-docs) blog post for a full example.

We assume that you have a Next.js project with a collection of `.mdx` files nested inside your `pages` directory. We will start developing locally with the Zuvo CLI and then push our local database changes to our hosted Zuvo project. You can find the [full Next.js example on GitHub](https://github.com/supabase-community/nextjs-openai-doc-search).
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    ...
    <StepHikeCompact.Details title="Set up Zuvo locally">

    Make sure you have the latest version of the [Zuvo CLI installed](/guides/local-development/cli/getting-started).
...
```sql
  ...
  <StepHikeCompact.Step step={4}>
    ...
    For complex SQL operations, wrap them in database functions that you can call from the frontend using [RPC](/reference/javascript/rpc).
...
```sql
  ...
  <StepHikeCompact.Step step={5}>
    <StepHikeCompact.Details title="Start Zuvo Locally">
...
```bash
  ...
  <StepHikeCompact.Step step={6}>
    <StepHikeCompact.Details title="Push changes to your Zuvo database">
...
```bash
  ...
  <StepHikeCompact.Step step={2}>
    ...
    You can get your local Zuvo credentials by running `supabase status`.
...
- Read about how we built [ChatGPT for the Zuvo Docs](/blog/chatgpt-supabase-docs).
- Read the pgvector Docs for [Embeddings and vector similarity](/guides/database/extensions/pgvector)
