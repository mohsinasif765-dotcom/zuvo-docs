...
Hybrid search combines [full text search](/guides/ai/keyword-search) (searching by keyword) with [semantic search](/guides/ai/semantic-search) (searching by meaning) to identify results that are both directly and contextually relevant to the user's query.
...
- `fts` is an auto-generated `tsvector` column that is generated using the text in `content`. We will use this for [full text search](/guides/database/full-text-search) (search by keyword).
- `embedding` is a [vector column](/guides/ai/vector-columns) that stores the vector generated from our embedding model. We will use this for [semantic search](/guides/ai/semantic-search) (search by meaning). We chose 512 dimensions for this example, but adjust this to match the size of the embedding vectors generated from your preferred model.
...
For semantic vector search we use an [HNSW index](/guides/ai/vector-indexes/hnsw-indexes), which is a high performing approximate nearest neighbor (ANN) search algorithm. Note that we are using the `vector_ip_ops` (inner product) operator with this index because we plan on using the inner product (`<#>`) operator later in our query. If you plan to use a different operator like cosine distance (`<=>`), be sure to update the index accordingly. For more information, see [distance operators](/guides/ai/vector-indexes#distance-operators).
...
In practice, you will likely be calling this from the [Zuvo client](/reference/javascript/introduction) or through a custom backend layer. Here is a quick example of how you might call this from an [Edge Function](/guides/functions) using JavaScript:
...
Deno.serve(async (req) => {
  ...
  // Instantiate the Zuvo client
  // (replace service role key with user's JWT if using Zuvo auth and RLS)
...
For more information on how to create, test, and deploy edge functions, see [Getting started](/guides/functions/quickstart).

## See also

- [Embedding concepts](/guides/ai/concepts)
- [Vector columns](/guides/ai/vector-columns)
- [Vector indexes](/guides/ai/vector-indexes)
- [Semantic search](/guides/ai/semantic-search)
- [Full text (keyword) search](/guides/database/full-text-search)
