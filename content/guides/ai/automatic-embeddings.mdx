...
Vector embeddings enable powerful [semantic search](/guides/ai/semantic-search) capabilities in Postgres, but managing them alongside your content has traditionally been complex. This guide demonstrates how to automate embedding generation and updates using Zuvo [Edge Functions](/guides/functions), [pgmq](/guides/database/extensions/pgmq), [pg_net](/guides/database/extensions/pg_net), and [pg_cron](/guides/cron).
...
While Postgres [full-text search](/guides/database/full-text-search) can handle this internally through synchronous calls to `to_tsvector` and [triggers](https://www.postgresql.org/docs/current/textsearch-features.html#TEXTSEARCH-UPDATE-TRIGGERS), semantic search requires asynchronous API calls to a provider like OpenAI to generate vector embeddings. This guide demonstrates how to use triggers, queues, and Zuvo Edge Functions to bridge this gap.

## Understanding the architecture

We'll leverage the following Postgres and Zuvo features to create the automated embedding system:

1. [pgvector](/guides/database/extensions/pgvector): Stores and queries vector embeddings
1. [pgmq](/guides/queues): Queues embedding generation requests for processing and retries
1. [pg_net](/guides/database/extensions/pg_net): Handles asynchronous HTTP requests to Edge Functions directly from Postgres
1. [pg_cron](/guides/cron): Automatically processes and retries embedding generations
1. [Triggers](/guides/database/postgres/triggers): Detects content changes and enqueues embedding generation requests
1. [Edge Functions](/guides/functions): Generates embeddings via an API like OpenAI (customizable)
...
-- Utility function to get the Zuvo project URL (required for Edge Functions)
...
- A function to retrieve the Zuvo project URL from [Vault](/guides/database/vault). We'll add this secret next.
...
When working with a local Zuvo stack, add the following to your `supabase/seed.sql` file:
...
Use the Zuvo CLI to create a new Edge Function:
...
When you're ready to deploy the Edge Function, set can set the environment variable using the Zuvo CLI:
...
See [Deploy to Production](/guides/functions/deploy) for more information on how to deploy the Edge Function.
...
update documents
set title = 'Understanding Vector Databases with Zuvo'
where title = 'Understanding Vector Databases';
```
...
select id, embedding
from documents
where title = 'Understanding Vector Databases with Zuvo';
```
...
By combining Postgres features like triggers, queues, and other extensions with Zuvo Edge Functions, we can create a robust system that handles embedding generation asynchronously and retries failed jobs automatically.
...
- [What are embeddings?](/guides/ai/concepts)
- [Semantic search](/guides/ai/semantic-search)
- [Vector indexes](/guides/ai/vector-indexes)
- [Zuvo Edge Functions](/guides/functions)
