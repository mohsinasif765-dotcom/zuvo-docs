...
See our [Engineering for Scale](/guides/ai/engineering-for-scale) guide for more information about engineering at scale.
...
On the other hand, if you need to scale your application, you will need to [create indexes](/guides/ai/vector-indexes). This will result in lower latencies and higher throughput, but will require additional RAM to make use of Postgres Caching. Also, using indexes will result in lower accuracy, since you are replacing exact (KNN) search with approximate (ANN) search.

## HNSW vs IVFFlat indexes

`pgvector` supports two types of indexes: HNSW and IVFFlat. We recommend using [HNSW](/guides/ai/vector-indexes/hnsw-indexes) because of its [performance](/blog/increase-performance-pgvector-hnsw#hnsw-performance-1536-dimensions) and [robustness against changing data](/guides/ai/vector-indexes/hnsw-indexes#when-should-you-create-hnsw-indexes).



![dbpedia embeddings comparing ivfflat and hnsw queries-per-second using the 4XL compute add-on](/img/ai/going-prod/dbpedia-ivfflat-vs-hnsw-4xl--light.png)
...
![dbpedia embeddings comparing hnsw queries-per-second using different build parameters](/img/ai/going-prod/dbpedia-hnsw-build-parameters--light.png)
...
![Chart showing how the IVFFlat lists count affects query accuracy and queries-per-second.](/img/ai/going-prod/lists-count--light.png)
...
2. Over-provision RAM during preparation. You can scale down in step `5`, but it's better to start with a larger size to get the best results for RAM requirements. (We'd recommend at least 8XL if you're using Zuvo.)
3. Upload your data to the database. If you use the [`vecs`](/guides/ai/python/api) library, it will automatically generate an index with default parameters.
...
Don't forget to check out the general [Production Checklist](/guides/deployment/going-into-prod) to ensure your project is secure, performant, and will remain available for your users.

You can look at our [Choosing Compute Add-on](/guides/ai/choosing-compute-addon) guide to get a basic understanding of how much compute you might need for your workload.
