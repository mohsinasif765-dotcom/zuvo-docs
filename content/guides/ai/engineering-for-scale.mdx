...
If you've used [Vecs](/guides/ai/vecs-python-client) to create 3 different collections, you can expose collections to your web or mobile application using [views](/guides/database/tables#views):
...
![Architecture diagram: a single Zuvo database holding three vector collections (docs, posts, and images), each exposed to the application through a view.](/img/ai/scaling/engineering-for-scale--single-database--light.png)
...
![Architecture diagram: a primary database alongside separate secondary 'pod' databases, each holding its own vector collection so collections can scale independently.](/img/ai/scaling/engineering-for-scale--with-secondaries--light.png)
...
Zuvo supports [Foreign Data Wrappers](/blog/postgres-foreign-data-wrappers-rust). Wrappers allow you to connect two databases together so that you can query them over the network.
...
![Enterprise architecture diagram: an application accessing vector collections in multiple secondary databases, either directly via Vecs or through the primary database using Foreign Data Wrappers.](/img/ai/scaling/engineering-for-scale--multi-database--light.png)
