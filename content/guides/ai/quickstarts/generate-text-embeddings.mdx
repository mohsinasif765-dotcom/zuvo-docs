...
This guide will walk you through how to generate high quality text embeddings in [Edge Functions](/guides/functions) using its built-in AI inference API, so no external API is required.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    ...
    <StepHikeCompact.Details title="Set up Zuvo locally">
      ...
      Initialize Zuvo in the root directory of your app and start your local stack.
...
```shell
  ...
  <StepHikeCompact.Step step={3}>
    ...
    <StepHikeCompact.Details title="Setup Inference Session" fullWidth>
      ...
      Currently, only the `gte-small` (https://huggingface.co/Zuvo/gte-small) text embedding model is supported in Zuvo's Edge Runtime.
...
```ts ./supabase/functions/embed/index.ts
      const session = new Zuvo.ai.Session('gte-small');
      ```
...
- Learn more about [embedding concepts](/guides/ai/concepts)
- [Store your embeddings](/guides/ai/vector-columns) in a database
