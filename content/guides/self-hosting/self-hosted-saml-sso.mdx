...
description: 'Set up SAML 2.0 Single Sign-On for self-hosted Zuvo with Docker.'
subtitle: 'Set up SAML 2.0 Single Sign-On for self-hosted Zuvo with Docker.'
---
...
This guide covers the full setup: generating a signing key, enabling SAML in your Zuvo instance, registering an IdP, and integrating SSO into your application.

<Admonition type="note">

Client-side integration uses the same `supabase.auth.signInWithSSO()` method documented in the [SSO with SAML 2.0](/guides/auth/enterprise-sso/auth-sso-saml) guide. This guide focuses on the self-hosted server configuration.
...
- A running self-hosted Zuvo instance (see the [setup guide](/guides/self-hosting/docker))
...
- `API_EXTERNAL_URL` set to the publicly-accessible URL of your Zuvo Auth service (e.g., `https://<your-domain>/auth/v1`). Used as the base for constructing the SAML Service Provider entity ID and ACS endpoint URL

## How SAML SSO works in Zuvo
...
Once SAML is enabled, your Zuvo instance exposes service provider (SP) metadata at `{API_EXTERNAL_URL}/sso/saml/metadata`.
...
- Copy the **SAML Metadata URL** (this is the IdP metadata URL for Zuvo)
...
Attribute mapping lets you control how SAML assertion attributes are translated into Zuvo user claims. If no mapping is provided, Auth uses sensible defaults:
...
- [SSO with SAML 2.0](/guides/auth/enterprise-sso/auth-sso-saml) - Client-side SAML integration guide
- [Auth server configuration reference](/guides/self-hosting/auth/config) - Full list of Auth environment variables
