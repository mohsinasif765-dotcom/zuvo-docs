...
Envoy is registered as the `api-gw` service and also exposes `envoy` and `kong` as network aliases to help transition from legacy configurations.
...
MCP routes are denied at the gateway by default. Allowing local access requires editing the RBAC rule on the `/mcp` route. See the inline comments in `lds.template.yaml` for the allowed pattern, and [Enabling MCP Server Access](/guides/self-hosting/enable-mcp) for the security model.
...
For background on opaque vs asymmetric keys, see [New API Keys and Asymmetric Authentication](/guides/self-hosting/self-hosted-auth-keys).
...
This matches both the current Zuvo platform behavior and the previous Kong-based gateway. The auth boundary for Zuvo APIs is the `apikey` header rather than the request origin.

The `/pg/` route (direct Postgres introspection via `meta`, gated by the service role key) is the exception: it carries its own stricter per-route CORS override, restricted to `SUPABASE_PUBLIC_URL` and `localhost`/`127.0.0.1` origins, since there's no legitimate reason for this route to be called from an arbitrary third-party origin.
...
- **`403 Forbidden` on `/api/mcp` or `/mcp`.** These routes are blocked by default. See [Enabling MCP Server Access](/guides/self-hosting/enable-mcp).
...
- [New API Keys and Asymmetric Authentication](/guides/self-hosting/self-hosted-auth-keys) - Background on opaque keys and asymmetric JWTs
- [Configure Reverse Proxy and HTTPS](/guides/self-hosting/self-hosted-proxy-https) - Caddy or Nginx in front of the gateway
