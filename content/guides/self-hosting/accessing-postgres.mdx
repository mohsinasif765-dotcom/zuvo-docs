...
This guide explains how to connect to Postgres in self-hosted Zuvo, using the Supavisor pooler, the optional PgBouncer pooler, or a direct connection.

Self-hosted Zuvo uses [Supavisor](https://github.com/supabase/supavisor) as its default connection pooler. A pooler sits in front of Postgres and shares a small set of database connections across many clients, which avoids exhausting Postgres connection limits.

## Choose a connection mode

Self-hosted Zuvo offers three ways to reach Postgres:
...
To change `max_connections` or other Postgres settings, refer to [custom Postgres configuration](/guides/self-hosting/postgres-upgrade-17#custom-postgres-configuration).
...
Self-hosted Zuvo includes an optional [PgBouncer](https://www.pgbouncer.org/) override. It disables Supavisor and runs PgBouncer in transaction mode on `POOLER_PROXY_PORT_TRANSACTION`.
