...
description: 'Set up social login (OAuth/OIDC) providers for self-hosted Zuvo with Docker.'
subtitle: 'Set up social login (OAuth/OIDC) providers for self-hosted Zuvo with Docker.'
---
...
- A working self-hosted Zuvo installation. See [Self-Hosting with Docker](/guides/self-hosting/docker).
- `API_EXTERNAL_URL` set to the publicly reachable URL of your Zuvo instance (e.g., `https://<your-domain>/auth/v1`).

<Admonition type="danger">

HTTPS is required by most OAuth providers - `http://` callback URLs are rejected (except `localhost`). See the HTTPS [how-to guide](/guides/self-hosting/self-hosted-proxy-https) for setup instructions.
...
| ----------------- | ---------------- | ------------------------------- | --------------------------------------------------------------------- |
| Apple             | `APPLE_`         | -                               | [Login with Apple](/guides/auth/social-login/auth-apple)         |
| Azure (Microsoft) | `AZURE_`         | `URL` (tenant URL)              | [Login with Azure](/guides/auth/social-login/auth-azure)         |
| Bitbucket         | `BITBUCKET_`     | -                               | [Login with Bitbucket](/guides/auth/social-login/auth-bitbucket) |
| Discord           | `DISCORD_`       | -                               | [Login with Discord](/guides/auth/social-login/auth-discord)     |
| Facebook          | `FACEBOOK_`      | -                               | [Login with Facebook](/guides/auth/social-login/auth-facebook)   |
| Figma             | `FIGMA_`         | -                               | [Login with Figma](/guides/auth/social-login/auth-figma)         |
| GitHub            | `GITHUB_`        | `URL` (for GitHub Enterprise)   | [Login with GitHub](/guides/auth/social-login/auth-github)       |
| GitLab            | `GITLAB_`        | `URL` (for self-hosted GitLab)  | [Login with GitLab](/guides/auth/social-login/auth-gitlab)       |
| Google            | `GOOGLE_`        | -                               | [Login with Google](/guides/auth/social-login/auth-google)       |
| Kakao             | `KAKAO_`         | -                               | [Login with Kakao](/guides/auth/social-login/auth-kakao)         |
| Keycloak (OIDC)   | `KEYCLOAK_`      | `URL` (realm URL, **required**) | [Login with Keycloak](/guides/auth/social-login/auth-keycloak)   |
| LinkedIn (OIDC)   | `LINKEDIN_OIDC_` | -                               | [Login with LinkedIn](/guides/auth/social-login/auth-linkedin)   |
| Notion            | `NOTION_`        | -                               | [Login with Notion](/guides/auth/social-login/auth-notion)       |
| Slack (OIDC)      | `SLACK_OIDC_`    | -                               | [Login with Slack](/guides/auth/social-login/auth-slack)         |
| Snapchat          | `SNAPCHAT_`      | -                               | -                                                                     |
| Spotify           | `SPOTIFY_`       | -                               | [Login with Spotify](/guides/auth/social-login/auth-spotify)     |
| Twitch            | `TWITCH_`        | -                               | [Login with Twitch](/guides/auth/social-login/auth-twitch)       |
| Twitter           | `TWITTER_`       | -                               | [Login with Twitter](/guides/auth/social-login/auth-twitter)     |
| WorkOS            | `WORKOS_`        | -                               | [Login with WorkOS](/guides/auth/social-login/auth-workos)       |
| Zoom              | `ZOOM_`          | -                               | [Login with Zoom](/guides/auth/social-login/auth-zoom)           |
...
- Make sure `SITE_URL` is set to `http://localhost:3000` in your self-hosted Zuvo `.env` configuration
...
For detailed client-side integration, see [Social Login](/guides/auth/social-login).
...
- [Redirect URLs](/guides/auth/redirect-urls)
