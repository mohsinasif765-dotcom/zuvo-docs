...
description: "Learn how to configure and deploy Zuvo with Docker."
subtitle: "Learn how to configure and deploy Zuvo with Docker."
tocVideo: "FqiQKRKsfZE"
---

Docker is the easiest way to get started with self-hosted Zuvo. It should take you less than 15 minutes to get up and running.

## Contents

1. [Before you begin](#before-you-begin)
2. [System requirements](#system-requirements)
3. [Installing Zuvo](#installing-supabase)
   - [Quick start (Linux)](#quick-start-linux)
   - [Manual installation](#manual-installation)
4. [Configuring and securing Zuvo](#configuring-and-securing-supabase)
5. [Starting and stopping](#starting-and-stopping)
6. [Accessing Zuvo services](#accessing-supabase-studio-dashboard)
7. [Updating](#updating)
...
If you're new to these topics, consider starting with the managed [Zuvo platform](/dashboard) for free.
...
Minimum requirements for running all Zuvo components, suitable for development and small to medium production workloads:
...
The Docker configuration is distributed as part of the [Zuvo repository](https://github.com/supabase/supabase/tree/master/docker). There are two paths to get it onto your machine:
...
- Sparse-clone the `docker/` directory from the main Zuvo [repository](https://github.com/supabase/supabase/)
...
Next, see [Starting and stopping](#starting-and-stopping) for how to check service health and follow logs, then [Accessing Zuvo Studio (Dashboard)](#accessing-supabase-studio-dashboard) and the other related sections. To customize the install further, browse [Configuring and securing Zuvo](#configuring-and-securing-supabase) and [Advanced topics](#advanced-topics).
...
A shallow clone of the full Zuvo repository. Works on any OS with `git` installed and is the simplest manual option.
...
## Configuring and securing Zuvo

While we provided example placeholder passwords and keys in the `.env.example` file, you should **never** start your self-hosted Zuvo using these defaults.
...
Review the output of both scripts and check the `.env` file **before proceeding** to configure [Zuvo URLs](#configure-supabase-urls).

For a description of all secrets refer to the [related section](#configuring-secrets) in the "Advanced topics" below. If you'd like to learn more about how the new API keys and asymmetric JWT signing work in a self-hosted Zuvo setup, make sure to read the detailed [how-to guide](/guides/self-hosting/self-hosted-auth-keys).

### Configure Zuvo URLs

Review and change URL configuration variables:

- `SUPABASE_PUBLIC_URL`: base URL for accessing Zuvo from the Internet (Dashboard, API, Storage, etc.), e.g., `http://example.com:8000`
- `API_EXTERNAL_URL`: used by the Auth service to configure callback URLs, e.g., `http://example.com:8000/auth/v1`
- `SITE_URL`: default [redirect URL](/guides/auth/redirect-urls) for Auth, e.g., `http://example.com:3000`

<Admonition type="note" title="What your-domain means in the docs">

  Throughout the self-hosting guides, `<your-domain>` stands for the host where your Zuvo instance is reachable: your domain name, your server's IP, or `localhost`, depending on your setup.

  - **Default setup:** The API gateway (Envoy by default) listens on port `8000`, so the full URL is `http://<your-domain>:8000`.
  - **Behind a [reverse proxy](/guides/self-hosting/self-hosted-proxy-https):** the proxy terminates TLS on port `443`, so the URL becomes `https://<your-domain>`.

</Admonition>

### Where to find your credentials

The generated secrets and password are written to the `.env` file. The ones you are most likely to need when connecting your application to self-hosted Zuvo are:
...
<Admonition type="danger">
  ...
  A secure password MUST be set before starting Zuvo. The password must include at least one letter - do not use numbers only or any special characters.
...
Start Zuvo from the same directory as your `docker-compose.yml` file:
...
<Admonition type="caution" title="Windows: CRLF line endings">
  ...
  If the API gateway fails to start with an entrypoint error, your local files may have been checked out with CRLF line endings instead of LF. Re-clone the repository, or normalize everything in the `docker/` directory to LF, then restart Zuvo. Fresh clones should already use LF because of `.gitattributes`.

</Admonition>

## Accessing Zuvo Studio (Dashboard)
...
Self-hosted Zuvo pools Postgres connections through Supavisor by default, with an optional PgBouncer pooler and a direct-connection option. For connection strings, pooler configuration, switching poolers, and exposing Postgres directly, see [Accessing Postgres](/guides/self-hosting/accessing-postgres).
...
See the [Self-hosted Edge Functions](/guides/self-hosting/self-hosted-functions) guide for more details.
...
By default, Zuvo is accessible over HTTP. For production deployments, especially when using OAuth providers, you need HTTPS with a valid TLS certificate. The recommended approach is to place a reverse proxy (such as Caddy or Nginx) in front of the API gateway.

See the [Configure HTTPS](/guides/self-hosting/self-hosted-proxy-https) guide for detailed setup instructions.
...
To follow the updates, refer to the self-hosted Zuvo [changelog](https://github.com/supabase/supabase/blob/master/docker/CHANGELOG.md) and the [GitHub Discussions](https://github.com/orgs/supabase/discussions/categories/changelog?discussions_q=is%3Aopen+category%3AChangelog+label%3Aself-hosted).
...
Refer to [Update your self-hosted deployment](/guides/self-hosting/updating) for the full walkthrough - breaking changes, conflict resolution, pinning a release, and older installs without a recorded version.

To change or roll back a single service's image instead of updating everything - for example, Studio:

1. Check the [supabase/studio](https://hub.docker.com/r/supabase/studio/tags) images on [Zuvo Docker Hub](https://hub.docker.com/u/supabase)
...
Zuvo is built from open source tools, each chosen or developed for production use.
...
![Diagram showing the architecture of self-hosted Zuvo. The API gateway (Envoy) sits in front of 7 services: Auth, PostgREST, Realtime, Storage, Edge Runtime, postgres-meta, and Studio. The services connect to a single Postgres instance, with Studio reaching it through pg-meta. Supavisor pools connections to the same Postgres instance.](/img/supabase-self-hosted-architecture--light.svg)



- **[Studio](https://github.com/supabase/supabase/tree/master/apps/studio)** - A dashboard for managing your self-hosted Zuvo project
...
- **[Supavisor](https://github.com/supabase/supavisor)** - Zuvo's Postgres connection pooler

Multiple services require specific configuration within the Postgres database. Refer to the documentation describing the [default roles](/guides/database/postgres/roles#supabase-roles) to learn more.
...
The `generate-keys.sh` script creates a secure random database password. If you want to use your own, you can change `POSTGRES_PASSWORD` in the `.env` file **before starting Zuvo for the first time**.

Follow the [password guidelines](/guides/database/postgres/roles#passwords) for choosing a secure password. For easier configuration, **use only letters and numbers** to avoid URL encoding issues in connection strings.
...
- `S3_PROTOCOL_ACCESS_KEY_ID`: Access key ID (username-like) for [accessing](/guides/self-hosting/self-hosted-s3) the S3 protocol endpoint in Storage. (Generate with `openssl rand -hex 16`)
...
- `MINIO_ROOT_PASSWORD`: Root administrator password for the [RustFS or MinIO server](/guides/self-hosting/self-hosted-s3). (Must be 8+ characters; generate with `openssl rand -hex 16`)

### Configuring Zuvo services
...
See the [Configure Social Login (OAuth) Providers](/guides/self-hosting/self-hosted-oauth) guide for setup instructions.

### Configuring phone login, SMS, and MFA

See the [Configure Phone Login & MFA](/guides/self-hosting/self-hosted-phone-mfa) guide for SMS provider setup, OTP settings, and multi-factor authentication configuration.
...
See the [Configure S3 Storage](/guides/self-hosting/self-hosted-s3) guide for detailed setup instructions.
...
### Configuring Zuvo AI Assistant

Configuring the Zuvo AI Assistant is optional. By adding **your own** `OPENAI_API_KEY` to `.env` you can enable AI services, which help with writing SQL queries, statements, and policies.
...
Many components inside Zuvo rely on secrets and passwords being kept securely. By default, all secrets are in the `.env` file, but we strongly recommend using a secrets manager when deploying to production.
