...
description: 'Start a new self-hosted Zuvo deployment with Postgres 17, or upgrade an existing Postgres 15 installation.'
subtitle: 'Start a new self-hosted Zuvo deployment with Postgres 17, or upgrade an existing Postgres 15 installation.'
---

Self-hosted Zuvo ships with Postgres 17 by default. This guide covers two scenarios:
...
The rest of the setup is unchanged (see the Docker install [guide](/guides/self-hosting/docker)).
...
2. Pulls supplemental upgrade scripts from Zuvo's [Postgres](https://github.com/supabase/postgres) repository
3. Stops all self-hosted Zuvo containers
...
7. Starts self-hosted Zuvo with Postgres 17
...
- All self-hosted Zuvo containers must be running before starting the upgrade
- Requires `bash`
...
None of the above extensions are installed by default in the self-hosted Zuvo setup. If you have installed any of them manually and need to keep them, **do not proceed** with the upgrade.
...
This restores the original data directory, fixes file ownership on the `db-config` volume, and configures Zuvo to use the old Postgres 15 image. Zuvo's Postgres 15 and 17 images use different user IDs, so the `chown` and startup must run as Postgres 15.
...
This is a Zuvo Postgres 17 image feature. The Postgres 15 image does not load files from `conf.d/`.
...
After both phases, the upgrade script applies migrations that normally run only on a fresh install, and reconciles installed extensions to the target image's versions. It then preserves the original Postgres 15 data directory as a backup and starts the full Zuvo stack with Postgres 17.
...
Start self-hosted Zuvo with Postgres 15:
