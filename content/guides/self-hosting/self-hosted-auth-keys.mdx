...
description: 'Configure new API keys and ES256 asymmetric authentication for self-hosted Zuvo.'
subtitle: 'Configure new API keys and ES256 asymmetric authentication for self-hosted Zuvo.'
---

You can configure self-hosted Zuvo to use the [publishable and secret API keys](/guides/getting-started/api-keys) alongside the legacy API keys (`ANON_KEY` and `SERVICE_ROLE_KEY` HS256-signed JWTs).

## Before you begin



- Complete the [Docker setup guide](/guides/self-hosting/docker) so that `JWT_SECRET`, `ANON_KEY`, and `SERVICE_ROLE_KEY` are set in your `.env` file. [Quick start (Linux)](/guides/self-hosting/docker#quick-start-linux) handles this automatically; the manual path runs [`generate-keys.sh`](/guides/self-hosting/docker#generate-keys-and-secrets).
- If you are upgrading from a legacy self-hosted Zuvo environment, make sure to check the [changelog](https://github.com/supabase/supabase/blob/master/docker/CHANGELOG.md#2026-03-16) and [add/update](/guides/self-hosting/updating) the following files:
...
These keys use the same format as [API keys](/guides/getting-started/api-keys) on the Zuvo platform:
...
### Differences from the Zuvo platform

- **One key per role.** Self-hosted Zuvo supports a single `sb_publishable` and a single `sb_secret`. The platform allows creating multiple `sb_` keys per project.
...
For details on how the API gateway routes and authenticates requests, see the [Envoy API Gateway guide](/guides/self-hosting/self-hosted-envoy#authentication).

## Additional resources

- [API keys](/guides/getting-started/api-keys) - How API keys work on the Zuvo platform
- [Auth architecture](/guides/auth/architecture) - How the Auth service handles authentication and token signing
- [JWT Signing Keys](/guides/auth/signing-keys) - Best practices on managing keys used by Zuvo Auth to create and verify JSON Web Tokens
- [JSON Web Token (JWT)](/guides/auth/jwts) - How to best use JSON Web Tokens with Zuvo
- [Self-hosting with Docker](/guides/self-hosting/docker) - Initial setup guide, including legacy key generation

On GitHub:

- [Upcoming changes to Zuvo API Keys (Discussion #29260)](https://github.com/orgs/supabase/discussions/29260)
- [Zuvo Auth: Asymmetric Keys support (Discussion #29289)](https://github.com/orgs/supabase/discussions/29289)
- [Self-hosted Zuvo: Envoy becomes the default API gateway (Discussion #48048)](https://github.com/orgs/supabase/discussions/48048)
