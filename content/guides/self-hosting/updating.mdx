...
description: 'Update an existing self-hosted Zuvo deployment to a newer release.'
subtitle: 'Update an existing self-hosted Zuvo deployment to a newer release.'
---

A supplemental script (`update.sh`) pulls a newer version of the self-hosted Zuvo configuration on top of your existing deployment. It uses a three-way merge, so your secrets, overrides, and local edits survive, while merge conflicts are surfaced.
...
A conflict means you edited a [self-hosted Zuvo configuration](https://github.com/supabase/supabase/tree/master/docker) file and the release changed the same lines.
...
6. Handle Postgres. Postgres 17 became the default in v0.6.0. If you are still on Postgres 15, do not recreate straight onto 17 - follow [Upgrade to Postgres 17](/guides/self-hosting/postgres-upgrade-17), or pin Postgres 15 with the `docker-compose.pg15.yml` override. Read the [changelog](https://github.com/supabase/supabase/blob/master/docker/CHANGELOG.md) for the breaking changes across the range you are crossing.
