...
All changes made in this screen will disconnect all your connected clients to ensure Realtime starts with the appropriate settings and all changes are stored in Zuvo middleware.

</Admonition>



![Usage page navigation bar](/img/guides/platform/realtime/realtime-settings--light.png)



You can set the following settings using the Realtime Settings screen in your Dashboard. For the ceilings your plan allows, see [Realtime Limits](/guides/realtime/limits); the rate and payload limits are only editable while your organization's spend cap is disabled. For the errors below, see [Operational Error Codes](/guides/realtime/error_codes).
...
Determines whether Realtime allows public channels, or restricts your project to private channels with [Realtime Authorization](/guides/realtime/authorization).
...
See [Database connections](/guides/realtime/concepts#database-connections) for the defaults per compute size.

### Postgres Changes connection pool size

**Type:** Number of connections · **Range:** 1 to 20 · **Default:** 2

Determines the number of connections used to create [Postgres Changes](/guides/realtime/postgres-changes) subscriptions when clients subscribe. It's only used while subscriptions are created; streaming the changes uses a separate connection.
...
**Type:** Number of clients · **Range:** 1 to your plan's [concurrent connections](/guides/realtime/limits#limits-by-plan) limit · **Default:** your plan's limit
...
**Type:** Number of events per second · **Range:** 1 to your plan's [messages per second](/guides/realtime/limits#limits-by-plan) limit · **Default:** your plan's limit
...
**Type:** Number of events per second · **Range:** 1 to your plan's [presence messages per second](/guides/realtime/limits#limits-by-plan) limit · **Default:** your plan's limit
...
A separate per-client limit also applies to presence, independent of this project-wide setting. A client that exceeds it is closed with `Client presence rate limit exceeded`. See [Realtime Limits](/guides/realtime/limits) for the value on your plan.

</Admonition>

### Max payload size in KB

**Type:** Size in KB · **Range:** 1 to your plan's [broadcast payload size](/guides/realtime/limits#limits-by-plan) limit · **Default:** your plan's limit
...
- **Too high**: large messages increase memory and bandwidth usage on every subscriber, since each message is delivered to all clients on the channel. Postgres Changes payloads have a [separate limit](/guides/realtime/limits#postgres-changes-payload-limit).
