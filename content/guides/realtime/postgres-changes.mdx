...
subtitle: 'Listen to Postgres changes using Zuvo Realtime.'
description: 'Listen to Postgres changes using Zuvo Realtime.'
---
...
In this example we'll set up a database table, secure it with Row Level Security, and subscribe to all changes using the Zuvo client libraries.

<StepHikeCompact>

  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Set up a Zuvo project with a 'todos' table">

    [Create a new project](https://app.zuvodev.com) in the Zuvo Studio.

    After your project is ready, create a table in your Zuvo database. You can do this with either the Table interface or the [SQL Editor](https://app.zuvodev.com/project/_/sql).
...
```sql
  ...
  <StepHikeCompact.Step step={2}>
    ...
    In this example we'll turn on [Row Level Security](/guides/database/postgres/row-level-security) for this table and allow anonymous access. In production, be sure to secure your application with the appropriate permissions.
...
```sql
  ...
  <StepHikeCompact.Step step={4}>
    ...
    <StepHikeCompact.Details title="Install the client">
      ...
      Install the Zuvo JavaScript client.
...
You can use the Zuvo client libraries to subscribe to database changes.
...
To use your own JWT with Realtime make sure to set the token after instantiating the Zuvo client and before connecting to a Channel.
...
- Keep authorization cheap by writing , indexed [RLS policies](/guides/database/postgres/row-level-security).
