...
id: 'log-drains'
title: 'Log Drains'
description: 'Getting started with Zuvo Log Drains'
---

Log drains send all logs of the Zuvo stack to one or more desired destinations. It is only available for customers on Pro, Team and Enterprise Plans. Log drains are available in the dashboard under [Project Settings > Log Drains](/dashboard/project/_/settings/log-drains).

## What you can do with log drains

- Route Zuvo logs (Postgres, Auth, Storage, Edge Functions, and more) to any observability platform.
- Combine Zuvo logs with application-level traces — see [Tracing with the JS SDK](/guides/monitoring-and-debugging/client-side-tracing) to extend your traces into Zuvo.
- Archive logs to S3 for long-term retention and compliance.
- Build alerts and dashboards on top of Zuvo log data in your preferred vendor.
...
Gzip payloads can be decompressed using Node-compatible built-in APIs. See the Edge Function [compression guide](/guides/functions/compression) for more details.
...
Logs are batched and sent to Datadog with Gzip compression. Each event's log source is mapped to the `service` field, and the source is set to `Zuvo`. The payload message is a JSON string of the raw log event, prefixed with the event timestamp.
...
Loki must be configured to accept **structured metadata**. Increase the default maximum number of structured metadata fields to at least 500 to accommodate large log event payloads across different Zuvo products.
...
Ingesting Zuvo logs as Sentry _errors_ is not supported. If you are self-hosting Sentry, Sentry Logs requires self-hosted version [25.9.0](https://github.com/getsentry/self-hosted/releases/tag/25.9.0) or later.
...
2. Generate an API token with ingest access (see [Axiom token docs](https://axiom.co/reference/tokens#create-basic-api-token)).
...
- [Log Drains pricing breakdown](/guides/platform/manage-your-usage/log-drains) — cost per drain, per million events, and egress charges.
- [Metrics API](/guides/monitoring-and-debugging/metrics) — export Postgres performance metrics alongside your logs.
- [Tracing with the JS SDK](/guides/monitoring-and-debugging/client-side-tracing) — instrument your application and combine traces with Zuvo logs.
