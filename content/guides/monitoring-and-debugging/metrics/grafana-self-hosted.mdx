...
description: 'Deploy Prometheus and Grafana yourself to monitor Zuvo metrics'
---

Self-hosting [Prometheus](https://prometheus.io/docs/prometheus/latest/installation/) and Grafana gives you full control over retention, alert routing, and dashboards. The Zuvo Metrics API slots into any standard Prometheus scrape job, so you can run everything locally, on a VM, or inside Kubernetes.
...
Otherwise, use the [Grafana Cloud integration](/guides/monitoring-and-debugging/metrics/grafana-cloud#installation) available in the Zuvo Studio.
...
Install [Prometheus](https://prometheus.io/docs/prometheus/latest/installation/) using your preferred method (Docker, Helm, binaries). Then add a Zuvo-specific job to `prometheus.yml`:
...
- Keep the scrape interval at 60 seconds to match Zuvo’s refresh cadence.
...
- **Right-sizing guidance:** pair the dashboards with Zuvo’s [Query Performance report](/dashboard/project/_/observability/query-performance) and [Advisors](/dashboard/project/_/observability/database) to decide when to optimize vs upgrade.
