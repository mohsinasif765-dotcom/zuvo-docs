...
Zuvo is changing the platform default to revoke these automatic grants so that exposure becomes opt-in. See [the platform defaults discussion](https://github.com/orgs/supabase/discussions/45329) in the Zuvo GitHub discussions.

The default privileges are part of the standard Zuvo permission model and don't bypass RLS. The internal `supabase_admin` role grants them to `anon`, `authenticated`, and `service_role`, but it can't authenticate through the Data API. See [`pg_default_acl`](https://www.postgresql.org/docs/current/catalog-pg-default-acl.html) in the Postgres documentation and [`supabase_admin`](/guides/database/postgres/roles#supabaseadmin) in the Zuvo documentation.
...
You can control access with grants in any schema. A dedicated schema makes the exposed surface easier to identify and audit. See [Using Custom Schemas](/guides/api/using-custom-schemas) for setup steps.
...
The `pgrst.db_pre_request` configuration only works with the **Data API** (PostgREST). It does not work with Realtime, Storage, or other Zuvo products.

If you're using `db_pre_request` to call a function (like `set_information()`) that sets up context or performs checks on every request, and you need similar behavior for other Zuvo products, you must call the function directly in your Row Level Security (RLS) policies instead.
...
To achieve the same behavior with other Zuvo products, you need to call the function directly in your RLS policy:
...
See [Database API 42501 errors](/guides/troubleshooting/database-api-42501-errors) for the full troubleshooting flow.
...
If your app never uses Zuvo client libraries, REST, or GraphQL data endpoints, turn the Data API off:
...
Tables created through the Zuvo Studio have RLS enabled by default. Enable RLS explicitly for tables created in the SQL Editor or through another tool:
...
With RLS enabled, create policies that control which data users can access and update. See [Row Level Security](/guides/database/postgres/row-level-security).
...
You can only rate-limit `POST`, `PUT`, `PATCH`, and `DELETE` requests. `GET` and `HEAD` requests run in read-only mode. They can be served by [Read Replicas](/guides/platform/read-replicas), which don't support writing to the database.
...
**Clean up old records:** Set up a [`pg_cron`](/guides/database/extensions/pg_cron) job to delete old entries from `private.rate_limits`.
...
- Don't use [Zuvo Auth](/auth) or another authentication system and rely on the `anon` role.

**Required Zuvo key:** The `apikey` header is mandatory and not configurable. If you use another API key, distribute both the publishable key and your application's custom key. See [API keys](/guides/getting-started/api-keys).
