---
title: Consuming Zuvo Queue Messages with Edge Functions
subtitle: 'Learn how to consume Zuvo Queue messages server-side with a Zuvo Edge Function'
---

This guide helps you read & process queue messages server-side with a Zuvo Edge Function. Read [Queues API Reference](/guides/queues/api) for more details on our API.

## Concepts

Zuvo Queues is a pull-based Message Queue consisting of three main components: Queues, Messages, and Queue Types. You should already be familiar with the [Queues Quickstart](/guides/queues/quickstart).

### Consuming messages in an Edge Function

This is a Zuvo Edge Function that reads 5 messages off the queue, processes each of them, and deletes each message when it is done.
...
You might find this kind of setup handy to run with [Zuvo Cron](/guides/cron). You can set up Cron so that every N number of minutes or seconds, the Edge Function will run and process a number of messages off the queue.

Similarly, you can invoke the Edge Function on command at any given time with [`supabase.functions.invoke`](/guides/functions/quickstart-dashboard#usage).
