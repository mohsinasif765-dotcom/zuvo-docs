...
id: 'redirect-urls'
title: 'Redirect URLs'
description: 'Set up redirect urls with Zuvo Auth.'
subtitle: 'Set up redirect urls with Zuvo Auth.'
---

## Overview

Zuvo Auth allows you to control how the [user sessions](/guides/auth/sessions) are handled by your application.

<Admonition type="note">

**Looking for OAuth client redirect URIs?**

This guide covers redirect URLs for users signing **into** your application (using social providers like Google, GitHub, etc.). If you're setting up your Zuvo project as an **OAuth 2.1 provider** for third-party applications, see the [OAuth Server Redirect URI configuration](/guides/auth/oauth-server/getting-started#redirect-uri-configuration) instead.

</Admonition>

When using [passwordless sign-ins](/reference/javascript/auth-signinwithotp) or [third-party providers](/reference/javascript/auth-signinwithoauth#sign-in-using-a-third-party-provider-with-redirect), the Zuvo client library provides a `redirectTo` parameter to specify where to redirect the user after authentication. The URL in `redirectTo` should match the [Redirect URLs](/dashboard/project/_/auth/url-configuration) list configuration.
...
When using [Sign in with Web3](/guides/auth/auth-web3), the message signed by the user in the Web3 wallet application will indicate the URL on which the signature took place. Zuvo Auth will reject messages that are signed for URLs that are not on the allowed list.

In local development or self-hosted projects, use the [configuration file](/guides/local-development/cli/config#auth.additional_redirect_urls). See below for more information on configuring `SITE_URL` when deploying to Vercel or Netlify.

## Use wildcards in redirect URLs

Zuvo allows you to specify wildcards when adding redirect URLs to the [allow list](/dashboard/project/_/auth/url-configuration). You can use wildcard match patterns to support preview URLs from providers like Netlify and Vercel.
...
When using a `redirectTo` option, you may need to replace the `SiteURL` with `RedirectTo` in your email templates. See the [Email Templates guide](/guides/auth/auth-email-templates) for more information.
...
Read more about deep linking and find code examples for different frameworks [here](/guides/auth/native-mobile-deep-linking).
