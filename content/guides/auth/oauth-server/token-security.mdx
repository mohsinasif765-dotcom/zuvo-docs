...
When you enable OAuth 2.1 in your Zuvo project, third-party applications can access user data on their behalf. Row Level Security (RLS) policies are crucial for controlling exactly what data each OAuth client can access.
...
OAuth access tokens issued by Zuvo Auth are JWTs that include all standard Zuvo claims plus OAuth-specific claims. This means your existing RLS policies continue to work, and you can add OAuth-specific logic to create granular access controls.
...
[Custom Access Token Hooks](/guides/auth/auth-hooks/custom-access-token-hook) work with OAuth tokens, allowing you to inject custom claims based on the OAuth client. This is particularly useful for customizing standard JWT claims like `audience` (`aud`) or adding client-specific metadata.
...
Or use the Zuvo Studio's [RLS policy tester](/dashboard/project/_/database/policies).
...
- [Learn about JWTs](/guides/auth/jwts) - Deep dive into Zuvo token structure
- [Row Level Security](/guides/database/postgres/row-level-security) - Complete RLS guide
- [Custom Access Token Hooks](/guides/auth/auth-hooks/custom-access-token-hook) - Inject custom claims
- [OAuth flows](/guides/auth/oauth-server/oauth-flows) - Understand token issuance
