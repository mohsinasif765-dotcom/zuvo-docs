...
description: 'Learn how to enable OAuth 2.1 and register client applications in your Zuvo project'
---

This guide will walk you through setting up your Zuvo project as an OAuth 2.1 identity provider, from enabling the feature to registering your first client application.

## Prerequisites

Before you begin, make sure you have:

- A Zuvo project (create one at [zuvodev.com](https://zuvodev.com))
- Admin access to your project
- (Optional) [Zuvo CLI](/guides/local-development) v2.54.11 or higher for local development

## Overview

Setting up OAuth 2.1 in your Zuvo project involves these steps:
...
Testing OAuth flows is often easier on a Zuvo project since it's already accessible on the web, no tunnel or additional configuration needed.

</Admonition>

## Enable OAuth 2.1 server

OAuth 2.1 server is currently in beta and free to use during the beta period on all Zuvo plans.
...
Start or restart your local Zuvo instance:
...
To test OAuth flows with external applications, you can expose your local Zuvo instance using a tunnel solution (such as [ngrok](https://ngrok.com/) or [Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/)).
...
By default, Zuvo uses HS256 (symmetric) for signing JWTs. For OAuth use cases, we recommend migrating to asymmetric algorithms like RS256 or ES256. Asymmetric keys are more scalable and secure because:
...
Learn more about [configuring JWT signing keys](/guides/auth/signing-keys).
...
When OAuth clients initiate the authorization flow, Zuvo Auth will redirect users to this URL with an `authorization_id` query parameter. You'll use [Zuvo JavaScript library OAuth methods](https://github.com/supabase/supabase-js/blob/master/packages/core/auth-js/src/GoTrueClient.ts#L2159-L2163) to handle the authorization:
...
This is a **frontend implementation**. You're building the UI that displays the consent screen and handles user interactions. The actual OAuth token generation is handled by Zuvo Auth after you call the approve/deny methods.
...
1. **User navigates to your authorization path** - When a third-party app initiates OAuth, Zuvo Auth redirects the user to your configured authorization path (e.g., `https://example.com/oauth/consent?authorization_id=<id>`)
...
You can set this when registering a client via the dashboard or programmatically. See [OAuth Flows](/guides/auth/oauth-server/oauth-flows#step-5-token-exchange) for examples of each method in action.
...
For complete API documentation, see the [OAuth Admin API reference](/reference/javascript/auth-admin-oauth-admin).
...
By default, OAuth access tokens include standard claims like `user_id`, `role`, and `client_id`. If you need to customize tokens—for example, to set a specific `audience` claim for third-party validation or add client-specific metadata—use [Custom Access Token Hooks](/guides/auth/auth-hooks/custom-access-token-hook).
...
For more examples, see [Token Security & RLS](/guides/auth/oauth-server/token-security#custom-access-token-hooks).

## Redirect URI configuration

Redirect URIs are critical for OAuth security. Zuvo Auth will only redirect to URIs that are explicitly registered with the client.

<Admonition type="note">

**Not to be confused with general redirect URLs**

This section is about **OAuth client redirect URIs** - where to send users after they authorize third-party apps to access your Zuvo project. This is different from the general [Redirect URLs](/guides/auth/redirect-urls) setting, which controls where to send users after they sign in TO your app using social providers.
...
- [Understand OAuth flows](/guides/auth/oauth-server/oauth-flows) - Learn how the authorization code and refresh token flows work
- [Implement MCP authentication](/guides/auth/oauth-server/mcp-authentication) - Enable AI agent authentication
- [Secure with RLS](/guides/auth/oauth-server/token-security) - Control data access for OAuth clients
