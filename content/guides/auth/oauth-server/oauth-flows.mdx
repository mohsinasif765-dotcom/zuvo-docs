...
Zuvo Auth implements OAuth 2.1 with OpenID Connect (OIDC), supporting the authorization code flow with PKCE and refresh token flow. This guide explains how these flows work in detail.

<Admonition type="note">

This guide explains the OAuth 2.1 flows for **third-party client applications** that authenticate with your Zuvo project. These flows require custom implementation and are not available in the `@supabase/supabase-js` library. The `supabase-js` library is for authenticating **with** Zuvo Auth as an identity provider, not for building your own OAuth server.

</Admonition>

## Supported grant types

Zuvo Auth supports two OAuth 2.1 grant types:
...
1. **Client initiates authorization** - Third-party app redirects user to Zuvo Auth's authorize endpoint
2. **Zuvo validates and redirects** - Zuvo Auth validates OAuth parameters and redirects user to your configured authorization URL
...
4. **Authorization code issued** - Zuvo Auth generates a short-lived authorization code and redirects back to client
...
- Third-party client redirects user to **Zuvo Auth's authorize endpoint** (not directly to your UI)
- Zuvo Auth validates OAuth parameters and redirects to **your authorization path**
- Your frontend UI handles authentication and consent using `supabase-js` OAuth methods
- Zuvo Auth handles all backend OAuth logic (code generation, token issuance)
...
After receiving the authorization request, Zuvo Auth validates the OAuth parameters (client_id, redirect_uri, PKCE, etc.) and then redirects the user to your configured **authorization path** (e.g., `https://example.com/oauth/consent?authorization_id=<id>`).
...
This is a **frontend implementation** using `supabase-js`. Zuvo Auth handles all the backend OAuth logic (generating authorization codes, validating requests, etc.) after you call the approve/deny methods.

See the [Getting Started guide](/guides/auth/oauth-server/getting-started#example-authorization-ui) for complete implementation examples.

### Step 4: Authorization code issued

If the user approves access, Zuvo Auth redirects back to the client's redirect URI with an authorization code:
...
If the user denies access, Zuvo Auth redirects with error information in query parameters:
...
The client exchanges the authorization code for tokens by making a POST request to the token endpoint. How the client authenticates depends on its `token_endpoint_auth_method` (set during [client registration](/guides/auth/oauth-server/getting-started#token-endpoint-authentication-method)).
...
On success, Zuvo Auth returns a JSON response with tokens:
...
Access tokens are JWTs containing standard Zuvo claims plus OAuth-specific claims:
...
All other claims follow the standard [Zuvo JWT structure](/guides/auth/jwts).
...
Zuvo Auth supports OpenID Connect, an identity layer on top of OAuth 2.1.
...
ID tokens are JWTs that contain user identity information. They are signed by Zuvo Auth and can be verified by clients.
...
Zuvo Auth exposes OpenID Connect and OAuth 2.1 discovery endpoints that describe its capabilities:
...
Learn how to [configure asymmetric JWT signing keys](/guides/auth/signing-keys) in your project.
...
Zuvo Auth exposes a JSON Web Key Set (JWKS) endpoint containing public keys for token verification:
...
1. **Signature** - Token is signed by Zuvo Auth
...
For complete API reference, see the [OAuth methods in supabase-js](/reference/javascript/auth-admin-oauth-server).
...
- [Learn about JWTs](/guides/auth/jwts) - Understand Zuvo token structure
