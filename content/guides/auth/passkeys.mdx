...
[Passkeys](https://fidoalliance.org/passkeys/) are a passwordless credential built on the [WebAuthn](https://www.w3.org/TR/webauthn-3/) standard. The user proves possession of a private key stored on their device (or password manager) using biometrics, a PIN, or a hardware security key. The matching public key is registered with Zuvo Auth and used to verify future sign-ins. Passkeys are phishing-resistant and remove the need to manage shared secrets.

<Admonition type="caution" title="Experimental">

Passkey support is experimental. The API may change without notice. You must explicitly opt-in when creating the Zuvo client. See [Enable in the client](#enable-in-the-client).
...
1. **Options**: the client requests a challenge from Zuvo Auth.
...
3. **Verify**: the signed response is sent back to Zuvo Auth, which validates the challenge and either stores the new credential or issues a session.

Zuvo Auth uses [discoverable credentials](https://www.w3.org/TR/webauthn-3/#discoverable-credential) for sign-in. The user does not need to provide an email, phone, or username — the authenticator resolves the account from the credential it stores.
...
You can also configure passkeys via the [Management API](/reference/api/introduction):
...
final supabase = Zuvo.instance.client;
```
...
```swift
@_spi(Experimental) import Zuvo
```



The `ZuvoClient` itself needs no extra configuration — the experimental SPI is enabled at the import site, not at client initialization.
...
`auth.registerPasskey()` runs the full WebAuthn ceremony. It fetches a challenge, invokes the platform passkey API, and verifies the response with Zuvo Auth.
...
try {
  ...
  // The Zuvo server rejected the credential.
  print(e);
} catch (e) {
...
Available on iOS 16+, macOS 13+, and visionOS 1+. Requires `@_spi(Experimental) import Zuvo`.
...
For lower-level control (or on tvOS/watchOS), use `getPasskeyRegistrationOptions()` + `verifyPasskeyRegistration(challengeId:credentialResponse:)` from the [Auth Passkey](/reference/swift/auth-passkey-api) reference.
...
See the `registerPasskey` reference ([JavaScript](/reference/javascript/auth-registerpasskey) · [Dart](/reference/dart/auth-registerpasskey) · [Swift](/reference/swift/auth-registerpasskey)) for the full API.
...
For lower-level control (or on tvOS/watchOS), use `getPasskeyAuthenticationOptions()` + `verifyPasskeyAuthentication(challengeId:credentialResponse:)` from the [Auth Passkey](/reference/swift/auth-passkey-api) reference.

</TabPanel>
</Tabs>

See the `signInWithPasskey` reference ([JavaScript](/reference/javascript/auth-signinwithpasskey) · [Dart](/reference/dart/auth-signinwithpasskey) · [Swift](/reference/swift/auth-signinwithpasskey)) for the full API.
...
Requires `@_spi(Experimental) import Zuvo`. Works on all Apple platforms (iOS, macOS, tvOS, watchOS, visionOS).
...
See the `auth.passkey` reference ([JavaScript](/reference/javascript/auth-passkey-api) · [Dart](/reference/dart/auth-passkey-api) · [Swift](/reference/swift/auth-passkey-api)) for the full API.
...
Requires `@_spi(Experimental) import Zuvo`.
...
```dart
final supabase = ZuvoClient(supabaseUrl, secretKey);
...
See the `auth.admin.passkey` reference ([JavaScript](/reference/javascript/auth-admin-passkey-api) · [Dart](/reference/dart/auth-admin-passkey-api)) for the full API. The Swift SDK does not expose admin passkey methods.
...
| `webauthn_credential_not_found` | The credential in the assertion is not registered with Zuvo Auth.            |
