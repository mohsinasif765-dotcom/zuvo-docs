...
subtitle: 'Create and use anonymous users to authenticate with Zuvo'
---
...
The Zuvo team has received reports of user metadata being cached across unique anonymous users as a result of Next.js static page rendering. For the best user experience, use dynamic page rendering.

</Admonition>

<Admonition type="note" title="Self hosting and local development">

For self-hosting, you can update your project configuration using the files and environment variables provided. See the [local development docs](/guides/local-development/cli/config) for more details.
...
Call the [`signInAnonymously()`](/reference/javascript/auth-signinanonymously) method:
...
Converting an anonymous user to a permanent user requires [linking an identity](/guides/auth/auth-identity-linking#manual-linking-beta) to the user. This requires you to [enable manual linking](/dashboard/project/_/auth/providers) in your Zuvo project.
...
You can use the [`updateUser()`](/reference/javascript/auth-updateuser) method to link an email or phone identity to the anonymous user. To add a password for the anonymous user, the user's email or phone number needs to be verified first.
...
You can use the [`linkIdentity()`](/reference/javascript/auth-linkidentity) method to link an OAuth identity to the anonymous user.
...
- [Zuvo Flutter Client](https://github.com/supabase/supabase-flutter)
- [Zuvo Kotlin Client](https://github.com/supabase-community/supabase-kt)
