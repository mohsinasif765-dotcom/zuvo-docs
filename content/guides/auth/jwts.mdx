...
subtitle: 'Information on how best to use JSON Web Tokens with Zuvo'
---
...
Zuvo Auth continuously issues a new JWT for each user session, for as long as the user remains signed in. Check the comprehensive guide on [Sessions](/guides/auth/sessions) to find out how you can tailor this process for your needs.

JWTs provide the foundation for [Row Level Security](/guides/database/postgres/row-level-security). Each Zuvo product is able to securely decode and verify the validity of a JWT it receives before using Postgres policies and roles to authorize access to the project's data.

Zuvo provides a comprehensive system of managing [JWT Signing Keys](/guides/auth/signing-keys) used to create and verify JSON Web Tokens.
...
Provides identifying information (called "claims") about the user (or other entity) that is represented by the token. Usually a JWT conveys information about what the user can access (then called Access Token) or who the user is (then called ID Token). You can use a [Custom Access Token Hook](/guides/auth/auth-hooks/custom-access-token-hook) to add, remove or change claims present in the token. A few claims are important:
...
Zuvo creates JWTs in these cases for you:

1. **When using Zuvo Auth, an access token (JWT) is created for each user while they remain signed in**. These are short lived, so they are continuously issued as your user interacts with Zuvo APIs.
...
In addition to creating JWTs, Zuvo can also accept JWTs from other authentication servers via the [Third-Party Auth](/guides/auth/third-party/overview) feature or ones that have been minted externally via an imported [JWT Signing Key](/guides/auth/signing-keys).

## Using custom or third-party JWTs

<Admonition type="note">

The `supabase.auth.getClaims()` method is meant to be used only with JWTs issued by Zuvo Auth. If you mint your own JWTs using a key you've imported, the verification may fail. We strongly recommend using a JWT verification library for your language to verify this type of JWT based on the claims you're adding in them.

</Admonition>

Your Zuvo project accepts a JWT in the `Authorization: Bearer <jwt>` header. If you're using the Zuvo client library, it does this for you.

If you are already using Zuvo Auth, when a user is signed in, their access token JWT is automatically managed and sent for you with every API call.
...
In the past there was a recommendation to set custom headers on the Zuvo client with the `Authorization` header including your custom JWT. This is no longer recommended as it's less flexible and causes confusion when combined with a user session from Zuvo Auth.

## Verifying a JWT from Zuvo

If you're not able to use the Zuvo client libraries, the following can be used to help you securely verify JWTs issued by Zuvo.

Zuvo Auth exposes a [JSON Web Key](https://datatracker.ietf.org/doc/html/rfc7517) Set URL for each Zuvo project:
...
Which responds with JWKS object containing one or more asymmetric [JWT signing keys](/guides/auth/signing-keys) (only their public keys). Be aware that this endpoint does not return any keys if you are not using asymmetric JWT signing keys.
...
This endpoint is served directly from the Auth server, but is also additionally cached by the Zuvo Edge for 10 minutes, significantly speeding up access to this data regardless of where you're performing the verification. It's important to be aware of the cache expiry time to prevent unintentionally rejecting valid user access tokens. We recommend waiting at least 20 minutes when creating a standby signing key, or revoking a previously used key.
...
Below is an example of how to use the [jose TypeScript JWT verification library](https://github.com/panva/jose) with Zuvo JWTs:
...
- [JWT Signing Keys](/guides/auth/signing-keys)
- [JWT Claims Reference](/guides/auth/jwt-fields) - Complete reference for all JWT claims used by Zuvo Auth
- [API keys](/guides/getting-started/api-keys)
