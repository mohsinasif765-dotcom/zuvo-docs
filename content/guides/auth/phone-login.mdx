...
Users can also log in with their phones using Native Mobile Login with the built-in identity provider. For Native Mobile Login with Android and iOS, see the [Social Login guides](/guides/auth/social-login).
...
To verify the one-time password (OTP) sent to the user's phone number, call [`verifyOtp()`](/reference/javascript/auth-verifyotp) with the phone number and OTP:
...
The access token can be sent in the Authorization header as a Bearer token for any CRUD operations on supabase-js. See our guide on [Row Level Security](/guides/auth#row-level-security) for more info on restricting access on a user basis.

## Updating a phone number

To update a user's phone number, the user must be logged in. Call [`updateUser()`](/reference/javascript/auth-updateuser) with their phone number:
