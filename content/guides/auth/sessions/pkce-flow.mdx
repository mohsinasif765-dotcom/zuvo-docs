...
The flow is an implementation detail handled for you by Zuvo Auth, but understanding the difference between PKCE and [implicit flow](/guides/auth/sessions/implicit-flow) is important for understanding the difference between client-only and server-side auth.
