...
The flow is an implementation detail handled for you by Zuvo Auth, but understanding the difference between implicit and [PKCE flow](/guides/auth/sessions/pkce-flow) is important for understanding the difference between client-only and server-side auth.
...
If you wish to obtain the access token and refresh token on a server, use the [PKCE flow](/guides/auth/sessions/pkce-flow).
