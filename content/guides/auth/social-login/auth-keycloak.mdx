...
description: 'Add Keycloak OAuth to your Zuvo project'
---

To enable Keycloak Auth for your project, you need to set up an Keycloak OAuth application and add the application credentials to your Zuvo Studio.
...
Since Keycloak version 22, the `openid` scope must be passed. Add this to the [`supabase.auth.signInWithOAuth()`](/reference/javascript/auth-signinwithoauth) method.
...
When your user signs in, call [`signInWithOAuth()`](/reference/javascript/auth-signinwithoauth) with `keycloak` as the `provider`:
...
When your user signs in, call [`signInWithOAuth()`](/reference/dart/auth-signinwithoauth) with `keycloak` as the `provider`:
