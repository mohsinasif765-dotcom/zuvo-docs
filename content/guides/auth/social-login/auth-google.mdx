...
- Use the [Google Auth Platform console](https://console.cloud.google.com/auth/overview) to register and set up your application's:
  ...
  - [**Branding**](https://console.cloud.google.com/auth/branding) and [**Verification**](https://console.cloud.google.com/auth/verification) show a logo and name instead of the Zuvo project ID in the consent screen, improving user retention. Brand verification may take a few business days.

### Setup required scopes

Zuvo Auth needs a few scopes granting access to profile data of your end users, which you have to configure in the [**Data Access (Scopes)**](https://console.cloud.google.com/auth/scopes) screen:
...
1. Set up a [custom domain for your project](/guides/platform/custom-domains) to present the user with a clear relationship to the website they clicked Sign in with Google on.
...
To support Sign In with Google, you need to configure the Google provider for your Zuvo project.
...
1. Under **Authorized JavaScript origins** add your application's URL. These should also be configured as the [Site URL or redirect configuration in your project](/guides/auth/redirect-urls).
   - If your app is hosted on `https://example.com/app` add `https://example.com`.
   - Add `http://localhost:<port>` while developing locally. Remember to remove this when your application [goes into production](/guides/deployment/going-into-prod).
1. Under **Authorized redirect URIs** add your Zuvo project's callback URL.
...
1. [Create a new OAuth client ID](https://console.cloud.google.com/auth/clients/create) and choose **Android** or **iOS** depending on the OS you're building the app for.
   ...
   - You will have a different set of SHA-1 certificate fingerprints for testing locally and going to production. Make sure to add both to the Google Cloud Console, and add all of the Client IDs to the Zuvo dashboard.
...
Follow the same configuration guide as if your app was a Web application when building a desktop Flutter application.
  ...
  <TabPanel id="android" label="Kotlin (Android and iOS)">
...
Use the [PATCH `/v1/projects/{ref}/config/auth` Management API endpoint](/reference/api/v1-update-auth-service-config) to configure the project's Auth settings programmatically. For configuring the Google provider send these options:
...
The tokens saved by your application are the Zuvo Auth tokens. Your app might additionally need the Google OAuth 2.0 tokens to access Google services on the user's behalf.
...
1. Create a `handleSignInWithGoogle` function that takes the `CredentialResponse` and passes the included token to Zuvo. The function needs to be available in the global scope for Google's code to find it.
...
```ts
   ...
   Note that the nonce should be the same in both places, but because Zuvo Auth expects the provider to hash it (SHA-256, hexadecimal representation), you need to provide a hashed version to Google and a non-hashed version to `signInWithIdToken`.
...
When the user provides consent, Google issues an identity token (commonly abbreviated as ID token) that you then send to your project's Zuvo Auth server. When valid, a new user session is started by issuing an access and refresh token from Zuvo Auth.

By default, Zuvo Auth implements nonce validation during the authentication flow. This can be disabled in production under `Authentication > Providers > Google > Skip Nonce Check` in the Dashboard, or when developing locally by setting `auth.external.<provider>.skip_nonce_check`. Only disable this if your client libraries cannot properly handle nonce verification.

When working with Expo, you can use the [`@react-native-google-signin/google-signin` library](https://github.com/react-native-google-signin/google-signin#expo-installation) to obtain an ID token that you can pass to supabase-js [`signInWithIdToken` method](/reference/javascript/auth-signinwithidtoken).

Follow the [Expo installation docs](https://react-native-google-signin.github.io/docs/setting-up/expo) for installation and configuration instructions. See the [supabase-js reference](/reference/javascript/initializing?example=react-native-options-async-storage) for instructions on initializing the supabase-js client in React Native.
...
Google sign-in with Zuvo is done through the [google_sign_in](https://pub.dev/packages/google_sign_in) package for iOS and Android.

When the user provides consent, Google issues an identity token (commonly abbreviated as ID token) that is then sent to your project's Zuvo Auth server. When valid, a new user session is started by issuing an access and refresh token from Zuvo Auth.

Follow the code sample below to implement native Google sign-in with Zuvo in your Flutter iOS and Android app.
...
Future<void> _nativeGoogleSignIn() async {
  ...
  /// Authorization is required to obtain the access token with the appropriate scopes for Zuvo authentication,
...
Google sign-in with Zuvo on Web, macOS, Windows, and Linux is done through the [`signInWithOAuth`](/reference/dart/auth-signinwithoauth) method.

This method of signing in is web based, and will open a browser window to perform the sign in. For non-web platforms, the user is brought back to the app via [deep linking](/guides/auth/native-mobile-deep-linking?platform=flutter).
...
This call takes the user to Google's consent screen. Once the flow ends, the user's profile information is exchanged and validated with Zuvo Auth before it redirects back to your Flutter application with an access and refresh token representing the user's session.
...
<TabPanel id="android" label="Kotlin (Android)">
...
Follow the code sample below to implement native Google sign-in with Zuvo using Credential Manager in your Android app.
    ...
    val onClick: () -> Unit = {
        ...
        coroutineScope.launch {
            try {
                ...
                // Handle RestException thrown by Zuvo
            } catch (e: Exception) {
...
<TabPanel id="kotlin-mp" label="Kotlin (Multiplatform)">
...
```kotlin
val supabaseClient = createZuvoClient(
...
Then you should call the [`chrome.identity.launchWebAuthFlow()`](https://developer.chrome.com/docs/extensions/reference/identity/#method-launchWebAuthFlow) function to trigger the sign in flow. On success, call the `supabase.auth.signInWithIdToken()` function to complete sign in with your Zuvo project.
