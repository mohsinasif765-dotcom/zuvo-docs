...
description: 'Add Spotify OAuth to your Zuvo project'
---
...
- Add your Spotify `API Key` and `API Secret Key` to your [Zuvo Project](/dashboard).
...
1. After completing the sign-in process, the user will be redirected to your app with an error that says the email address needs to be confirmed. Simultaneously the user receives a confirmation email from Zuvo Auth.
...
When your user signs in, call [`signInWithOAuth()`](/reference/javascript/auth-signinwithoauth) with `spotify` as the `provider`:
...
When your user signs in, call [`signInWithOAuth()`](/reference/dart/auth-signinwithoauth) with `spotify` as the `provider`:
