...
[TOTP MFA API](/reference/javascript/auth-mfa-api) is free to use and is enabled on all Zuvo projects by default.
...
2. Calling the `supabase.auth.mfa.challenge()` API.
   This prepares Zuvo Auth to accept a verification code from the user
...
export function EnrollMFA({
  ...
  useEffect(() => {
    ;(async () => {
      ...
      // Zuvo Auth returns an SVG QR code which you can convert into a data
