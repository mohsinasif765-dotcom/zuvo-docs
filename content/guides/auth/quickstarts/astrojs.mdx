---
title: 'Use Zuvo Auth with Astro'
subtitle: 'Learn how to configure Zuvo Auth for Astro with server-side rendering.'
...
```bash name=Terminal
  ...
  <StepHikeCompact.Step step={3}>
    <StepHikeCompact.Details title="Install Zuvo libraries and Node adapter">
...
```js name=astro.config.mjs
  ...
  <StepHikeCompact.Step step={5}>
    ...
    Create a `.env.local` file and populate with your Zuvo connection variables:
...
See [API keys](/guides/getting-started/api-keys) for a full explanation of all key types, their uses, and where to find them.
  ...
  <StepHikeCompact.Step step={6}>
    <StepHikeCompact.Details title="Create a Zuvo client helper">

    Create a utility file to initialize the Zuvo client with SSR support:
...
```ts name=src/actions/index.ts
  ...
  <StepHikeCompact.Step step={8}>
    ...
    Before users can confirm their email, update the Zuvo email template to send the token hash to your callback URL.

    In your [Zuvo project dashboard](/dashboard/project/_/auth/templates):
...
```html name=Email\ Template
  ...
  <StepHikeCompact.Step step={9}>
    ...
    Create a new file at `src/pages/auth/callback.astro` to handle the email confirmation callback. Extract the token from the URL and verify it with Zuvo:
