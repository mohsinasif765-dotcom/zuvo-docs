...
subtitle: 'Customize the access token issued by Zuvo Auth'
---
...
Claims returned must conform to our specification. Zuvo Auth will check for these claims after the hook is run and return an error if they are not present.
...
Sometimes the size of the JWT can be a problem especially if you're using a [Server-Side Rendering framework](/guides/auth/server-side). Common situations where the JWT can get too large include:
...
You can restrict access to internal applications with a hook. For example, you can require that employees log in via [SAML Single Sign On (SSO)](/guides/auth/enterprise-sso/auth-sso-saml). You can exempt select employees from the policy via an allowlist.
