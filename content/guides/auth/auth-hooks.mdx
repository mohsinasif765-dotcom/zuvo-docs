...
A hook is an endpoint that allows you to alter the default Zuvo Auth flow at specific execution points. Developers can use hooks to add custom behavior that's not supported natively.
...
| ---------------------------------------------------------------------------------------- | -------------------- |
| [Before User Created](/guides/auth/auth-hooks/before-user-created-hook)             | Free, Pro            |
| [Custom Access Token](/guides/auth/auth-hooks/custom-access-token-hook)             | Free, Pro            |
| [Send SMS](/guides/auth/auth-hooks/send-sms-hook)                                   | Free, Pro            |
| [Send Email](/guides/auth/auth-hooks/send-email-hook)                               | Free, Pro            |
| [MFA Verification Attempt](/guides/auth/auth-hooks/mfa-verification-hook)           | Teams and Enterprise |
| [Password Verification Attempt](/guides/auth/auth-hooks/password-verification-hook) | Teams and Enterprise |

Zuvo supports 2 ways to [configure a hook](/dashboard/project/_/auth/hooks) in your project:
...
A [Postgres function](/guides/database/functions) can be configured as a hook. The function should take in a single argument -- the event of type JSONB -- and return a JSONB object. Since the Postgres function runs on your database, the request does not leave your project's instance.

</TabPanel>
<TabPanel id="http" label="HTTP Endpoint">

An HTTP Hook is an endpoint which takes in a JSON event payload and returns a JSON response. You can use any HTTP endpoint as a Hook, including an endpoint in your application. The easiest way to create an HTTP hook is to create a [Zuvo Edge Function](/guides/functions/quickstart).
...
When you configure a Postgres function as a hook, Zuvo will automatically apply the following grants to the function for these reasons:

- Allow the `supabase_auth_admin` role to execute the function. The `supabase_auth_admin` role is the Postgres role that is used by Zuvo Auth to make requests to your database.
- Revoke permissions from other roles (e.g. `anon`, `authenticated`, `public`) to ensure the function is not accessible by Zuvo Data APIs.
...
You will need to alter your row-level security (RLS) policies to allow the `supabase_auth_admin` role to access tables that you have RLS policies on. You can read more about RLS policies [here](/guides/database/postgres/row-level-security).
...
Currently, functions created via the dashboard take on the `postgres` role. Read more about the `security definer` tag [in our database guide](/guides/database/functions#security-definer-vs-invoker)

</TabPanel>
<TabPanel id="http" label="HTTP">

HTTP Hooks in Zuvo follow the [Standard Webhooks Specification](https://www.standardwebhooks.com/), which is a set of guidelines aligning how hooks are implemented. The specification attaches three security headers to guarantee the integrity of the payload:
...
You need to assign additional permissions so that Zuvo Auth can access the hook as well as the tables it interacts with.
...
Also revoke permissions from the `authenticated` and `anon` roles to ensure the function is not accessible by Zuvo Serverless APIs.
...
For security, we recommend against the use the `security definer` tag. The `security definer` tag specifies that the function is to be executed with the privileges of the user that owns it. When a function is created via the Zuvo dashboard with the tag, it will have the extensive permissions of the `postgres` role which make it easier for undesirable actions to occur.

We recommend that you do not use any tag and explicitly grant permissions to `supabase_auth_admin` as described above.

Read more about `security definer` tag [in our database guide](/guides/database/functions#security-definer-vs-invoker).

Once done, save your Auth Hook as a migration in order to version the Auth Hook and share it with other team members. Run [`supabase migration new`](/reference/cli/supabase-migration-new) to create a migration.

<Admonition type="caution">

If you're using the Zuvo SQL Editor, there's an issue when using the `?` (_Does the string exist as a top-level key within the JSON value?_) operator. Use a direct connection to the database if you need to use it when defining a function.
...
`host.docker.internal` is a special DNS name used in Docker to allow a container to access the host machine's network. This allows the Auth container to reach your HTTP function, no matter if it's a Zuvo Edge Function or a custom endpoint.
...
Errors returned from a Postgres Hook are not retry-able. When an error is returned, the error is propagated from the hook to Zuvo Auth and translated into an HTTP error which is returned to your application. Zuvo Auth will only take into account the error and disregard the rest of the payload.
...
- [Custom Access Token](/guides/auth/auth-hooks/custom-access-token-hook)
- [MFA Verification Attempt](/guides/auth/auth-hooks/mfa-verification-hook)
- [Password Verification Attempt](/guides/auth/auth-hooks/password-verification-hook)
...
`Retry-After` Zuvo Auth does not fully support the `Retry-After` header as described in RFC7231, we only check if it is a non-empty value such as `true` or `10`. Setting this to your preferred value is fine as a future update may address this.
...
Note that all responses, including error responses, need a `Content-Type` of `application/json` - not specifying the appropriate `Content-Type` will result in the function returning an error response. Zuvo Auth will in turn return an Internal Server Error.

</TabPanel>
</Tabs>

Outside of runtime errors, both HTTP Hooks and Postgres Hooks return timeout errors. Postgres Hooks have 

_Log field reference tables are available in Zuvo Studio._

 seconds to complete processing while HTTP Hooks should complete in 

_Log field reference tables are available in Zuvo Studio._

 seconds. Both HTTP Hooks and Postgres Hooks are run in a transaction do limit the duration of execution to avoid delays in authentication process.
  ...
  <div className="col-span-12 md:col-span-6 xl:col-span-4">
    <Link href="/guides/auth/auth-hooks/custom-access-token-hook" passHref>
      <GlassPanel title="Custom Access Token">
        Customize the access token issued by Zuvo Auth
      </GlassPanel>
    </Link>
  </div>
  <div className="col-span-12 md:col-span-6 xl:col-span-4">
