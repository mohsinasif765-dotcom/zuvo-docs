...
When a user authenticates with Zuvo Auth, two pieces of information are issued by the server:
...
If you're not using SSR, you might also be using the [implicit flow](/guides/auth/sessions/implicit-flow) to get the access and refresh tokens. The server can't access the tokens in this flow, so for SSR, you should change to the [PKCE flow](/guides/auth/sessions/pkce-flow). You can change the flow type when initiating your Zuvo client if your client library provides this option.

<Admonition type="note">

In the `@supabase/ssr` package, Zuvo clients are initiated to use the PKCE flow by default. They are also automatically configured to handle the saving and retrieval of session information in cookies.
...
To maintain the session, these tokens must be stored in a storage medium securely shared between client and server, which is traditionally cookies. Whenever the session is refreshed, the auth and refresh tokens in the shared storage medium must be updated. Zuvo client libraries provide a customizable `storage` option when a client is initiated, allowing you to change where tokens are stored.
...
To improve experience for your users, we recommend redirecting users to one specific page after sign-in that does not include any route prefetching from Next.js. Once the Zuvo client library running in the browser has obtained the access and refresh tokens from the URL fragment, you can send users to any pages that use prefetching.
...
If you use ISR on pages that trigger a Zuvo session refresh, the cached response will include the `Set-Cookie` header containing the refreshed JWT. When that cached response is served to a subsequent user, their browser stores the token and they are signed in as the wrong person.
...
Vercel's Fluid compute model can keep server instances warm and reuse them across requests. In some cases this means a Zuvo client initialized in module scope — or stored in a shared variable — may be reused across requests from different users, causing one user's session to leak into another user's request.

Always initialize the Zuvo client inside the request handler, not at module level. Do not store the client or any user-specific state in a variable that persists between requests.

### Which authentication flows have PKCE support?

At present, PKCE is supported on the Magic Link, OAuth, Sign Up, and Password Recovery routes. These correspond to the `signInWithOtp`, `signInWithOAuth`, `signUp`, and `resetPasswordForEmail` methods on the Zuvo client library. When using PKCE with Phone and Email OTPs, there is no behavior change with respect to the implicit flow - an access token will be returned in the body when a request is successful.
