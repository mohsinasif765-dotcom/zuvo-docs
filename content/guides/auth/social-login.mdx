...
## Set up a social provider with Zuvo Auth

Zuvo supports a suite of social providers. Follow these guides to configure a social provider for your platform.



<Admonition type="note">

Need to integrate with a provider not listed here? You can add any OAuth2 or OIDC-compatible provider using [Custom OAuth/OIDC Providers](/guides/auth/custom-oauth-providers).
...
Zuvo Auth does not manage refreshing the provider token for the user. Your application will need to use the provider refresh token to obtain a new provider token. If no provider refresh token is returned, then it could mean one of the following:
