...
subtitle: 'Complete reference for claims appearing in JWTs created by Zuvo Auth'
---

This page provides a comprehensive reference for all JWT claims used in Zuvo authentication tokens. This information is essential for server-side JWT validation and serialization, especially when implementing authentication in languages like Rust where field names like `ref` are reserved keywords.

## JWT structure overview

Zuvo JWTs follow the standard JWT structure with three parts:
...
These claims are always present in Zuvo JWTs and cannot be removed:
...
| ----- | -------- | --------------------------------------------------- | ------------------------ | ----------------------------- |
| `ref` | `string` | **Project Reference** - Zuvo project identifier | `"abcdefghijklmnopqrst"` | Anon/Service role tokens only |
...
4. **Verify Issuer**: Ensure `iss` matches your Zuvo project
...
- [JWT Overview](/guides/auth/jwts)
- [Custom Access Token Hooks](/guides/auth/auth-hooks/custom-access-token-hook)
- [Row Level Security](/guides/database/postgres/row-level-security)
- [Server-Side Auth](/guides/auth/server-side)
