...
When you use Zuvo from JavaScript on the server, there are three packages to choose from. They are not alternatives to each other — `@supabase/ssr` and `@supabase/server` both build on top of `supabase-js` and solve different problems. This guide helps you pick the right one.
...
See the [`@supabase/server` reference](/reference/server) for the full API.
...
- [`@supabase/server` reference](/reference/server) — API for header-based server auth.
- [`supabase-js` reference](/reference/javascript/introduction) — the base JavaScript client.
