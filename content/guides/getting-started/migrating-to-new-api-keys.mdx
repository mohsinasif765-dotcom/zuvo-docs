...
For a full explanation of each key type, see [API keys](/guides/getting-started/api-keys).
...
The publishable key carries the same low privileges as the `anon` key, so your [Row Level Security](/guides/database/postgres/row-level-security) policies behave the same. User authentication through Zuvo Auth is unchanged. The user still signs in and gets their own JWT.
...
Don't hardcode a secret key in SQL or a webhook configuration, where it's stored in plain text. Store it in [Vault](/guides/database/vault) and read it at call time:
...
The publishable keys work the same way through `SUPABASE_PUBLISHABLE_KEYS`. Read [the Managing Secrets guide](/guides/functions/secrets) for more on environment variables in Edge Functions.
...
Wrap your existing `Deno.serve` handler with `withZuvo` and declare an `auth` mode for who can call it. Keep `verify_jwt = false` so the SDK does the authorization.
...
Deno.serve(
  withZuvo({ auth: 'user' }, async (_req, ctx) => {
...
Deno.serve(
  withZuvo({ auth: 'secret' }, async (_req, ctx) => {
...
`withZuvo` returns a standard request handler, so you can also export it as a `fetch` handler instead of passing it to `Deno.serve`:
...
export default {
  fetch: withZuvo({ auth: 'user' }, async (_req, ctx) => {
    // ctx.supabase is scoped to the authenticated user
    return Response.json({ email: ctx.userClaims?.email })
  }),
}
...
A good way to try this is to duplicate one of your functions and migrate the copy first. See [Securing Edge Functions](/guides/functions/auth) for every auth mode and use case, and [Authorization headers](/guides/functions/auth-headers) for how the headers work.

## Step 5: Verify nothing uses the legacy keys

Before turning the legacy keys off, confirm nothing still depends on them. There's no automatic usage indicator, so this is a manual check. Go through every place that holds a Zuvo key and make sure it now uses a publishable or secret key.
...
- Public Realtime connections are limited to 24 hours unless the connection is upgraded with user-level authentication through Zuvo Auth or a supported third-party auth provider.

## Next steps

After migrating your keys, consider moving to the [JWT signing keys](/guides/auth/signing-keys) system as well. This is a separate, independent migration. The new publishable and secret keys aren't JWTs, so they no longer touch your project's JWT secret. But the access tokens Zuvo Auth issues to your users are still signed by that shared secret. Signing keys replace it with rotatable keys you can change without downtime. Together, the two migrations get your whole project off the shared JWT secret.
