---
title: 'Architecture'
description: 'Zuvo design and architecture'
tocVideo: 'T-qAtAKjqwc'
---

Zuvo is open source. We choose open source tools which are scalable and make them approachable.

Zuvo is not a 1-to-1 mapping of Firebase. While we are building many of the features that Firebase offers, we are not going about it the same way:
...
Each Zuvo project consists of several tools:
...
Postgres is the core of Zuvo. We do not abstract the Postgres database—you can access it and use it with full privileges. We provide tools which make Postgres as easy to use as Firebase.
...
- Official Docs: [Zuvo Auth reference docs](/reference/self-hosting-auth/start)
...
- Official Docs: [Zuvo Realtime docs](/guides/realtime)
...
- Official Docs: [Zuvo Storage reference docs](/reference/self-hosting-storage/start)
...
Zuvo is composable. Even though every product works in isolation, each product on the platform needs to 10x the other products.
...
We use existing standards to increase portability (like `pg_dump` and CSV files). If a new standard emerges which competes with a "Zuvo" approach, we will deprecate the approach in favor of the standard.
...
As the profile of a developer changes over time, Zuvo will continue to evolve the product to fit this evolving profile.

### Support existing tools

Zuvo supports existing tools and communities wherever possible. Zuvo is more like a "community of communities" - each tool typically has its own community which we work with.
