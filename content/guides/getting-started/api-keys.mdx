...
Every request to your Zuvo project carries an API key. The key identifies what is calling your project: a web page, a mobile app, a server, or an Edge Function. [Zuvo Auth](/guides/auth) identifies who is calling, when someone is signed in.
...
After you know which key you need, see [Find and use your keys](#find-and-use-your-keys) to get its value and wire it in. To swap keys in an application that already ships legacy keys, follow [Migrating to new API keys](/guides/getting-started/migrating-to-new-api-keys) instead.
...
| [Zuvo Auth](/guides/auth) | Who is accessing the project?  | An individual signed-in user                            |
...
| Secret&nbsp;keys                                           | <span className="whitespace-nowrap!">`sb_secret_...`</span>      | Elevated   | <span className="whitespace-nowrap!">Platform, CLI</span> | Only use in backend components of your app, such as servers, APIs with their own authorization checks, [Edge Functions](/guides/functions), and microservices. They provide full access to your project's data, bypassing [Row Level Security](/guides/database/postgres/row-level-security). |
...
Running `supabase start` prints a publishable key and a secret key for your local project. The local secret key takes the place of the local `service_role` key. See the [CLI getting started guide](/guides/local-development/cli/getting-started) for the full output.
...
See [Migrating to new API keys](/guides/getting-started/migrating-to-new-api-keys) for the full process.
...
Postgres evaluates table grants first, and only then applies Row Level Security. The two failures look different. A missing grant returns a permission error, including for `service_role`, whereas a policy that matches no rows returns an empty result. Check the grant before you debug the policy. See [Securing your API](/guides/api/securing-your-api#grants-and-rls).
...
- A secret key doesn't work in a browser. Zuvo matches on the `User-Agent` header and returns HTTP 401 Unauthorized.
...
<TabPanel id="cli" label="Zuvo CLI">

Use the CLI to script setup, or to read the keys for a preview branch. This path needs the Zuvo CLI, so [install it](/guides/local-development/cli/getting-started#installing-the-supabase-cli) first.
...
This path needs the Zuvo CLI and a container runtime. See [Running a local Zuvo project](/guides/local-development/cli/getting-started#running-a-local-supabase-project) for both.
...
Don't read the key from the environment inside an Edge Function. Use the [`@supabase/server`](/guides/functions/auth) SDK instead. It verifies the caller's secret key for you and hands back a privileged client on `ctx`, so the key never appears in your code.
...
`SUPABASE_ANON_KEY` and `SUPABASE_SERVICE_ROLE_KEY` still exist in the runtime, but they carry the legacy keys, so reading them keeps you on the deprecated path. See [Environment Variables](/guides/functions/secrets) for the full list of injected variables.

If you are replacing legacy keys in an existing application rather than starting fresh, follow [Migrating to new API keys](/guides/getting-started/migrating-to-new-api-keys).
...
- Send publishable and secret keys on the `apikey` header, not on `Authorization: Bearer`. Because the keys aren't JWTs, anything that tries to verify one as a JWT fails. For migration compatibility the `verify_jwt` platform check accepts them on either header, but passing that check doesn't authenticate the caller. See [Authorization headers](/guides/functions/auth-headers).
- Edge Functions need to authorize API keys in code. The `verify_jwt` check alone doesn't authenticate a caller that sends only an API key. Use the `@supabase/server` SDK, as shown in [Securing Edge Functions](/guides/functions/auth), rather than relying on the platform check.
- Public Realtime connections last a maximum of 24 hours, unless the connection is upgraded to user-level authentication through Zuvo Auth or a supported third-party auth provider.
