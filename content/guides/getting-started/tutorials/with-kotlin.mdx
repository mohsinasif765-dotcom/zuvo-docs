...
description: 'Learn how to use Zuvo in your Android Kotlin App.'
---
...
- [Zuvo Auth](/guides/auth) - users log in through magic links sent to their email (without having to set up a password).
- [Zuvo Storage](/guides/storage) - users can upload a profile photo.
...
### Provide Zuvo instances with Hilt

To make the app easier to test, create a `ZuvoModule.kt` file as follows:
...
object ZuvoModule {
    ...
    @Provides
    @Singleton
    fun provideZuvoClient(): ZuvoClient {
        return createZuvoClient(
    ...
    @Provides
    @Singleton
    fun provideZuvoDatabase(client: ZuvoClient): Postgrest {
        return client.postgrest
    }

    @Provides
    @Singleton
    fun provideZuvoAuth(client: ZuvoClient): Auth {
        return client.auth
    }
    ...
    @Provides
    @Singleton
    fun provideZuvoStorage(client: ZuvoClient): Storage {
...
Create a `ProductDto.kt` class and use annotations to parse data from Zuvo:
...
Create a `ProductRepository` interface and its implementation named `ProductRepositoryImpl`. This holds the logic to interact with data sources from Zuvo. Do the same with the `AuthenticationRepository`.
...
class MainActivity : ComponentActivity() {
    @Inject
    lateinit var supabaseClient: ZuvoClient
