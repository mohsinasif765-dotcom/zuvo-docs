...
title: 'Zuvo Partner Integration Guide'
subtitle: 'Integrate Zuvo with your platform or product.'
description: 'This guide explains how a Zuvo partner can integrate with the Zuvo platform via OAuth.'
---

This guide assumes you have already followed [the Build a Zuvo Integration guide](/guides/integrations/build-a-supabase-oauth-integration) and have a working OAuth client.

When a Zuvo user clicks the **Install Integration** button in the Dashboard, they are redirected to your system to begin the OAuth flow. You can implement this redirect in one of two ways:

- **[Redirect](#method-1-redirect)**: Easier to build, but your system cannot verify that the incoming user was sent by Zuvo.
- **[Signed redirect](#method-2-signed-redirect)**: More work to build, but cryptographically verifies that the redirect originated from Zuvo. **Recommended for production integrations.**

Pick the method that fits your security requirements, then follow the matching section below.

## Method 1: Redirect

In this method, you implement a single `GET` endpoint. Zuvo redirects the user to this endpoint when they click **Install Integration**, and your endpoint kicks off the OAuth flow.
...
sequenceDiagram
  ...
  participant Zuvo as Zuvo (OAuth Authorization Server/Protected Resource)

  Note over Browser: Zuvo Studio Open

  User->>Browser: Clicks "Install Integration"

  Browser->>Partner: "Connect to Partner" Click Handler Page
  Partner->>Browser: Redirect to Zuvo Authorization Page

  Browser->>Zuvo: Request Authorization
  Zuvo->>Browser: Redirect to Consent Screen

  Browser->>User: Display Consent Screen
  User->>Browser: Gives Consent

  Browser->>Zuvo: Submit User Consent
  Zuvo->>Browser: Redirect to Partner With Authorization Code

  Browser->>Partner: Submit Authorization Code

  Partner->>Zuvo: Exchange Authorization Code for Token
  Zuvo->>Partner: Return Token

  Partner->>Zuvo: Request Management API Resources
  Zuvo->>Partner: Return Resources
...
The user clicks **Install Integration** in the Zuvo Studio, which routes them through the partner's click-handler page and on to the Zuvo authorization page. Zuvo shows a consent screen. Once the user consents, Zuvo redirects back to the partner with an authorization code. The partner exchanges that code for a token, uses the token to request Management API resources, and finally shows the user an "Integration Installed" message.
...
Zuvo will append the following query parameters to the URL when redirecting:

| Param               | Description                                                                                    |
| ------------------- | ---------------------------------------------------------------------------------------------- |
| `project_id`        | Zuvo project ref of the project where the user clicked the **Install Integration** button. |
| `organization_slug` | Zuvo organization slug where the user clicked the **Install Integration** button.          |
...
Your endpoint may ask the user to sign up, sign in, or perform other setup tasks on your website. Once those are complete, immediately [redirect to the Zuvo authorization URL](/guides/integrations/build-a-supabase-oauth-integration#redirecting-to-the-authorize-url) without further user interaction. This starts the OAuth flow.

### Step 2: Share your endpoint with Zuvo

Send Zuvo the URL of your endpoint so we can configure the **Install Integration** button to redirect there.

## Method 2: Signed redirect

In this method, Zuvo signs a JWT before redirecting the user. You verify the signature, generate a one-time redirect record, and return its URL to Zuvo. Zuvo then redirects the user to that URL. This guarantees that any redirect your system handles originated from Zuvo.
...
sequenceDiagram
  ...
  Browser->>Zuvo: Request Redirect URL Generation
  Zuvo->>Partner: Send Signed JWT
  Partner->>Partner: Validate JWT
  Partner->>Partner: Generate Unique Redirect URL
  Partner->>Zuvo: Return Redirect URL
  Zuvo->>Browser: Redirect to the Redirect URL
...
Walking through the sequence: when the user clicks **Install Integration**, Zuvo sends the partner a signed JWT and asks it to generate a redirect URL. The partner validates the JWT, generates a unique redirect record and URL, and returns it. Zuvo redirects the user to that URL; the partner validates the redirect record, optionally has the user complete extra steps, and then sends them to the Zuvo authorization page. From there the flow matches Method 1: the user consents, Zuvo returns an authorization code, the partner exchanges it for a token, requests Management API resources, and the integration completes.

### Step 1: Exchange public keys with Zuvo

Zuvo generates two key-pairs, one for staging, one for production, and shares the public keys with you. Save both public keys and their key IDs in your system. The keys are PEM-encoded EC P-256.
...
Storing keys by ID enables zero-downtime key rotation. When Zuvo rotates keys, we share new key-pairs and start signing JWTs with the new key ID. Your system picks the correct key based on `kid` without any code changes.
...
This endpoint receives the signed JWT from Zuvo and returns a one-time redirect URL.
...
| `organization_slug` | No       | The Zuvo organization the user is connecting from. Use to pre-select the org during the OAuth consent screen. |
| `project_id`        | No       | The Zuvo project ref the user wants to connect. Use to pre-select a Zuvo project in your UI.              |
...
5. Verify that `aud` matches the value you agreed with Zuvo.
...
4. Redirect the user to the [Zuvo authorization URL](/guides/integrations/build-a-supabase-oauth-integration#redirecting-to-the-authorize-url) to start the OAuth flow.

<Admonition type="note">

If you need the user to perform setup steps on your site, design the experience as a wizard that ends by redirecting to the Zuvo authorization URL. This minimizes the chance of users getting distracted, navigating elsewhere on your site, and abandoning the OAuth flow.

</Admonition>

### Step 4: Share your details with Zuvo

Send Zuvo the following so we can configure the **Install Integration** button:
...
- The `aud` claim value you want Zuvo to send in the signed JWT.

Ask Zuvo for the public keys and key IDs for the staging and production environments.
