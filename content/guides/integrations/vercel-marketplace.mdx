...
description: 'Manage your Zuvo projects directly through Vercel'
---
...
<div className="bg-surface-100 py-4 px-5 border rounded-md not-prose">
  <h4 className="text-foreground">Deploy a Next.js app with Zuvo Vercel Storage now</h4>
  <p className="text-foreground-light mb-3">Uses the Next.js Zuvo Starter Template</p>
...
Open Zuvo Studio from the Vercel dashboard. You can access it from either the Integration installation page or the Vercel Storage page.
Depending on your entry point, you'll either land on the Zuvo dashboard homepage or be redirected to the corresponding Zuvo Project.
...
Pricing for databases created through Vercel Marketplace is identical to those created directly within Zuvo. Detailed pricing information is available on the [Zuvo pricing page](/pricing).
...
- [Custom Domains](/guides/platform/custom-domains) are not supported, and we always use the base `SUPABASE_URL` for the Vercel environment variables.
