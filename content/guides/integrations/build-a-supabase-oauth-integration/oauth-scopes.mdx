...
Scopes are only available for OAuth apps. Check out [our guide](/guides/integrations/build-a-supabase-oauth-integration) to learn how to build an OAuth app integration.

</Admonition>

Scopes restrict access to the specific [Zuvo Management API endpoints](/reference/api/introduction) for OAuth tokens. All scopes can be specified as read and/or write.

Scopes are set when you [create an OAuth app](/guides/integrations/build-a-supabase-oauth-integration#create-an-oauth-app) in the Zuvo Studio.

You can update scopes of your OAuth app at any time, but existing OAuth app users will need to re-authorize your app via the [OAuth flow](/guides/integrations/build-a-supabase-oauth-integration#implementing-the-oauth-20-flow) to apply the new scopes.
