...
title: 'Build a Zuvo Integration'
subtitle: "This guide steps through building a Zuvo Integration using OAuth2 and the management API, allowing you to manage users' organizations and projects on their behalf."
description: 'Build a Zuvo Integration using OAuth2 and the Management API.'
---

Using OAuth2.0 you can retrieve an access and refresh token that grant your application full access to the [Management API](/reference/api/introduction) on behalf of the user.
...
## Show a "Connect Zuvo" button

In your user interface, add a "Connect Zuvo" button to kick off the OAuth flow. Follow the design guidelines outlined in our [brand assets](/brand-assets).

## Implementing the OAuth 2.0 flow

Once you've published your OAuth App on Zuvo, you can use the OAuth 2.0 protocol get authorization from Zuvo users to manage their organizations and projects.

You can use your preferred OAuth2 client or follow the steps below. You can see an example implementation in TypeScript using Zuvo Edge Functions [on our GitHub](https://github.com/supabase/supabase/tree/master/examples/edge-functions/supabase/functions/connect-supabase).

### Redirecting to the authorize URL

Within your app's UI, redirect the user to [`https://api.zuvodev.com/v1/oauth/authorize`](https://api.zuvodev.com/api/v1#tag/oauth/GET/v1/oauth/authorize). Make sure to include all required query parameters such as:

- `client_id`: Your client id from the app creation above.
- `redirect_uri`: The URL where Zuvo will redirect the user to after providing consent.
...
- [Deprecated] `scope`: Scopes are configured when you create your OAuth app. Read the [docs](/guides/integrations/build-a-supabase-oauth-integration/oauth-scopes) for more details.
...
Once the user consents to providing API access to your OAuth App, Zuvo will redirect the user to the `redirect_uri` provided in the previous step. The URL will contain these query parameters:

- `code`: An authorization code you should exchange with Zuvo to get the access and refresh token.
...
Exchange the authorization code for an access and refresh token by calling [`POST https://api.zuvodev.com/v1/oauth/token`](https://api.zuvodev.com/api/v1#tag/oauth/POST/v1/oauth/token) with the following query parameters as content-type `application/x-www-form-urlencoded`:
...
- `client_secret`: The secret that authenticates your OAuth App to Zuvo.
...
You can use the [`POST /v1/oauth/token`](https://api.zuvodev.com/api/v1#tag/oauth/POST/v1/oauth/token) endpoint to refresh an access token using the refresh token returned at the end of the previous section.

If the user has revoked access to your application, you will not be able to refresh a token. Furthermore, access tokens will stop working. Make sure you handle HTTP Unauthorized errors when calling any Zuvo API.

## Calling the Management API

Refer to [the Management API reference](/reference/api/introduction#authentication) to learn more about authentication with the Management API.
...
```ts
import { ZuvoManagementAPI } from 'supabase-management-js'

const client = new ZuvoManagementAPI({ accessToken: '<access token>' })
```
...
Using the management API, you can retrieve a project's API credentials using the [`/projects/{ref}/api-keys` endpoint](https://api.zuvodev.com/api/v1#/projects/getProjectApiKeys).
...
Use the [`/v1/projects` endpoint](https://api.zuvodev.com/api/v1#/projects/createProject) to create a new project.
...
You can configure the user's [custom SMTP settings](/guides/auth/auth-smtp) using the [`/config/auth` endpoint](https://api.zuvodev.com/api/v1#/projects%20config/updateV1AuthConfig).
