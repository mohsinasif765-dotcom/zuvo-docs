...
Cache hits can be determined via the `log_attributes['response.headers.cf_cache_status']` key in the [logs](/guides/monitoring-and-debugging/logs). Any value that corresponds to either `HIT`, `STALE`, `REVALIDATED`, or `UPDATING` is categorized as a cache hit.
