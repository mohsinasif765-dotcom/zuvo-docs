...
subtitle: 'Learn about authenticating with Zuvo Storage S3.'
sidebar_label: 'S3'
---

You have two options to authenticate with Zuvo Storage S3:
...
This is all the information you need to connect to Zuvo Storage using any S3-compatible service.



![Storage S3 Access keys](/img/storage/s3-credentials.png)
...
On [local development](/guides/local-development), use these values:
...
You can authenticate to Zuvo S3 with a user JWT token to provide limited access via RLS to all S3 operations. This is useful when you want initialize the S3 client on the server scoped to a specific user, or use the S3 client directly from the client side.
...
- On self-hosted Zuvo, the `accessKeyId` is the `STORAGE_TENANT_ID` environment variable defined in the `.env` file. Refer to the [self-hosted S3 guide](/guides/self-hosting/self-hosted-s3#session-token) for more details.
- On [local development](/guides/local-development), use the following values:
  - `region`: `local`
  - `endpoint`: IP and port e.g. `http://127.0.0.1:54321/storage/v1/s3`
  - `accessKeyId`: `stub`
