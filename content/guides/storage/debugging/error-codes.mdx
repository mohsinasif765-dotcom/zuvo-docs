...
When using the Zuvo SDKs, storage errors provide detailed information to help you debug issues:
...
| `SignatureDoesNotMatch`     | The request signature does not match the calculated signature.  | 403          | Check your credentials, access key id / access secret key / region that are all correct, refer to [S3 Authentication](/guides/storage/s3/authentication).                              |
...
- Add an RLS policy to grant permission to the resource. See our [Access Control docs](/guides/storage/security/access-control) for more information.
...
- Use the `upsert` functionality in order to overwrite the file. Find out more [here](/guides/storage/uploads/standard-uploads#overwriting-files).
...
- Add RLS policy to grant permission. See our [Access Control docs](/guides/storage/security/access-control) for more information.
- Ensure you include the user `Authorization` header
