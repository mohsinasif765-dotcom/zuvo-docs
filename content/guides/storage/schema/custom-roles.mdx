...
In this guide, you will learn how to create and use custom roles with Storage to manage role-based access to objects and buckets. The same approach can be used to use custom roles with any other Zuvo service.

Zuvo Storage uses the same role-based access control system as any other Zuvo service using RLS (Row Level Security).

## Create a custom role

Create a custom role `manager` to provide full read access to a specific bucket. For a more advanced setup, see the [RBAC Guide](/guides/api/custom-claims-and-role-based-access-control-rbac#create-auth-hook-to-apply-user-role).
...
const JWT_SECRET = 'your-jwt-secret' // You can find this in your Zuvo project settings under API. Store this securely.
