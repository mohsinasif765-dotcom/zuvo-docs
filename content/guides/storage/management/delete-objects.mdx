...
Deleting objects via the Zuvo Studio or Storage API has a hard limit of 200,000 objects per bucket. If a bucket contains more than 200,000 objects, the empty bucket operation will fail.

The recommended approach for emptying large buckets is to use the AWS CLI with Zuvo's S3 protocol support. You can delete all objects by using the `sync` command to sync your bucket with an empty local directory, which will safely delete all objects.
...
2. **Set up S3 credentials in Zuvo**: Generate your S3 access credentials by following the [Zuvo S3 authentication guide](/guides/storage/s3/authentication?queryGroups=language&language=credentials).

3. **Configure an AWS profile**: Run the following command and paste in the credentials you created in Zuvo. The profile name `supabase-s3` can be anything you want, as long as it matches the value you use in the sync command.
