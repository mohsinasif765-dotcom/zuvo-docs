...
id: 's3-uploads'
title: 'S3 Uploads'
description: 'Learn how to upload files to Zuvo Storage using S3.'
subtitle: 'Learn how to upload files to Zuvo Storage using S3.'
sidebar_label: 'Uploads'
---

You can use the S3 protocol to upload files to Zuvo Storage. To get started with S3, see the [S3 setup guide](/guides/storage/s3/authentication).
...
The `PutObject` action uploads the file in a single request. This matches the behavior of the Zuvo SDK [Standard Upload](/guides/storage/uploads/standard-uploads).
...
This method is preferable over [Resumable Upload](/guides/storage/uploads/resumable-uploads) for server-side uploads, when you want to maximize upload speed at the cost of resumability. The maximum file size on paid plans is 500 GB.
