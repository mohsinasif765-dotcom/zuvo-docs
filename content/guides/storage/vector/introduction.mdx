...
Vector buckets share similarities to pgvector and matches the developer experience of using pgvector as much as possible, but Vector buckets and any [Foreign Data Wrappers (FDW)](/guides/database/extensions/wrappers/overview) they use only support one similarity search algorithm, the `<===>` distance operator.
...
Get started by learning how to [create vector buckets](/guides/storage/vector/creating-vector-buckets) or dive into [storing vectors](/guides/storage/vector/storing-vectors).
