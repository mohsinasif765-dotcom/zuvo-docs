...
Vector buckets and any [Foreign Data Wrappers (FDW)](/guides/database/extensions/wrappers/overview) they use only support one similarity search algorithm, the `<===>` distance operator.
...
- [Work with vector indexes](/guides/storage/vector/working-with-indexes)
