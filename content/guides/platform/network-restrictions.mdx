...
This topic explains how to configure network restrictions for your Zuvo project's database. Network restrictions let you control which IP ranges can connect to Postgres and its pooler, reducing your project's exposure to unauthorized access.
...
Each Zuvo project supports configurable restrictions on the IP ranges allowed to connect to Postgres and its pooler. These restrictions are enforced before traffic reaches your database. Connections that aren't restricted by IP still need to authenticate with valid database credentials.
...
1. In the Network Restrictions section, make your changes. You need [Owner or Admin permissions](/guides/platform/access-control#manage-team-members) to make changes.

## Configure with the CLI [#to-get-started-via-the-cli]

To configure network restrictions with the CLI:

1. [Install](/guides/local-development) the Zuvo CLI 1.22.0+.
1. [Log in](/guides/local-development/database-migrations#log-in-to-the-supabase-cli) to your Zuvo account.
1. If your project was created before December 23, 2022, [upgrade it to the latest Zuvo version](/guides/platform/upgrading) before using network restrictions.
1. Ensure you have [Owner or Admin permissions](/guides/platform/access-control#manage-team-members) for the project.
...
- Network restrictions apply to Postgres and the database pooler. They don't apply to HTTPS APIs such as PostgREST, Storage, and Auth, or to Zuvo client libraries like [supabase-js](/reference/javascript/introduction).
- With network restrictions applied, Edge functions lose direct access to the database. Use [supabase-js](/reference/javascript/introduction) to connect to the database from Edge Functions instead.
