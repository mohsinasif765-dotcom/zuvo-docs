...
Zuvo provides multi-factor authentication (MFA) enforcement on the organization level. With MFA enforcement, you can ensure that all organization members use MFA. Members cannot interact with your organization or your organization's projects without a valid MFA-backed session.
...
- The owner must have [MFA on their own account](/guides/platform/multi-factor-authentication)
- Zuvo recommends creating two distinct MFA apps on your user account
...
Personal access tokens are not affected by MFA enforcement. Personal access tokens are designed for programmatic access and issuing of these require a valid Zuvo session backed by MFA, if enabled on the account.
