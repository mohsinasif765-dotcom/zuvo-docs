...
id: 'privatelink'
title: 'PrivateLink'
description: 'Secure private network connectivity to your Zuvo database using AWS VPC Lattice.'
---
...
Zuvo PrivateLink supports direct database connections on port `5432` and PgBouncer connections on port `6543`. It does not support other Zuvo services like API, Storage, Auth, or Realtime. These services will continue to operate over public internet connections.

## Requirements

To use PrivateLink with your Zuvo project:

- Team or Enterprise Zuvo subscription
- AWS VPC in the same region as your Zuvo project
...
1. Go to your Zuvo project dashboard
...
After submission, Zuvo creates a VPC Lattice Resource Configuration for your project and sends an AWS Resource Share to the specified AWS account ID. This process may take a few moments. Once complete, the connection will show a "Waiting" status, indicating that the resource share has been sent to your AWS account and still needs to be accepted. You must accept the resource share within 12 hours, or the request expires and can no longer be accepted in AWS. You'll need to create a new connection to try again.
...
Zuvo will send you an AWS Resource Share containing the VPC Lattice Resource Configurations for your projects. To accept this share:

1. Login to your AWS Management Console, ensure you are in the AWS region where your Zuvo project is located
...
4. Locate the resource share from Zuvo.
...
- **Service Scope**: PrivateLink only supports database connections (Postgres and PgBouncer). Other Zuvo services (API, Storage, Auth, Realtime) will continue to operate over public internet connections.
