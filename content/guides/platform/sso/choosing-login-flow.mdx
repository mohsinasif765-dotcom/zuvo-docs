...
```
Do users need to start login at zuvodev.com?
│
...
└─ Yes → Do you need multiple SAML apps per domain?
         ...
         └─ No → Enable both flows ✅
                  - SP-initiated for users who bookmark zuvodev.com
...
- You need separate Zuvo organizations for Dev, Staging, and Production
...
- Users click "Zuvo Dev", "Zuvo Staging", or "Zuvo Prod" tiles
- No domain conflicts

**How to configure:**

1. In your IdP, create three SAML apps:
   - "Zuvo Dev" → Points to dev org ACS URL
   - "Zuvo Staging" → Points to staging org ACS URL
   - "Zuvo Production" → Points to prod org ACS URL
2. In each Zuvo organization:
   - Enable SSO
...
- One Zuvo organization for your entire company
...
- Users access Zuvo with one click from IdP
- Fewer potential failure points

**How to configure:**

1. Enable SSO in your Zuvo organization
2. Leave "Enable SP-initiated login" **OFF**
3. Configure identity provider metadata
4. Create Zuvo app tile in your IdP

**Result:** One-click SSO login for all users.

---

### Scenario 3: Users bookmark zuvodev.com

**Your situation:**

- Users frequently bookmark zuvodev.com directly
- You want to support starting login from Zuvo
...
- Supports users who start at zuvodev.com (SP-initiated)
...
5. Create Zuvo app tile in your IdP (optional but recommended)

**Result:** Users can start login from either Zuvo or IdP.
...
- Users are used to starting at zuvodev.com
...
- All use the same Zuvo organization
...
5. Create Zuvo app tiles in IdP (IdP-initiated also works)
...
| Users bookmark zuvodev.com              | ✅ Yes        | ✅ Yes       | Yes               |
...
1. **IdP-initiated:** Click app tile in IdP → Verify redirect to Zuvo
2. **SP-initiated:** Go to zuvodev.com/sign-in-sso → Enter email → Verify IdP redirect
3. **Auto-join:** Test with new user accounts
4. **Domain restrictions:** Try non-matching domain (should fail for SP-initiated)

See our comprehensive [SSO Testing and Best Practices guide](/guides/platform/sso/testing-best-practices) for detailed testing procedures.
...
- Users who bookmarked zuvodev.com need to use IdP tiles instead
...
If you need help choosing the right configuration for your organization, contact Zuvo support with details about your use case. We're happy to provide personalized recommendations.

</Admonition>

## Next steps

- **Understand the technical details:** Read [Understanding SSO Login Flows](/guides/platform/sso/login-flows)
- **Configure your provider:** Follow our [provider-specific guides](/guides/platform/sso#supported-providers)
- **Test your setup:** Review [SSO Testing and Best Practices](/guides/platform/sso/testing-best-practices)
- **Enable auto-join:** Configure [auto-join settings](/guides/platform/sso#key-configuration-options)
