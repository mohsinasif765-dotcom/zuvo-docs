...
- Login flow type decided (IdP-initiated, SP-initiated, or both) - see [Choosing a Login Flow](/guides/platform/sso/choosing-login-flow)
...
Before testing auto-join and other features, verify which login flows work for your SSO configuration. See [Understanding SSO Login Flows](/guides/platform/sso/login-flows) for technical details.
...
**Multiple environments:** If you're setting up Dev/Staging/Prod, this domainless pattern is recommended. See [Multiple SSO Providers](/guides/platform/sso/multiple-providers) for detailed configuration guidance.
...
7. **Test with domainless (IdP-initiated only) configuration**:
   ...
   This test is critical if you're using multiple environments under the same domain. See [Multiple SSO Providers](/guides/platform/sso/multiple-providers) for details.
...
If you're configuring multiple SSO providers for different environments (dev/staging/prod), the testing steps outlined here apply to each provider individually. For advanced multi-provider configuration strategies, see the [Multiple SSO Providers guide](/guides/platform/sso/multiple-providers).
...
5. Check domain configuration in Zuvo matches user email
...
- See [access control documentation](/guides/platform/access-control) for role details
...
- See [Multiple SSO Providers guide](/guides/platform/sso/multiple-providers) for details
