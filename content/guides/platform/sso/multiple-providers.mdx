...
Many enterprises need multiple single sign-on (SSO) providers configured within Zuvo to support different environments, teams, or organizational structures. This guide explains when and how to set up multiple providers effectively.
...
For technical details, see [the Understanding SSO Login Flows guide](/guides/platform/sso/login-flows#multiple-saml-apps-per-domain).
...
You have three Zuvo organizations:
...
Create three separate SAML apps in your identity provider, each pointing to a different Zuvo organization.

#### In your identity provider (Okta, Azure AD, Google Workspace)

1. **Create "Zuvo Dev" SAML app**
   - Configure with Dev organization's ACS URL
   - Assign developers and testers
   - Deploy app tile labeled "Zuvo Dev"

2. **Create "Zuvo Staging" SAML app**
   - Configure with Staging organization's ACS URL
   - Assign QA team and release managers
   - Deploy app tile labeled "Zuvo Staging"

3. **Create "Zuvo Production" SAML app**
   - Configure with Production organization's ACS URL
   - Assign production users only
   - Deploy app tile labeled "Zuvo Production"

#### In each Zuvo organization
...
| ----------- | ------------ | ------------------ | ------------------------------------ |
| Dev         | `dev`        | "Zuvo Dev"     | `https://...dev-org.../saml/acs`     |
| Staging     | `staging`    | "Zuvo Staging" | `https://...staging-org.../saml/acs` |
| Production  | `prod`       | "Zuvo Prod"    | `https://...prod-org.../saml/acs`    |
...
Some organizations need to isolate teams within separate Zuvo organizations.
...
- "Zuvo Engineering" → Points to `engineering-org`
- "Zuvo Data" → Points to `data-org`
...
- Users can log in via SP-initiated at zuvodev.com
...
4. Users with any matching domain can log in via zuvodev.com
...
- [Okta setup guide](/guides/platform/sso/okta)
- [Azure AD setup guide](/guides/platform/sso/azure)
- [Google Workspace setup guide](/guides/platform/sso/gsuite)
...
#### Step 3: Configure each Zuvo organization
...
3. Verify redirect to correct Zuvo organization
4. Check that user information is populated correctly
5. Verify auto-join behavior (if enabled)

See [the SSO Testing and Best Practices guide](/guides/platform/sso/testing-best-practices) for comprehensive testing procedures.
...
Engineering Team:
├─ Zuvo Dev (assigned) ✅
├─ Zuvo Staging (assigned) ✅
└─ Zuvo Prod (assigned) ✅

QA Team:
├─ Zuvo Dev (assigned) ✅
├─ Zuvo Staging (assigned) ✅
└─ Zuvo Prod (NOT assigned) ❌

Contractors:
├─ Zuvo Dev (assigned) ✅
├─ Zuvo Staging (NOT assigned) ❌
└─ Zuvo Prod (NOT assigned) ❌
```
...
#### Zuvo organization names

Match your IDP app names when possible:

- IDP app: "Zuvo - Production" → Org: `acme-production`
- IDP app: "Zuvo - Staging" → Org: `acme-staging`
- IDP app: "Zuvo - Dev" → Org: `acme-development`
...
2. **Test thoroughly** (see [testing guide](/guides/platform/sso/testing-best-practices))
...
User clicks "Zuvo Prod" tile but sees the dev environment.
...
2. Compare metadata in Zuvo SSO settings
...
User cannot find "Zuvo Staging" tile in IDP dashboard.
...
1. Create additional Zuvo organizations
2. Configure projects within each organization
3. Migrate data if needed (see [project transfer](/guides/platform/project-transfer))
...
- **Configure your IDP:** Follow your [provider-specific guide](/guides/platform/sso#supported-providers)
- **Test thoroughly:** Review [SSO Testing and Best Practices](/guides/platform/sso/testing-best-practices)
- **Understand login flows:** Read [Understanding SSO Login Flows](/guides/platform/sso/login-flows)
- **Choose the right flow:** See [Choosing the Right Login Flow](/guides/platform/sso/choosing-login-flow)
