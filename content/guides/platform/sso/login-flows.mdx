...
Most enterprises use IdP-initiated flow for its simplicity and better user experience. Enable SP-initiated only if you need users to start their login journey at zuvodev.com.

See our [Choosing the Right Login Flow guide](/guides/platform/sso/choosing-login-flow) for use case examples.
...
With IdP-initiated flow, users start their login journey from your identity provider (Okta, Azure AD, Google Workspace, etc.) and are directly authenticated into Zuvo.

**User experience:**

1. User opens their identity provider dashboard (e.g., Okta homepage, Azure MyApps)
2. User clicks the Zuvo app tile or bookmark
3. User is immediately logged into Zuvo (if already authenticated with IdP)
...
- ✅ Default behavior in Zuvo

### SP-initiated (Service Provider Initiated)

With SP-initiated flow, users start at zuvodev.com, enter their email address, and are redirected to your identity provider for authentication.

**User experience:**

1. User visits zuvodev.com and clicks "Sign in with SSO"
2. User enters their email address
3. User is redirected to their identity provider
4. After authenticating, user is redirected back to Zuvo

**Key characteristics:**

- ✅ Familiar flow for users who bookmark zuvodev.com
...
- "Users should never need to remember zuvodev.com"

### When to use SP-initiated

**Best for:**

- Organizations where users bookmark zuvodev.com directly
- Migrating from password-based authentication
- Users unfamiliar with identity provider dashboards

**Common scenarios:**

- "Some users bookmark zuvodev.com and expect to start there"
...
Users can now access Zuvo through your IdP's app catalog.
...
Only users with email addresses matching your configured domains can use SP-initiated login. Users with other domains cannot sign in via SSO at zuvodev.com (but can still use IdP-initiated flow if you configure it in your IdP).
...
2. IdP generates SAML assertion and POSTs to Zuvo ACS URL
3. Zuvo validates assertion and creates session
4. User is redirected to Zuvo dashboard

**No domain lookup required** - The IdP assertion contains all necessary user information.

### How SP-initiated flow works

1. User enters email at zuvodev.com/sign-in-sso
2. Zuvo matches email domain to configured SSO provider
3. Zuvo generates SAML request and redirects to IdP
4. IdP authenticates user and generates SAML assertion
5. IdP POSTs assertion to Zuvo ACS URL
6. Zuvo validates assertion and creates session
...
- Create "Zuvo Dev" SAML app → Points to dev org's ACS URL
- Create "Zuvo Staging" SAML app → Points to staging org's ACS URL
- Create "Zuvo Production" SAML app → Points to prod org's ACS URL
...
Without domains, only IdP-initiated flow is available. Users cannot start their login at zuvodev.com.

### Does the IdP require configuration?

For **IdP-initiated flow:** Configure the Zuvo ACS URL and entity ID in your IdP. See our provider-specific guides:

- [Google Workspace](/guides/platform/sso/gsuite)
- [Azure Active Directory](/guides/platform/sso/azure)
- [Okta](/guides/platform/sso/okta)

For **SP-initiated flow:** Same configuration, but also ensure your IdP accepts SAML requests from Zuvo.
...
See our [SSO Testing and Best Practices](/guides/platform/sso/testing-best-practices) guide for security recommendations.

## Next steps

- **Choose your login flow:** See [Choosing the Right Login Flow](/guides/platform/sso/choosing-login-flow)
- **Configure your provider:** Follow our [provider-specific guides](/guides/platform/sso#supported-providers)
- **Test thoroughly:** Review [SSO Testing and Best Practices](/guides/platform/sso/testing-best-practices)
- **Enable auto-join:** Configure [auto-join settings](/guides/platform/sso#key-configuration-options) for seamless onboarding
