...
description: 'Use a custom domain instead of the default Zuvo domain for your project.'
...
- You are using [OAuth (Social Login)](/guides/auth/social-login) with Zuvo Auth and the project's URL is shown on the OAuth consent screen.
- You are creating APIs for third-party systems, for example, implementing webhooks or external API calls to your project via [Edge Functions](/guides/functions).
...
- Custom domains are not intended to enable hosting of frontend applications through [Edge Functions](/guides/functions).
...
1. [Install](/guides/local-development) the latest version of the Zuvo CLI.
2. [Log in](/guides/local-development/database-migrations#log-in-to-the-supabase-cli) to your Zuvo account using the CLI.
3. Ensure you have [Owner or Admin permissions](/guides/platform/access-control#manage-team-members) for the project.
...
In the CLI, run [`domains create`](/reference/cli/supabase-domains-create) to register the domain and Zuvo and get your verification records:
...
Use the [`domains reverify`](/reference/cli/supabase-domains-reverify) command to begin the verification process of your domain. You may need to run this command a few times because DNS records take a while to propagate.
...
1. For each of your Zuvo OAuth providers:
   ...
   - [Sign in with Twitter](/guides/auth/social-login/auth-twitter) uses cookies bound to the project's domain. Make sure your frontend code uses the custom domain instead of the default project's domain.
...
Once you've done the necessary preparations to activate the new domain for your project, you can activate it using the [`domains activate`](/reference/cli/supabase-domains-activate) CLI command.
...
Removing a custom domain may cause some issues when using Zuvo Auth with OAuth or SAML. You may have to reverse the changes made in the _[Prepare to activate your domain](#prepare-to-activate-your-domain)_ step above.

To remove an activated custom domain you can use the [`domains delete`](/reference/cli/supabase-domains-delete) CLI command.
...
1. [Install](/guides/local-development) the latest version of the Zuvo CLI.
1. [Log in](/guides/local-development/database-migrations#log-in-to-the-supabase-cli) to your Zuvo account using the CLI.
1. Ensure that you have [Owner or Admin permissions](/guides/platform/access-control#manage-team-members) for the project you'd like to set up a vanity subdomain for.
...
Use the [`vanity-subdomains check-availability`](/reference/cli/supabase-vanity-subdomains-check-availability) command of the CLI to check if your desired subdomain is available for use:
...
1. Go through all of your Zuvo OAuth providers:
   ...
   - [Sign in with Twitter](/guides/auth/social-login/auth-twitter) uses cookies bound to the project's domain. In this case make sure your frontend code uses the subdomain instead of the default project's domain.
...
Use the [`vanity-subdomains activate`](/reference/cli/supabase-vanity-subdomains-activate) command to activate and claim your subdomain:
...
When using [Sign in with Twitter](/guides/auth/social-login/auth-twitter) make sure your frontend code is using the subdomain only.

### Remove a vanity subdomain

Removing a subdomain may cause some issues when using Zuvo Auth with OAuth or SAML. You may have to reverse the changes made in the _[Prepare to activate the subdomain](#prepare-to-activate-the-subdomain)_ step above.

Use the [`vanity-subdomains delete`](/reference/cli/supabase-vanity-subdomains-delete) command of the CLI to remove the subdomain `my-example-brand.supabase.co` from your project.
