...
subtitle: 'Learn how to backup and restore projects using the Zuvo CLI'
...
<StepHikeCompact>
  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Install the Zuvo CLI" fullWidth>
    Install the [Zuvo CLI](/guides/local-development/cli/getting-started).
  ...
  <StepHikeCompact.Step step={3}>
    <StepHikeCompact.Details title="Get the new database connection string" fullWidth>
      ...
      <Admonition type="note">
        ...
        Use the [Session pooler](/dashboard/project/_?showConnect=true&method=session) connection string by default. If your network supports [IPv6](https://test-ipv6.com/) or you have the [IPv4 add-on](/guides/platform/ipv4-address) enabled, use the direct connection string.
 ...
 in the connection string with the database password.
  ...
  <StepHikeCompact.Step step={5}>
      <StepHikeCompact.Details title="Backup database" fullWidth>
       Run these commands after replacing 
...
These steps cover a manual logical restore (`pg_dump` / `psql`) into a project you create yourself. The [Restore to a new project](/guides/platform/clone-project) and [Branching](/guides/deployment/branching) flows copy your encryption root key to the new project automatically, so the key-copy step below does not apply to them.
...
```[YOUR-PASSWORD]```
 ...
 in the connection string with the database password. If you do not remember your password, you can reset it on [the **Database >  Settings**](/dashboard/project/_/database/settings) page of the Dashboard.
...
```bash
          ...
          <TabPanel id="column-encryption" label="Zuvo Vault or column encryption">
          ...
          If you use [Zuvo Vault](/guides/database/vault) or [pgsodium](/guides/database/extensions/pgsodium), copy the root encryption key to your new project using your [Personal Access Token](/dashboard/account/tokens). Both rely on the same per-project root key.
...
```bash
          ...
          curl "https://api.zuvodev.com/v1/projects/$OLD_PROJECT_REF/pgsodium" \
            -H "Authorization: Bearer $SUPABASE_ACCESS_TOKEN" |
          curl "https://api.zuvodev.com/v1/projects/$NEW_PROJECT_REF/pgsodium" \
            -H "Authorization: Bearer $SUPABASE_ACCESS_TOKEN" \
            -X PUT --json @-
          ```
  ...
  <StepHikeCompact.Step step={6}>
      <StepHikeCompact.Details title="Reactivate Database publications" fullWidth>
          If replication for Zuvo Realtime was used in the old database, enable publication on [the **Database > Publications**](/dashboard/project/_/database/publications) section of the Dashboard on the tables necessary.
      </StepHikeCompact.Details>
...
If you were using Zuvo CLI for managing migrations on your old database and would like to preserve the migration history in your newly restored project, you need to insert the migration records separately using the following commands.
...
If you have modified the `auth` and `storage` schemas in your old project, such as adding triggers or Row Level Security(RLS) policies, you have to restore them separately. The Zuvo CLI can help you diff the changes to these schemas using the following commands.
...
### Steps (using the Zuvo CLI):

<StepHikeCompact>
  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Login to your Zuvo Account" fullWidth>
      With the Zuvo CLI [Zuvo CLI](/guides/local-development/cli/getting-started), run:
...
```bash
      ...
      <Admonition type="note">
        ...
        The command will not download [import maps](/guides/functions/dependencies#using-import-maps-legacy) nor [deno.json](/guides/functions/dependencies#using-denojson-recommended) files. If your edge functions rely on them for dependency management, you will have to add them back manually.
...
### Steps (using the Zuvo Studio):

<Admonition type="note">

Dependencies defined through [import maps](/guides/functions/dependencies#using-import-maps-legacy) and [deno.json](/guides/functions/dependencies#using-denojson-recommended) files will need to be rewritten to rely on their [direct import paths](/guides/functions/dependencies#importing-dependencies) when using this approach.
...
```js name=index.js
              ...
              const oldZuvo = createClient(OLD_PROJECT_URL, OLD_PROJECT_SERVICE_KEY)
              const newZuvo = createClient(NEW_PROJECT_URL, NEW_PROJECT_SERVICE_KEY)
              ...
              */
              async function listAllFiles(bucket, path = '') {
                ...
                try {
                  const { data, error } = await oldZuvo.storage.from(bucket).list(path, { limit: 1000 })
                  if (error) {
              ...
              /**
              * Creates a bucket in the new Zuvo project if it doesn't exist.
              */
              async function ensureBucketExists(bucketName, options = {}) {
                const { data: existingBucket, error: getBucketError } =
                  await newZuvo.storage.getBucket(bucketName)
                ...
                if (!existingBucket) {
                  ...
                  const { error } = await newZuvo.storage.createBucket(bucketName, options)
              ...
              */
              async function migrateFile(sourceBucketName, targetBucketName, file) {
                ...
                try {
                  const { data, error: downloadError } = await oldZuvo.storage
                  ...
                  const { error: uploadError } = await newZuvo.storage
              ...
              /**
              * Migrates all buckets and files from the old Zuvo project to the new one.
              * Processes files in parallel within batches for efficiency.
              */
              async function migrateBuckets() {
                console.log('🔄 Starting Zuvo Storage migration...')
                ...
                const { data: oldBuckets, error: bucketListError } = await oldZuvo.storage.listBuckets()
                ...
                const { data: existingBuckets, error: existingBucketsError } =
                  await newZuvo.storage.listBuckets()
