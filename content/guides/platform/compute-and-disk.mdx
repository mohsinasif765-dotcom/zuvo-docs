...
Every project on the Zuvo Platform comes with its own dedicated Postgres instance.
...
In paid organizations, Nano Compute are billed at the same price as Micro Compute. It is recommended to upgrade your Project from Nano Compute to Micro Compute when it's convenient for you. Compute sizes are not auto-upgraded because of the downtime incurred. See [Zuvo Pricing](/pricing) for more information. You cannot launch Nano instances on paid plans, only Micro and above - but you might have Nano instances after upgrading from Free Plan.
...
[^1]: Database max connections are recommended values and can be [customized via `max_connections`](/guides/database/custom-postgres-config) depending on your use case. Be aware of [these considerations](/guides/troubleshooting/how-to-change-max-database-connections-_BQ8P5) before modifying.
...
Compute sizes can be changed by first selecting your project in the dashboard [here](/dashboard/project/_/settings/infrastructure) and the upgrade process will [incur downtime](/guides/platform/compute-and-disk#upgrades).



![Compute Size Selection](/img/guides/platform/compute-size-selection--light.png)



We charge hourly for additional compute based on your usage. Read more about [usage-based billing for compute](/guides/platform/manage-your-usage/compute).

### Dedicated vs shared CPU

All Postgres databases on Zuvo run in isolated environments. Compute instances `Nano` to `2XL` compute size have CPUs which can burst to higher performance levels for short periods of time. Instances bigger than `Large` have predictable performance levels and do not exhibit the same burst behavior.
...
Larger compute instances (4XL and above) are designed for sustained, high performance with specific IOPS and throughput limits which you can [configure](/guides/platform/manage-your-usage/disk-throughput). If you hit your IOPS or throughput limit, throttling will occur.
...
[Replication Slots](https://postgresqlco.nf/doc/en/param/max_replication_slots) and [WAL Senders](https://postgresqlco.nf/doc/en/param/max_wal_senders/) are used to enable [Postgres Replication](/guides/database/replication). Each compute instance also has limits on the maximum number of database connections and connection pooler clients it can handle.
