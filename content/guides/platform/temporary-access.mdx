...
description: 'Enable temporary access for short-lived database connections tied to a Zuvo user.'
---
...
Temporary access authentication only applies for Zuvo project users authenticating to the database, and assuming the given Postgres role. No new roles or users are created in the Postgres database and the assumed role's permissions will still apply.
