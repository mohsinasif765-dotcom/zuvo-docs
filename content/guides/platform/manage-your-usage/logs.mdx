...
- **Logs Ingest** — the total GB of log data Zuvo ingests across all your project's services (Postgres, API gateway, Auth, Storage, Realtime, Edge Functions, etc.) during the billing cycle.
...
- [Manage Logs Ingest usage](/guides/platform/manage-your-usage/logs-ingest)
- [Manage Logs Query usage](/guides/platform/manage-your-usage/logs-query)

## Logs vs log drains

[Log Drains](/guides/platform/manage-your-usage/log-drains) stream logs out of Zuvo to external destinations (Datadog, Better Stack, your own S3 bucket, etc.) and are billed separately on drain hours and events. Draining logs does not replace or reduce Logs Ingest charges — ingest is metered when Zuvo processes your logs, drains are metered when Zuvo streams them out. These are separate billing primitives, not overlapping charges.
