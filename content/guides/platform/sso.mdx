...
Looking for docs on how to add Single Sign-On support in your Zuvo project? Head on over to [Single Sign-On with SAML 2.0 for Projects](/guides/auth/enterprise-sso/auth-sso-saml).

</Admonition>

Zuvo offers single sign-on (SSO) as a login option to provide additional account security for your team. This allows company administrators to enforce the use of an identity provider when logging into Zuvo. SSO improves the onboarding and offboarding experience of the company as the employee only needs a single set of credentials to access third-party applications or tools which can also be revoked by an administrator.

<Admonition type="note">

Zuvo currently provides SAML SSO for [Team and Enterprise Plan customers](/pricing). If you are an existing Team or Enterprise Plan customer, continue with the setup below.

</Admonition>

## Supported providers

Zuvo supports practically all identity providers (IdP) that support the SAML 2.0 SSO protocol. These guides cover commonly used identity providers to help you get started. If you use a different provider, contact support.

- [Google Workspaces (formerly G Suite)](/guides/platform/sso/gsuite)
- [Azure Active Directory](/guides/platform/sso/azure)
- [Okta](/guides/platform/sso/okta)
...
After configuring your SSO provider, thorough testing is essential. See our [SSO Testing and Best Practices](/guides/platform/sso/testing-best-practices) guide for:
...
Zuvo supports two SSO login flows: **IdP-initiated** and **SP-initiated**. You can enable one or both depending on your organization's needs.
...
Users start their login at zuvodev.com by entering their email address, then are redirected to your identity provider. This flow requires configuring email domains to route users to the correct IdP.

**Best for:**

- Users who bookmark zuvodev.com directly
...
- **Detailed guidance:** See [Choosing the Right Login Flow](/guides/platform/sso/choosing-login-flow) for scenario-based recommendations.
- **Technical details:** Read [Understanding SSO Login Flows](/guides/platform/sso/login-flows) for in-depth explanations.

## Key configuration options

- **Login flows** - Choose between IdP-initiated (users start from identity provider), SP-initiated (users start at zuvodev.com), or both. IdP-initiated is recommended for most organizations and requires no domain configuration. See [Choosing the Right Login Flow](/guides/platform/sso/choosing-login-flow) for guidance.
- **Email domains** - Required only if you enable SP-initiated login. You can associate one or more email domains with your SSO provider. Users with matching email addresses can sign in via SSO at zuvodev.com. Not required for IdP-initiated flow.
...
- **Default role for auto-joined users** - Choose the role (e.g., `Read-only`, `Developer`, `Administrator`, `Owner`) that automatically joined users receive. We recommend using `Developer` as the default (principle of least privilege) and promoting users individually as needed. Refer to [access control](/guides/platform/access-control) for more information about roles.
...
- If an SSO user with the following email of `alice@foocorp.com` attempts to sign in with a GitHub account that uses the same email, a separate Zuvo account is created and will not be linked to the SSO user's account.
- SSO users will only see organizations/projects they've been invited to or auto-joined into. See [access control](/guides/platform/access-control) for more details.
...
6. Thoroughly test using our [SSO Testing and Best Practices](/guides/platform/sso/testing-best-practices) guide
...
- Downgrade or remove their permissions for any organizations in Zuvo.
...
Visit the [SSO Testing and Best Practices](/guides/platform/sso/testing-best-practices) guide for complete details.

## Advanced scenarios

Most organizations use a single SSO provider for all users. However, Zuvo supports multiple SSO providers within an organization for advanced use cases such as:
...
If you need to configure multiple SSO providers, refer to the [Multiple SSO Providers](/guides/platform/sso/multiple-providers) guide for detailed configuration steps, and contact your Zuvo support representative if you need additional guidance.

## Enterprise-managed authentication for MCP

Once SSO is configured, you can let your identity provider automatically authorize MCP clients for your organization, without individual members having to approve each one. See [Enterprise-Managed Authentication for MCP](/guides/platform/sso/enterprise-mcp-authentication) for details.
