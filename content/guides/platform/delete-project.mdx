...
Deleting a Zuvo project is a **permanent and irreversible action**. Before proceeding, understand the full scope of what will be deleted and take precautions to prevent accidental data loss.
...
For more information, see the [Zuvo CLI documentation](/reference/cli/supabase-projects-delete).
...
```bash
curl -X DELETE https://api.zuvodev.com/v1/projects/<project-ref> \
  -H "Authorization: Bearer <access-token>"
```



For more information, see the [Management API documentation](/reference/api/v1-delete-a-project).
...
See our [Fair Use Policy](/guides/platform/billing-faq#fair-use-policy) for further details.
...
You can backup your database by following the [backup and restore guide](/guides/platform/migrating-within-supabase/backup-restore) provided.

If you are on the Pro, Team or Enterprise Plan, with legacy logical backups, you can download a copy from [your Zuvo dashboard](/dashboard/project/_/database/backups/scheduled)

### 2. Download storage objects

- Back up all important files from Storage buckets
- Use the Zuvo dashboard or API to download files in bulk
- Store in a secure location outside of Zuvo
...
See [Access Controls](/guides/platform/access-control)
...
- Use the Zuvo dashboard to configure team member permissions
...
- Contact [Zuvo support](/support) for guidance
