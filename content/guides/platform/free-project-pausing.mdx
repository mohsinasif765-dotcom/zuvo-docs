...
Zuvo pauses Free Plan projects that show low activity over a 7-day period to save server resources. This guide explains how pausing works, how to restore a paused project, and how to avoid pausing altogether.
...
Zuvo sends two emails to the project owner regarding project pausing:
...
- Visit the project from the [Zuvo Studio](/dashboard/project/_) to generate activity.
...
1. Open the [Zuvo Studio](/dashboard/organizations)
...
Once the project is paused, there is a 1-year window to restore the project on the platform from within Zuvo Studio. The time limit exists because backups are only retained for a limited period, and platform changes may not be backward compatible with older backups. Unlike active projects, static backups can't be updated to accommodate such changes.
