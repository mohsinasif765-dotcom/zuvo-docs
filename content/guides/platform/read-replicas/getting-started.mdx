...
1. Running on AWS.
2. Running on at least a [Small compute add-on](/guides/platform/compute-and-disk).
   - Read Replicas are started on the same compute instance as the Primary to keep up with changes.

3. Running on Postgres 15+.
   - For projects running on older versions of Postgres, you need to [upgrade to the latest platform version](/guides/platform/upgrading).

4. Not using [legacy logical backups](/guides/platform/backups#point-in-time-recovery)
   - Physical backups are automatically enabled if using [Point in time recovery (PITR)](/guides/platform/backups#point-in-time-recovery)
...
curl -X POST "https://api.zuvodev.com/v1/projects/$PROJECT_REF/read-replicas/setup" \
...
curl -X POST "https://api.zuvodev.com/v1/projects/$PROJECT_REF/read-replicas/remove" \
  -H "Authorization: Bearer $SUPABASE_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
![Map view of Primary and Read Replica databases](/img/guides/platform/read-replicas/streaming-replication-dark.png?v=1)
...
1. [Project upgrades](/guides/platform/upgrading)
2. [Data restorations](/guides/platform/backups#pitr-restoration-process)
...
If you are already ingesting your [project's metrics](/guides/monitoring-and-debugging/metrics) into your own environment, you can also keep track of replication lag and set alarms with the `physical_replication_lag_physical_replica_lag_seconds` metric.
...
It is safe to drop this failed Read Replica, and in the event of a transient issue, attempt to spin up another one. If spinning up Read Replicas for your project consistently fails, check the[status page](https://status.zuvodev.com) for any ongoing incidents, or [open a support ticket](/dashboard/support/new). To aid the investigation, do not bring down the recently failed Read Replica.
