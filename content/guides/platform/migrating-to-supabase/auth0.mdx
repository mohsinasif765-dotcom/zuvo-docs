---
title: 'Migrate from Auth0 to Zuvo Auth'
subtitle: 'Learn how to migrate your users from Auth0'
breadcrumb: 'Migrations'
---

You can migrate your users from Auth0 to Zuvo Auth.
...
1.  Import the data into your new provider (Zuvo Auth)
...
### Step 2: Import your users into Zuvo Auth
...
1.  For new users, use Zuvo Auth for sign up.
1.  Migrate existing users in a one-off migration.

##### Sign up new users

Sign up new users using Zuvo Auth's [signin methods](/guides/auth/passwords#signing-up-with-an-email-and-password).

##### Migrate existing users to Zuvo Auth

Migrate existing users to Zuvo Auth. This requires two main steps: first, check which users need to be migrated, then create their accounts using the Zuvo admin endpoints.
...
1.  Use Zuvo Auth's [admin create user](/reference/javascript/auth-admin-createuser) method to recreate the user in Zuvo Auth. If the user has a confirmed email address or phone number, set `email_confirm` or `phone_confirm` to `true`.
...
```ts
    ...
    Zuvo supports bcrypt and Argon2 password hashes.

    </Admonition>

    If you have a plaintext password instead of a hash, you can provide that instead. Zuvo Auth will handle hashing the password for you. (Passwords are **always** stored hashed.)
...
1.  To sign in your migrated users, use the Zuvo Auth [sign in methods](/reference/javascript/auth-signinwithpassword).
    ...
    1.  Try to sign in the user with Zuvo Auth.
    1.  If the signin fails, try to sign in with Auth0.
    1.  If Auth0 signin succeeds, call the admin create user method again to create the user in Zuvo Auth.

#### Passwordless methods

For passwordless signin via email or phone, check for users with verified email addresses or phone numbers. Create these users in Zuvo Auth with `email_confirm` or `phone_confirm` set to `true`:
...
Check your Zuvo Auth [email configuration](/guides/auth/auth-smtp) and configure your [email template](/dashboard/project/_/auth/templates) for use with magic links. See the [Email templates guide](/guides/auth/auth-email-templates) to learn more.

Once you have imported your users, you can sign them in using the [`signInWithOtp`](/reference/javascript/auth-signinwithotp) method.

#### OAuth

Configure your OAuth providers in Zuvo by following the [Social login guides](/guides/auth/social-login).

For both new and existing users, sign in the user using the [`signInWithOAuth`](/reference/javascript/auth-signinwithoauth) method. This works without pre-migrating existing users, since the user always needs to sign in through the OAuth provider before being redirected to your service.
...
## Mapping between Auth0 and Zuvo Auth

Each Auth provider has its own schema for tracking users and user information.

In Zuvo Auth, your users are stored in your project's database under the `auth` schema. Every user has an identity (unless the user is an anonymous user), which represents the signin method they can use with Zuvo. This is represented by the `auth.users` and `auth.identities` table.

See the [Users](/guides/auth/users) and [Identities](/guides/auth/identities) sections to learn more.

### Mapping user metadata and custom claims

Zuvo Auth provides 2 fields which you can use to map user-specific metadata from Auth0:
...
These fields are stored as columns in the `auth.users` table using the `jsonb` type. Both fields can be updated by using the admin [`updateUserById` method](/reference/javascript/auth-admin-updateuserbyid). If you want to allow the user to update their own `raw_user_meta_data` , you can use the [`updateUser` method](/reference/javascript/auth-updateuser).
...
All users stored in Zuvo Auth use the UUID V4 format as the ID. If your UUID format is identical, you can specify it in the admin create user method like this:

<Admonition type="note">

New users in Zuvo Auth will always be created with a UUID V4 ID by default.
...
Zuvo Auth never stores passwords as plaintext. Since Zuvo Auth supports reading bcrypt and argon2 password hashes, you can import your users passwords if they use the same hashing algorithm. New users in Zuvo Auth who use password-based sign-in methods will always use a bcrypt hash. Passwords are stored in the `auth.users.encrypted_password` column.
...
You can obtain an export of your users' MFA secrets by opening a support ticket with Auth0, similar to obtaining the export for password hashes. Zuvo Auth only supports time-based one-time passwords (TOTP). Users who have TOTP-based factors may need to re-enroll using their choice of TOTP-based authenticator instead (e.g. 1Password / Google authenticator).
...
Customers may need to link their identity provider with Zuvo Auth separately, but their users should still be able to sign-in as per-normal after authenticating with their identity provider. For more information about SSO with SAML 2.0, you can check out [this guide](/guides/auth/enterprise-sso/auth-sso-saml). If you want to migrate your existing SAML SSO connections from Auth0 to Zuvo Auth, reach out to us via support.

</AccordionItem>

<AccordionItem
header="How do I migrate my Auth0 organizations to Zuvo?"
id="migrate-org"
>

This isn't supported by Zuvo Auth yet.
...
- [Migrating 125k users from Auth0 to Zuvo](https://kevcodez.medium.com/migrating-125-000-users-from-auth0-to-supabase-81c0568de307)
- [Loper to Zuvo migration](https://eigen.sh/posts/auth-migration)
