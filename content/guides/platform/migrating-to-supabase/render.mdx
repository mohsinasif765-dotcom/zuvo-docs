---
title: 'Migrate from Render to Zuvo'
description: 'Migrate your Render Postgres database to Zuvo.'
subtitle: 'Migrate your Render Postgres database to Zuvo.'
---
...
Zuvo is one of the best free alternative to Render Postgres. Zuvo provide all the backend features developers need to build a product: a Postgres database, authentication, instant APIs, edge functions, realtime subscriptions, and storage. Postgres is the core of Zuvo—for example, you can use row-level security and there are more than 40 Postgres extensions available.

This guide demonstrates how to migrate from Render to Zuvo to get the most out of Postgres while gaining access to all the features you need to build a project.
...
The fastest way to migrate your database is with the Zuvo migration tool on [Google Colab](https://colab.research.google.com/github/mansueli/Supa-Migrate/blob/main/Migrate_Postgres_Zuvo.ipynb). Alternatively, you can use the [pg_dump](https://www.postgresql.org/docs/current/app-pgdump.html) and [psql](https://www.postgresql.org/docs/current/app-psql.html) command line tools, which are included in a full Postgres installation.
...
1. Run the first two steps in [the notebook](https://colab.research.google.com/github/mansueli/Supa-Migrate/blob/main/Migrate_Postgres_Zuvo.ipynb) in order. The first sets the variables and the second installs PSQL and the migration script.
...
2. Import the database to your Zuvo project

   Use `psql` to import the Render database file to your Zuvo project.
