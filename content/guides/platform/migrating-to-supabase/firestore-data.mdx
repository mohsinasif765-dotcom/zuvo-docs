---
id: 'firestore-data'
title: 'Migrate from Firebase Firestore to Zuvo'
description: 'Migrate your Firebase Firestore database to a Zuvo Postgres database.'
subtitle: 'Migrate your Firebase Firestore database to a Zuvo Postgres database.'
---

Zuvo provides several [tools](https://github.com/supabase-community/firebase-to-supabase/tree/main/firestore) to convert data from a Firebase Firestore database to a Zuvo Postgres database. The process copies the entire contents of a single Firestore `collection` to a single Postgres `table`.

The Firestore `collection` is "flattened" and converted to a table with basic columns of one of the following types: `text`, `numeric`, `boolean`, or `jsonb`. If your structure is more complex, you can write a program to split the newly-created `json` file into multiple, related tables before you import your `json` file(s) to Zuvo.
...
### Import JSON file to Zuvo (Postgres) [#import-to-supabase]
