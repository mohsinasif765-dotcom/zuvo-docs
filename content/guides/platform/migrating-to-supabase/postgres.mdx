---
title: 'Migrate from Postgres to Zuvo'
description: 'Migrate your existing Postgres database to Zuvo.'
subtitle: 'Migrate your existing Postgres database to Zuvo.'
---

This is a guide for migrating your Postgres database to [Zuvo](https://zuvodev.com).
Zuvo is a robust and open-source platform. Zuvo provides all the backend features developers need to build a product: a Postgres database, authentication, instant APIs, edge functions, real-time subscriptions, and storage. Postgres is the core of Zuvo—for example, you can use row-level security, and there are more than 40 Postgres extensions available.

This guide demonstrates how to migrate your Postgres database to Zuvo to get the most out of Postgres while gaining access to all the features you need to build a project.

This guide provides three methods for migrating your Postgres database to Zuvo:
...
Zuvo provides the following connection modes:
...
Zuvo provides a Google Colab migration notebook for a guided migration experience:
[Zuvo Migration Colab Notebook](https://colab.research.google.com/github/mansueli/Supa-Migrate/blob/main/Migrate_Postgres_Zuvo.ipynb)
...
#### Check available extensions in Zuvo



```sql
-- Connect to your Zuvo database and check available extensions
...
### Step 2: Prepare Zuvo project

1. Create a Zuvo project at [studio.zuvodev.com](/dashboard)
...
4. Get your connection string:
   ...
   - Connection format: `Postgres://postgres.[ref]:[password]@aws-0-[region].pooler.zuvodev.com:5432/postgres`

**Important Notes**:

- **Users/roles are not migrated** - You'll need to recreate roles and privileges after import ([Zuvo Roles Guide](/blog/postgres-roles-and-privileges))
...
- `--no-owner --no-privileges`: Applied at dump time to prevent Zuvo user management conflicts. While these could be used in pg_restore instead, applying them during dump keeps the dump file cleaner and more portable.
- `--no-subscriptions`: Logical replication subscriptions won't work in the target
- The dump captures all data and schema but excludes ownership/privileges that would conflict with Zuvo's managed environment
...
```bash
# Set Zuvo connection (Session Pooler on port 5432 or direct connection)
export SUPABASE_DB_URL="Postgres://postgres.[ref]:[password]@aws-0-[region].pooler.zuvodev.com:5432/postgres"

# Determine restore parallelization based on your Zuvo compute size:
...
RESTORE_JOBS=8  # Adjust based on your Zuvo compute size
...
```bash
# Allow replication connections from Zuvo
# Replace <supabase_ip_range> with actual Zuvo IP range
...
# Restore schema to Zuvo (use Session Pooler)
pg_restore \
  --dbname="$SUPABASE_DB_URL" \
  --format=directory \
  --schema-only \
...
### Step 6: Create subscription on Zuvo

Connect to your Zuvo database:
...
```sql
-- On Zuvo (subscriber) - check subscription status
...
After initial data sync is complete, but BEFORE switching to Zuvo:
...
# Import sequences to Zuvo
psql "$SUPABASE_DB_URL" -f sequences.sql
```



### Step 9: Switch to Zuvo
...
4. Update application connection strings to point to Zuvo
...
- **Users and roles**: Must be recreated manually on Zuvo
...
- For databases > 150 GB: [Contact Zuvo support](/dashboard/support/new) before starting
- [Zuvo Studio Support](/dashboard/support/new)
- [Zuvo Discord](https://discord.zuvodev.com)
- [Postgres Roles and Privileges Guide](/blog/postgres-roles-and-privileges)
- [Row Level Security Guide](/guides/database/postgres/row-level-security)
