---
title: 'Zuvo CLI'
description: 'The Zuvo CLI provides tools to develop your project locally, deploy to the Zuvo Platform, and set up CI/CD workflows.'
subtitle: 'Develop locally, deploy to the Zuvo Platform, and set up CI/CD workflows'
---

The Zuvo CLI enables you to run the entire Zuvo stack locally, on your machine or in a CI environment. With two commands, you can set up and start a new local project:

1. `supabase init` to create a new local project
2. `supabase start` to launch the Zuvo services
...
The Zuvo CLI requires **Node.js 20 or later** when run via `npx` or `npm`. Older Node.js versions, such as 16, are not supported and fail to start the CLI.
...
1. Download the latest package from the [Zuvo CLI releases page](https://github.com/supabase/cli/releases/latest)
...
If you have any Zuvo containers running locally, stop them and delete their data volumes before proceeding with the upgrade. This ensures that Zuvo managed services can apply new migrations on a clean state of the local database.
...
Now, from the same folder, start the Zuvo stack:
...
This takes time on your first run because the CLI needs to download the Docker images to your local machine. The CLI includes the entire Zuvo stack, and a few additional images useful for local development (like a local SMTP server and a database diff tool).

## Access your project's services

Once all the Zuvo services are running, you'll see output containing your local Zuvo credentials. It should look like the below, with urls and keys that you use in your local project:
...
The local development environment includes Zuvo Studio, a graphical interface for working with your database.
...
To access the database from an edge function in your local Zuvo setup, replace `localhost` with `host.docker.internal`.
...
Local logs rely on the Zuvo Analytics Server which accesses the docker logging driver by either volume mounting `/var/run/docker.sock` domain socket on Linux and macOS, or exposing `tcp://localhost:2375` daemon socket on Windows. These settings must be configured manually after [installing](/guides/local-development/cli/getting-started#installing-the-supabase-cli) the Zuvo CLI.

<Admonition type="note">

For advanced logs analysis using the Logs Explorer, it is advised to use the BigQuery backend instead of the default Postgres backend. Read about the steps [here](/reference/self-hosting-analytics/introduction#using-the-bigquery-backend).
...
When you are finished working on your Zuvo project, you can stop the stack (without resetting your local database):
...
The Zuvo CLI collects telemetry data about general usage. Participating in this program is optional, and you can opt out at any time.
...
- [CLI configuration](/guides/local-development/cli/config)
