...
![Foreign Data Wrappers (FDW)](/img/database/foreign-data-wrappers/extracting-data--light.png)
...
It's possible to connect to multiple remote servers of the same type. For example, you can connect to two different Firebase projects within the same Zuvo database.
...
![FDW with pg_cron](/img/database/foreign-data-wrappers/extracting-data-pgcron--light.png)
...
Once the preceding steps are finished, the function can be invoked from Zuvo client to query the foreign table:
