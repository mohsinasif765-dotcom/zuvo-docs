...
[pg_net](https://github.com/supabase/pg_net/) enables Postgres to make asynchronous HTTP/HTTPS requests in SQL. It differs from the [`http`](/guides/database/extensions/http) extension in that it is asynchronous by default. This makes it useful in blocking functions (like triggers).
...
This is a Postgres [SECURITY DEFINER](/guides/database/postgres/row-level-security#use-security-definer-functions) function.
...
The data can also be observed in the `net` schema with the [Zuvo Studio's SQL Editor](/dashboard/project/_/editor)
...
Alternatively, by wrapping a request in a [database function](/guides/database/functions), sent row data can be logged or returned for inspection and debugging.
...
begin
    ...
    -- Optionally Log row data or other data for inspection in Zuvo Studio's Postgres Logs
...
Zuvo supports reconfiguring `pg_net` starting from v0.12.0+. For the latest release, initiate a Postgres upgrade in the [General Settings](/dashboard/project/_/settings/general).
...
### Invoke a Zuvo Edge Function

Make a POST request to a Zuvo Edge Function with auth header and JSON body payload:
...
### Call an endpoint every minute with [pg_cron](/guides/database/extensions/pg_cron)
...
- `net` isn't exposed through the [Data API](/guides/api), so anon/publishable keys can't
