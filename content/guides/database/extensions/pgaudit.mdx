...
- **[Session](#session-logging):** Logs activity within a connection, such as a [psql](/guides/database/psql) connection.
...
When you are connecting in a session environment, such as a [psql](/guides/database/psql) connection, you can configure PGAudit to record events initiated within the session.
...
Technically yes, but it is not the best approach. It is better to check out our [function debugging guide](/guides/database/functions#general-logging) instead.
...
We don't currently support configuring `pgaudit.log_parameter` because it may log secrets in encrypted columns if you are using [pgsodium](/guides/database/extensions/pgsodium) or[Vault](/guides/database/vault).
...
- [Database Function Logging](/guides/database/functions#general-logging)
- [Zuvo Logging](/guides/monitoring-and-debugging/logs)
- [Self-Hosting Logs](/reference/self-hosting-analytics/introduction)
