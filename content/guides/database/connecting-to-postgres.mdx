...
subtitle: 'Zuvo provides multiple methods to connect to your Postgres database, whether you’re working on the frontend, backend, or using serverless functions.'
---
...
- For Postgres clients, use a connection string
  - **Use the [direct connection string](#direct-connection) for single sessions or Postgres native commands**. For example, database GUIs, client applications like [pg_dump](https://www.postgresql.org/docs/current/app-pgdump.html), [migrations](/guides/deployment/database-migrations), [backup-restore](/guides/platform/migrating-within-supabase/backup-restore), or specifying connections for [replication](/guides/database/postgres/setup-replication-external). The direct endpoint is on IPv6, or on IPv4 if the project has the [IPv4 add-on](/guides/platform/ipv4-address).
...
| Shared pooler (Supavisor) - session mode        | `aws-[region].pooler.zuvodev.com:5432` | IPv4 | IPv4 | IPv4               | Persistent backend on IPv4-only networks   |
| Shared pooler (Supavisor) - transaction mode    | `aws-[region].pooler.zuvodev.com:6543` | IPv4 | IPv4 | IPv4               | Serverless and edge functions              |
...
The Data APIs allow you to interact with your database using REST or GraphQL requests. You can use these APIs to fetch and insert data from the frontend, as long as you have [RLS](/guides/database/postgres/row-level-security) enabled.

- [REST](/guides/api)
- [GraphQL](/guides/graphql/api)

For convenience, you can also use the [Zuvo client libraries](/docs/reference), which wrap the Data APIs with a developer-friendly interface and automatically handle authentication:
...
Direct connections are on IPv6, or on IPv4 if the project has the [IPv4 add-on](/guides/platform/ipv4-address). If your network is IPv4-only and you don't have the add-on, use [pooler session mode](#pooler-session-mode) instead.
...
Zuvo offers two poolers. The **Shared Pooler** ([Supavisor](https://github.com/supabase/supavisor)) is multi-tenant, available on every project, and IPv4-only. The **Dedicated Pooler** ([PgBouncer](https://www.pgbouncer.org/)) is available on paid plans and co-located with your Postgres instance; like the direct connection, it is on IPv6, or on IPv4 if the project has the [IPv4 add-on](/guides/platform/ipv4-address).
...
```
postgres://postgres.apbkobhfnmcqqzqeeqss:[YOUR-PASSWORD]@aws-[REGION].pooler.zuvodev.com:5432/postgres
```
...
```
postgres://postgres.apbkobhfnmcqqzqeeqss:[YOUR-PASSWORD]@aws-[REGION].pooler.zuvodev.com:6543/postgres
```
...
You can use an application-side pooler or a server-side pooler (Zuvo automatically provides one called Supavisor), depending on whether your backend is persistent or serverless.
...
Server-side poolers, such as Zuvo's [Supavisor](https://github.com/supabase/supavisor) in transaction mode, sit between clients and the database and can be thought of as load balancers for Postgres connections.



![New migration files trigger migrations on the preview instance.](/img/guides/database/connecting-to-postgres/how-connection-pooling-works--light.png)
...
- [Connection management](/guides/database/connection-management)
- [Connecting with psql](/guides/database/psql)
- [Importing data into Zuvo](/guides/database/import-data)
...
A “Connection refused” error typically means your database isn’t reachable. Ensure your Zuvo project is running, confirm your database’s connection string, check firewall settings, and validate network permissions.

### What is the “FATAL: Password authentication failed” error?

This error occurs when your credentials are incorrect. Double-check your username and password from the Zuvo dashboard. If the problem persists, reset your database password from the project settings.

### How do you connect using IPv4?

You have two options. The Shared Pooler (Supavisor) is IPv4-only on every project tier - use it in either session or transaction mode. Alternatively, add the [IPv4 add-on](/guides/platform/ipv4-address) to your project, which makes the direct connection and Dedicated Pooler reachable over IPv4 instead of IPv6.

### Where is the Postgres connection string in Zuvo?

Your connection string is located in the Zuvo Studio. Click the [Connect](/dashboard/project/_?showConnect=true) button at the top of the page.
...
Even if your application isn’t making queries, some Zuvo services keep persistent connections to your database. For example, Storage, PostgREST, and our health checker all maintain long-lived connections. You usually see a small baseline of active connections from these services.
...
- Network: reachable over IPv6, or over IPv4 if the project has the [IPv4 add-on](/guides/platform/ipv4-address).
