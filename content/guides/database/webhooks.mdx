...
When using Database Webhooks on your local Zuvo instance, you need to be aware that the Postgres database runs inside a Docker container. This means that `localhost` or `127.0.0.1` in your webhook URL will refer to the container itself, not your host machine where your application is running.
...
- [pg_net](/guides/database/extensions/pg_net): an async networking extension for Postgres
