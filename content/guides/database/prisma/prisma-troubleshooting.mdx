...
This guide addresses common Prisma errors that you might encounter while using Zuvo.
...
- **Check database health**: Use the [Observability Dashboard](/dashboard/project/_/observability/database) to monitor CPU, memory, and I/O usage. If the database is overloaded, consider increasing your [compute size](/guides/platform/compute-and-disk) or [optimizing your queries](/guides/database/query-optimization).
...
- **Optimize queries**: [Improve the efficiency of your queries](/guides/database/query-optimization) to reduce execution time.
...
- **Minimize database strain**:Check the Reports Page for database strain. If there is obvious strain, consider [optimizing](/guides/database/query-optimization) or increasing compute size
...
- **Zuvo Managed Schemas**: Zuvo may update managed schemas like auth and storage to introduce new features. Granting Prisma access to these schemas can lead to drift during updates.
...
- **Transaction Mode for serverless apps**: If you are using serverless functions (Zuvo Edge, Vercel, AWS Lambda), switch to transaction mode (port 6543). It handles more connections than session mode or direct connections.
...
- **Decrease query time**: Reduce query complexity or add [strategic indexes](/guides/database/postgres/indexes) to your tables to speed up queries.
...
- Multi-schema support: If the external schema isn't Zuvo managed, list the relevant schemas on the `datasource` block in your `schema.prisma` file.
...
- Zuvo managed schemas: Schemas managed by Zuvo, such as `auth` and `storage`, may be changed to support new features. Referencing these schemas directly will cause schema drift in the future. It is best to remove references to these schemas from your migrations.
