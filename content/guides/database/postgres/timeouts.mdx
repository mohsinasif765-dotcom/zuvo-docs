...
Dashboard and [Client](/guides/api/rest/client-libs) queries have a max-configurable timeout of 60 seconds. For longer transactions, use [Supavisor or direct connections](/guides/database/connecting-to-postgres#quick-summary).
...
Because it applies to sessions only, it can only be used with connections through Supavisor in session mode (port 5432) or a direct connection. It cannot be used in the Dashboard, with the Zuvo Client API, nor with Supavisor in Transaction mode (port 6543).
...
This works with the Database REST API when called from the Zuvo client libraries:
...
If you are changing the timeout for the Zuvo Client API calls, you will need to reload PostgREST to reflect the timeout changes by running the following script:
...
The Zuvo Studio contains tools to help you identify timed-out and long-running queries.
