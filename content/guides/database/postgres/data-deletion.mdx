...
- Drop dependent objects explicitly, use [CASCADE](/guides/database/postgres/cascade-deletes) with caution
- Choose a low traffic time to run the operation
- Run operations inside a [migration](/guides/deployment/database-migrations)
...
If you know in advance that such large deletes will have to happen in the business cycle of your database, then you should seriously think about using [table partitioning](/guides/database/partitions) as a management tool.
...
Combine soft deletes with a scheduled hard-delete job (using [pg_cron](/guides/database/extensions/pg_cron)) to permanently remove old soft-deleted rows in batches during low-traffic periods.
...
The `inspect` command in the [Zuvo CLI](/reference/cli/supabase-inspect-db-index-stats) can help you identify unused indexes:
...
The [Zuvo CLI](/reference/cli/supabase-inspect-db-locks) provides commands to view these metrics:
...
The most efficient way to reclaim disk space, without locks, is to use [pg_repack](/guides/database/extensions/pg_repack).

## Related links

- [Safe Cascading Deletes](/guides/database/postgres/cascade-deletes)
- [Inspecting your Database](/guides/database/inspect)
- [Understanding Database and Disk Size](/guides/platform/database-size)
