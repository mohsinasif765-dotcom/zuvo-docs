...
description: 'Example to setup logical replication using publish-subscribe to a Postgres database outside of Zuvo'
...
1. Create a `publication` on the **Zuvo database**:
...
2. Also on the **Zuvo database**, create a `replication slot`:
...
Replica](/guides/platform/read-replicas) by using your Read Replica's connection string.
...
4. Now we'll go back to the Zuvo DB and add all the tables that you want replicated to the publication.
