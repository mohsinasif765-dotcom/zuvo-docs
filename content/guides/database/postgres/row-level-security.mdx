...
Not every project grants these automatically. See [Default privileges](/guides/api/securing-your-api#default-privileges). Grant each role only the operations it needs.
...
Zuvo maps every request to one of the roles:

- `anon`: an unauthenticated request (the user is not logged in)
- `authenticated`: an authenticated request (the user is logged in)

These are [Postgres Roles](/guides/database/postgres/roles). You can use these roles within your Policies using the `TO` clause:
...
Using the `anon` Postgres role is different from an [anonymous user](/guides/auth/auth-anonymous) in Zuvo Auth. An anonymous user assumes the `authenticated` role to access the database and can be differentiated from a permanent user by checking the `is_anonymous` claim in the JWT.
...
Run these statements in the [SQL Editor](/dashboard/project/_/sql/new) for a one-off change, or in a [migration](/guides/deployment/database-migrations) to keep the change reproducible across environments. Grants and RLS belong in the same migration.
...
```sql
   ...
   Once RLS is enabled, no data is accessible through the [API](/guides/api) when using a publishable key, until you create policies.
...
If new tables still receive automatic grants, see [Revoke default privileges](/guides/api/securing-your-api#revoke-default-privileges). To enable RLS automatically on every new table, see [Event triggers](/guides/database/postgres/event-triggers).
...
Zuvo provides [helper functions](#helper-functions) that simplify RLS if you are using Zuvo Auth. `auth.uid()` returns the ID of the user making the request.
...
The files above live under `supabase/tests/` and run through [pgTAP](/guides/database/extensions/pgtap). Create them with `supabase test new <table>_rls.test`, and run them with `supabase test db`.

[`supabase-test-helpers`](https://github.com/usebasejump/supabase-test-helpers/tree/main) adds `tests.create_supabase_user()`, `tests.authenticate_as()`, and `tests.rls_enabled()`. See [Advanced pgTAP testing](/guides/local-development/testing/pgtap-extended) and [Testing your database](/guides/database/testing).

### Add indexes

Add an [index](/guides/database/postgres/indexes) on every column your policies filter on. Postgres evaluates the policy against each candidate row, so an unindexed filter column turns a read into a sequential scan. For a policy like this:
...
Indexing the filter columns and wrapping helper calls keeps policies fast as a table grows. For the measured impact, and for tuning beyond these two rules, see [Row Level Security performance](/guides/database/postgres/row-level-security-performance).
...
Zuvo provides some helper functions that make it easier to write policies.
...
The `auth.jwt()` function can be used to check for [Multi-Factor Authentication](/guides/auth/auth-mfa#enforce-rules-for-mfa-logins). For example, you could restrict a user from updating their profile unless they have at least 2 levels of authentication (Assurance Level 2):
...
This works because the function runs as its owner, and a `security definer` function only skips RLS when its owner can. On Zuvo the owner is `postgres`, which has `bypassrls`. A function owned by a role without `bypassrls`, or reading a table set to `force row level security`, evaluates the membership policy again and stays recursive.

### Bypassing Row Level Security

Use a [secret key](/guides/getting-started/api-keys) for administrative tasks that need to bypass RLS. A secret key authorizes access through the `service_role` Postgres role, which has the `bypassrls` attribute. Never use a secret key in the browser or expose it to customers.
...
You can also create new [Postgres Roles](/guides/database/postgres/roles) which can bypass Row Level Security using the "bypass RLS" privilege:
...
- [Row Level Security performance](/guides/database/postgres/row-level-security-performance): diagnose whether policies are your bottleneck, and tune ones that are already correct.
- [Advanced pgTAP testing](/guides/local-development/testing/pgtap-extended): schema-wide RLS test helpers and a worked multi-tenant example.
- [Testing your database](/guides/database/testing): the CLI test workflow that `supabase test db` runs.
- [Securing your API](/guides/api/securing-your-api): grants, dedicated schemas, and pre-request checks around the Data API.
- [Column Level Security](/guides/database/postgres/column-level-security): restrict access to individual columns.
