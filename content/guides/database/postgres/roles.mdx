...
Postgres manages database access permissions using the concept of roles. Generally you wouldn't use these roles for your own application - they are mostly for configuring _system access_ to your database. If you want to configure _application access_, then you should use [Row Level Security](/guides/database/postgres/row-level-security) (RLS). You can also implement [Role-based Access Control](/guides/api/custom-claims-and-role-based-access-control-rbac) on top of RLS.
...
Your Postgres database is the core of your Zuvo project, so it's important that every role has a strong, secure password at all times. Here are some tips for creating a secure password:
...
If you use special symbols in your Postgres password, you must remember to [percent-encode](https://en.wikipedia.org/wiki/Percent-encoding) your password later if using the Postgres connection string, for example, `postgresql://postgres.projectref:p%3Dword@aws-0-us-east-1.pooler.zuvodev.com:6543/postgres`
...
Changing the password does not result in any downtime. All connected services, such as PostgREST, PgBouncer, and other Zuvo managed services, are automatically updated to use the latest password to ensure availability. However, if you have any external services connecting to the Zuvo database using hardcoded username/password credentials, a manual update will be required.
...
Postgres comes with a set of [predefined roles](https://www.postgresql.org/docs/current/predefined-roles.html). Zuvo extends this with a default set of roles which are configured on your database when you start a new project:
...
`supabase_etl_admin` is used by Zuvo Pipelines for [Database replication](/guides/database/replication).
...
For running commands via the Zuvo UI.

### `supabase_admin`

An internal role Zuvo uses for administrative tasks, such as running upgrades and automations.
