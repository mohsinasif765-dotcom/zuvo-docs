...
Measure the cost of Row Level Security (RLS) and tune policies that are already correct. To learn how to write correct policies, see [Row Level Security](/guides/database/postgres/row-level-security).
...
- [Index the columns your policies filter on](/guides/database/postgres/row-level-security#add-indexes)
- [Call functions with `select`](/guides/database/postgres/row-level-security#call-functions-with-select)
- [Specify roles in your policies](/guides/database/postgres/row-level-security#specify-roles-in-your-policies)
...
PostgREST can return the query plan to a Zuvo client. Enable it first:
...
You can also use a [security definer function](/guides/database/postgres/row-level-security#use-security-definer-functions) to bypass RLS on the join table.
...
- [Managing indexes in Postgres](/guides/database/postgres/indexes)
- [Query optimization](/guides/database/query-optimization)
