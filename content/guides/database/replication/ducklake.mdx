...
description: 'Configure DuckLake as a Zuvo Pipelines destination.'
subtitle: 'Replicate Zuvo Postgres tables to DuckLake.'
...
[DuckLake](https://ducklake.select/) is an open lakehouse format that stores metadata in a SQL catalog and table data in object storage. Zuvo Pipelines keeps current-state DuckLake tables synchronized with published Postgres tables.
...
Use this mode to back the DuckLake with Zuvo projects. Pipelines provisions its own catalog and object-storage credentials when you create the destination.
...
Validation can show warnings that Zuvo catalog and Storage credentials are provisioned only when the destination is saved. This is expected for **Use Zuvo** mode. Review the selected projects and proceed to create the destination.
...
- **S3 URL style**: `path` for Zuvo Storage and many S3-compatible providers, or `vhost` for virtual-host-style addressing
...
For **Use Zuvo** mode, create separate read credentials for the selected catalog and Storage projects. The writer credentials generated for Pipelines aren't exposed. For **Custom parameters**, use separate read-only credentials when your catalog and storage provider support them.
...
See the [DuckDB connection guide](https://ducklake.select/docs/stable/duckdb/usage/connecting) for the current `ducklake` extension and `ATTACH` syntax. If your query client can't read a Zuvo-backed catalog during Early Access, [contact support](/dashboard/support/new).
...
| Zuvo mode shows credential-provisioning warnings   | This is expected before creation. Confirm your selected projects, bucket, and metadata schema, then proceed. Credentials are provisioned when the destination is saved.                             |
