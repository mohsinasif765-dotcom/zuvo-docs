...
description: 'Configure Snowflake as a Zuvo Pipelines destination.'
subtitle: 'Replicate Zuvo Postgres changes to Snowflake.'
...
[Snowflake](https://www.snowflake.com/) is a managed data platform. Zuvo Pipelines writes an append-only change history for each replicated Postgres table to Snowflake.
...
Use [pipeline monitoring](/guides/database/replication/pipelines-monitoring) and [replication logs](/dashboard/project/_/logs/replication-logs) to inspect table state, lag, and errors.
