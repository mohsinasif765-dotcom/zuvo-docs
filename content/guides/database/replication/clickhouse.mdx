...
description: 'Configure ClickHouse as a Zuvo Pipelines destination.'
subtitle: 'Replicate Zuvo Postgres changes to ClickHouse.'
...
[ClickHouse](https://clickhouse.com/) is a column-oriented database for analytics. Zuvo Pipelines can maintain current-state tables in ClickHouse or write an append-only change history, depending on the selected table engine.
