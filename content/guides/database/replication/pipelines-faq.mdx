...
id: 'pipelines-faq'
title: 'Pipelines FAQ'
description: 'Frequently asked questions about Zuvo Pipelines.'
subtitle: 'Common questions and answers about Zuvo Pipelines.'
sidebar_label: 'FAQ'
---
...
**BigQuery** is currently available as a managed destination. **ClickHouse**, **DuckLake**, and **Snowflake** are in Early Access. See the [ClickHouse](/guides/database/replication/clickhouse), [DuckLake](/guides/database/replication/ducklake), and [Snowflake](/guides/database/replication/snowflake) destination guides for setup instructions and data-model details.


[Request Early Access](/go/supabase-pipelines-new-destinations) for ClickHouse, DuckLake, or Snowflake. Supported destinations can be Zuvo-managed or third-party systems as support expands.
...
When you enable Pipelines, Zuvo installs database objects that help track replication state and support schema changes:

- An event trigger that runs on every `ALTER TABLE` statement. Zuvo uses this to support schema change handling.
...
Initial BigQuery table creation preserves whether each scalar, non-array Postgres column allows `NULL`. Arrays use BigQuery's `REPEATED` mode. For later changes, `DROP NOT NULL` relaxes a BigQuery `REQUIRED` column to `NULLABLE`, while `SET NOT NULL` leaves an existing BigQuery column nullable and logs a warning. Newly added scalar, top-level columns are always nullable in BigQuery. Supported defaults are applied separately as destination metadata and do not populate existing destination rows. Pipelines does not currently support changing column data types. See [BigQuery schema change support](/guides/database/replication/bigquery#schema-change-support) for details.

Snowflake Early Access supports adding, renaming, and dropping columns. It doesn't support column type changes or table and schema renames. Changes to whether existing columns accept `NULL` and changes to their defaults are ignored. Schema changes affect existing history. Dropping a column removes it from older events. See [Snowflake schema change support](/guides/database/replication/snowflake#schema-change-support) for details.

ClickHouse Early Access supports adding, renaming, and dropping columns, dropping `NOT NULL`, and supported default changes. It doesn't apply column type changes or `SET NOT NULL`. Changing a source primary-key column isn't supported for `ReplacingMergeTree`. ClickHouse DDL is not transactional, so an interrupted multi-column change can be partially applied. See [ClickHouse schema change support](/guides/database/replication/clickhouse#schema-change-support) for details.

DuckLake Early Access supports adding, renaming, and dropping columns, dropping `NOT NULL`, and supported default changes. It doesn't apply column type changes or `SET NOT NULL`. New columns are made nullable, and each multi-column schema change is applied in a transaction. See [DuckLake schema change support](/guides/database/replication/ducklake#schema-change-support) for details.
...
To remove Pipelines from a project, delete all Pipelines destinations first. After all destinations are deleted, the disable action becomes available. For the Dashboard steps, see [Disabling Pipelines](/guides/database/replication/pipelines#disabling-pipelines).

Disabling Pipelines stops Zuvo from managing replication for the project. It does not delete tables or data that were already written to your destination.
...
See [Partitioned tables](/guides/database/replication/pipelines#partitioned-tables) for examples and the full behavior.
...
Full replica identity increases WAL volume and only affects new WAL records. Fix the setting before generating more changes. If the failing update is already retained in WAL, restarting may fail again; recreate the pipeline or restart the affected table's initial sync. See the [BigQuery](/guides/database/replication/bigquery#source-table-requirements), [ClickHouse](/guides/database/replication/clickhouse#source-table-requirements), [DuckLake](/guides/database/replication/ducklake#source-table-requirements), or [Snowflake](/guides/database/replication/snowflake#source-table-requirements) requirements for details.

## Why aren't publication changes reflected after adding or removing tables?

After modifying your Postgres publication, you must restart the replication pipeline for changes to take effect. See [Adding or removing tables](/guides/database/replication/pipelines#adding-or-removing-tables) for instructions.
...
See [Handling errors](/guides/database/replication/pipelines-monitoring#handling-errors) for more details.
...
If the same error continues after restart, the pipeline may stop again. Review [Monitor pipeline status](/guides/database/replication/pipelines-monitoring) for troubleshooting steps.
...
Open [**Database > Replication**](/dashboard/project/_/database/replication), click **View pipeline**, and check **Waiting to sync**, **WAL retention remaining**, **Last check-in**, **Connected**, and **Slot status**. See [Dealing with replication lag](/guides/database/replication/pipelines-monitoring#dealing-with-replication-lag) for the full investigation and response flow.
...
See [Slot statuses](/guides/database/replication/pipelines-monitoring#slot-statuses) for all slot states and what to do next.
...
See [Monitor pipeline status](/guides/database/replication/pipelines-monitoring) for comprehensive monitoring instructions.

## How to stop replication

You can manage your pipeline using the actions menu in the destinations list. See [Managing your pipeline](/guides/database/replication/pipelines#managing-your-pipeline) for details on available actions.
...
1. Check [Set up Pipelines](/guides/database/replication/pipelines) and [Monitor pipeline status](/guides/database/replication/pipelines-monitoring)
