...
description: 'Monitor the status and health of pipelines created with Zuvo Pipelines.'
...
After setting up Zuvo Pipelines, you can monitor the status and health of your pipelines directly from the Dashboard. A pipeline first performs an initial sync of existing rows, then uses ongoing replication (CDC) to send subsequent database changes to your destination.
...
![Running pipeline status page showing replication metrics and a live table](/img/database/replication/pipelines-metrics.png)
...
- For BigQuery, verify that service account permissions, table requirements, and replica identity settings match the [BigQuery destination guide](/guides/database/replication/bigquery).
- For ClickHouse, check the HTTPS endpoint, target-database permissions, selected table engine, server version, and source table requirements against the [ClickHouse destination guide](/guides/database/replication/clickhouse).
- For DuckLake, check catalog connectivity, object-storage permissions, catalog and storage latency, and source row identity against the [DuckLake destination guide](/guides/database/replication/ducklake).
- For Snowflake, check the service user's default role, schema privileges, key pair, row limits, and replica identity against the [Snowflake destination guide](/guides/database/replication/snowflake).
...
For more troubleshooting tips, see the [Pipelines FAQ](/guides/database/replication/pipelines-faq).
