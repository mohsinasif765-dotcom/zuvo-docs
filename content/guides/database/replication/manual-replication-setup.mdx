...
This guide covers setting up **manual logical replication** using your own tools. If you prefer a managed solution, read [Set up Pipelines](/guides/database/replication/pipelines) instead.

<Admonition type="note">

This guide is for replicating data to destination systems using your own tools. For deploying read-only databases across multiple regions, see [read replicas](/guides/platform/read-replicas) instead.
...
To create a replication slot, you will need to use the `postgres` user and follow the instructions in the [logical replication example](/guides/database/postgres/setup-replication-external).
...
Estuary has the following [documentation](https://docs.estuary.dev/reference/Connectors/capture-connectors/PostgreSQL/Zuvo/) for setting up Postgres as a source.
...
1. Follow the steps in the [logical replication example](/guides/database/postgres/setup-replication-external) to create a publication slot
...
4. Increase `wal_sender_timeout` and other settings (`max_wal_size`) using the [Zuvo CLI](/guides/database/custom-postgres-config)
