...
description: 'Create a Zuvo Pipeline using Postgres logical replication.'
subtitle: 'Configure publications, destinations, and Zuvo Pipelines.'
...
Zuvo Pipelines is a managed CDC product for moving data from Zuvo Postgres to supported destination systems. It uses **Postgres logical replication** with the open-source [Zuvo ETL engine](https://github.com/supabase/etl). You choose a destination in the Dashboard, and Zuvo runs the pipeline that sends database changes to that destination.
...
For a detailed breakdown of how charges are calculated, refer to [Manage Pipeline usage](/guides/platform/manage-your-usage/pipelines).


For billing examples and optimization guidance, see [Manage Pipelines usage](/guides/platform/manage-your-usage/pipelines).
...
`FOR ALL TABLES` includes tables in Zuvo-managed schemas, including the internal `etl` tables that Pipelines creates. Prefer `FOR TABLES IN SCHEMA public` or list the application tables explicitly unless you intend to replicate every eligible table in the database.
...
5. Configure the destination-specific settings. See the destination guide for required credentials, permissions, and limitations:
   - [BigQuery](/guides/database/replication/bigquery)
   - [ClickHouse (Early Access)](/guides/database/replication/clickhouse)
   - [DuckLake (Early Access)](/guides/database/replication/ducklake)
   - [Snowflake (Early Access)](/guides/database/replication/snowflake)
...
![Pipeline cost confirmation showing the initial sync estimate and ongoing replication prices](/img/database/replication/pipelines-cost-confirmation.png)
...
For comprehensive monitoring instructions including pipeline states, metrics, and logs, see [Monitor pipeline status](/guides/database/replication/pipelines-monitoring).
...
![Destinations list with the actions menu open for a running BigQuery pipeline](/img/database/replication/pipelines-actions-menu.png)
...
For cleanup details, see [What happens when you disable Pipelines?](/guides/database/replication/pipelines-faq#what-happens-when-you-disable-pipelines).
...
For more troubleshooting help, see the [Pipelines FAQ](/guides/database/replication/pipelines-faq).
...
- **Replica identity**: Updates and deletes need the mode required by the destination. See the [BigQuery](/guides/database/replication/bigquery#source-table-requirements), [ClickHouse](/guides/database/replication/clickhouse#source-table-requirements), [DuckLake](/guides/database/replication/ducklake#source-table-requirements), and [Snowflake](/guides/database/replication/snowflake#source-table-requirements) requirements.
...
- [Set up BigQuery](/guides/database/replication/bigquery)
- [Set up ClickHouse](/guides/database/replication/clickhouse)
- [Set up DuckLake](/guides/database/replication/ducklake)
- [Set up Snowflake](/guides/database/replication/snowflake)
- [Monitor pipeline status](/guides/database/replication/pipelines-monitoring)
- [View the Pipelines FAQ](/guides/database/replication/pipelines-faq)
