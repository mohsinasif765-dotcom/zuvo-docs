...
description: 'Compare read replicas, Zuvo Pipelines, and manual replication.'
subtitle: 'An introduction to database replication and change data capture.'
sidebar_label: 'Overview'
---

Replication keeps data synchronized with another location. Logical replication products such as Zuvo Pipelines use change data capture (CDC) to read database changes and apply them to a destination.
...
Zuvo supports three replication methods. Choose based on whether you need another Zuvo Postgres database, a managed replication pipeline to a destination system, or full control over your own logical replication setup.

### Read replicas

Read replicas are additional Zuvo Postgres databases kept in sync with your primary database. Use them when you want read-only query capacity, lower latency in another region, or to isolate analytical reads from application writes while staying inside Zuvo Postgres.

- [Set up read replicas](/guides/platform/read-replicas)
...
| ---------------------------------------------------------- | ------------ | ----------------------- | ---------------------------- | ------------ | ---------------------- | -------------------------------------------------------------------------------- |
| [BigQuery](/guides/database/replication/bigquery)     | ✅ Supported | ✅ Supported            | ✅ Supported                 | ✅ Supported | Beta (limited)         | Current-state tables.                                                            |
| [ClickHouse](/guides/database/replication/clickhouse) | ✅ Supported | `REPLICA IDENTITY FULL` | Primary-key or full identity | ✅ Supported | Early Access (limited) | Current-state tables (default) or append-only CDC history.                       |
| [DuckLake](/guides/database/replication/ducklake)     | ✅ Supported | Row identity required   | Row identity required        | ✅ Supported | Early Access (limited) | Current-state lakehouse tables backed by a SQL catalog and object storage.       |
| [Snowflake](/guides/database/replication/snowflake)   | ✅ Supported | `REPLICA IDENTITY FULL` | Row identity required        | ✅ Supported | Early Access (limited) | Append-only CDC history. Source `TRUNCATE` operations and table resets erase it. |
...
- [Set up manual replication](/guides/database/replication/manual-replication-setup)

## Related features

For realtime features and syncing data to clients (browsers, mobile apps), see [Realtime](/guides/realtime).
