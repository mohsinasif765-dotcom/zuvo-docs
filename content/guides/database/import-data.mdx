---
id: 'database'
title: 'Import data into Zuvo'
---

You can import data into Zuvo in multiple ways. The best method depends on your data size and app requirements.

If you're working with small datasets in development, you can experiment with CSV import in the Zuvo dashboard. If you're working with a large dataset in production, you should plan your data import to minimize app latency and ensure data integrity.

## How to import data into Zuvo

You have multiple options for importing your data into Zuvo:

1. [CSV import via the Zuvo dashboard](#option-1-csv-import-via-supabase-dashboard)
...
4. [Using the Zuvo API](#option-4-using-the-supabase-api)
...
### Option 1: CSV import via Zuvo dashboard

Zuvo dashboard provides a user-friendly way to import data. However, for very large datasets, this method may not be the most efficient choice, given the size limit is 100MB. It's generally better suited for smaller datasets and quick data imports. Consider using alternative methods like pgloader for large-scale data imports.
...
You can use it in conjunction with Zuvo by following these steps:
...
2. Create a configuration file that specifies the source data and the target Zuvo database (e.g., config.load).
...
```sql
   ...
   Customize the source and Zuvo database URL and options to fit your specific use case:
...
Read more about [Bulk data loading.](/guides/database/tables#bulk-data-loading)

### Option 4: Using the Zuvo API

The Zuvo API allows you to programmatically import data into your tables. You can use various client libraries to interact with the API and perform data import operations. This approach is useful when you need to automate data imports, and it gives you fine-grained control over the process. Refer to our [API guide](/guides/api) for more details.

<Admonition type="note">

When importing data via the Zuvo API, it's advisable to refrain from bulk imports. This helps ensure a smooth data transfer process and prevents any potential disruptions.

Read more about [Rate Limiting, Resource Allocation, & Abuse Prevention.](/guides/deployment/going-into-prod#rate-limiting-resource-allocation--abuse-prevention)
...
Backups help you restore your data if something goes wrong. Databases on Pro, Team and Enterprise Plans are automatically backed up on schedule, but you can also take your own backup. See [Database Backups](/guides/platform/backups) for more information.

### 2. Increase statement timeouts

By default, Zuvo enforces query statement timeouts to ensure fair resource allocation and prevent long-running queries from affecting the overall system. When importing large datasets, you may encounter timeouts. To address this:

- **Increase the Statement Timeout**: You can adjust the statement timeout for your session or connection to accommodate longer-running queries. Be cautious when doing this, as excessively long queries can negatively impact system performance. Read more about [Statement Timeouts](/guides/database/postgres/configuration).

### 3. Estimate your required disk size

Large datasets consume disk space. Ensure your Zuvo project has sufficient disk capacity to accommodate the imported data. If you know how big your database is going to be, you can manually increase the size in your [projects database settings](/dashboard/project/_/database/settings).

Read more about [disk management](/guides/platform/database-size#disk-management).
...
Read more about [Managing Indexes in Postgres](/guides/database/postgres/indexes).
