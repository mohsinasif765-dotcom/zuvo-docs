...
Every [Compute Add-On](/guides/platform/compute-and-disk) has a pre-configured direct connection count and Supavisor pool size. This guide discusses ways to observe and manage them resourcefully.
...
These numbers are generalizations and depends on other Zuvo products that you use and the extent of their usage. The actual values depend on your concurrent peak connection usage. For instance, if you were only using 80 connections in a week period and your database max connections is set to 500, then realistically you could allocate the difference of 420 (minus a reasonable buffer) to service more demand.
...
![Database client connections chart](/img/database/reports/db-connections-chart-light.png)



For Teams and Enterprise plans, Zuvo provides Advanced Telemetry charts directly within the Dashboard. The `Database client connections` chart displays historical connection data broken down by connection type:

- **Postgres**: Direct connections from your application
- **PostgREST**: Connections from the PostgREST API layer
- **Reserved**: Administrative connections for Zuvo services
- **Auth**: Connections from Zuvo Auth service
- **Storage**: Connections from Zuvo Storage service
...
For more details on using these monitoring charts, see the [Reports guide](/guides/monitoring-and-debugging/reports#advanced-telemetry).

#### Grafana Dashboard

Zuvo offers a Grafana Dashboard that records and visualizes over 200 project metrics, including connections. For setup instructions, check the [metrics docs](/guides/monitoring-and-debugging/metrics).
...
| Role                         | API/Tool                                                                  |
| ---------------------------- | ------------------------------------------------------------------------- |
| `supabase_admin`             | Used by Zuvo for monitoring and by Realtime                           |
| `authenticator`              | Data API (PostgREST)                                                      |
| `supabase_auth_admin`        | Auth                                                                      |
| `supabase_storage_admin`     | Storage                                                                   |
| `supabase_replication_admin` | Synchronizes Read Replicas                                                |
| `postgres`                   | Zuvo Studio and External Tools (e.g., Prisma, SQLAlchemy, PSQL...) |
...
Both functions require you to either be a superuser, hold the `pg_signal_backend` role, or be signalling your own session - and even `pg_signal_backend` can't be used to stop a session belonging to an actual superuser role. If you hit a permission error while terminating a session, see [this troubleshooting guide](/guides/troubleshooting/high-cpu-and-slow-queries-with-error-must-be-a-superuser-to-terminate-superuser-process).

---

If you're on Zuvo, you can see all of this - session states, blocking chains, and a way to terminate a stuck session - on your project's [Database Connections](/dashboard/project/_/observability/connections) page in the dashboard, without writing any SQL.
