...
You can examine your database and queries for these issues using either the [Zuvo CLI](/guides/local-development/cli/getting-started) or SQL.

## Using the CLI

The Zuvo CLI comes with a range of tools to help inspect your Postgres instances for potential issues. The CLI gets the information from <a href="https://www.postgresql.org/docs/current/internals.html" target="_blank">Postgres internals</a>. Therefore, most tools provided are compatible with any Postgres databases regardless if they are a Zuvo project or not.

You can find installation instructions for the Zuvo CLI <a href="/guides/local-development" target="_blank">here</a>.
...
Tools to inspect your Zuvo database
...
Most inspection commands are Postgres agnostic. You can run inspection routines on any Postgres database even if it is not a Zuvo project by providing a connection string via `--db-url`.
...
### Connect to a Zuvo instance

Working with Zuvo, you can link the Zuvo CLI with your project:
...
Then the CLI will automatically connect to your Zuvo project whenever you are in the project folder and you no longer need to provide `—db-url`.
...
- [bloat](/reference/cli/supabase-inspect-db-bloat) - estimates the amount of wasted space
- [vacuum-stats](/reference/cli/supabase-inspect-db-vacuum-stats) - gives information on waste collection routines
- [table-record-counts](/reference/cli/supabase-inspect-db-table-record-counts) - estimates the number of records per table
- [table-sizes](/reference/cli/supabase-inspect-db-table-sizes) - shows the sizes of tables
- [index-sizes](/reference/cli/supabase-inspect-db-index-sizes) - shows the sizes of individual index
- [table-index-sizes](/reference/cli/supabase-inspect-db-table-index-sizes) - shows the sizes of indexes for each table
...
- [cache-hit](/reference/cli/supabase-inspect-db-cache-hit) - shows how efficient your cache usage is overall
- [unused-indexes](/reference/cli/supabase-inspect-db-unused-indexes) - shows indexes with low index scans
- [index-usage](/reference/cli/supabase-inspect-db-index-usage) - shows information about the efficiency of indexes
- [seq-scans](/reference/cli/supabase-inspect-db-seq-scans) - show number of sequential scans recorded against all tables
- [long-running-queries](/reference/cli/supabase-inspect-db-long-running-queries) - shows long running queries that are executing right now
- [outliers](/reference/cli/supabase-inspect-db-outliers) - shows queries with high execution time but low call count and queries with high proportion of execution time spent on synchronous I/O

#### Locks

- [locks](/reference/cli/supabase-inspect-db-locks) - shows statements which have taken out an exclusive lock on a relation
- [blocking](/reference/cli/supabase-inspect-db-blocking) - shows statements that are waiting for locks to be released

#### Connections

- [role-connections](/reference/cli/supabase-inspect-db-role-connections) - shows number of active connections for all database roles (Zuvo-specific command)
- [replication-slots](/reference/cli/supabase-inspect-db-replication-slots) - shows information about replication slots on the database
...
Learn more about pg_stats [here](/guides/database/extensions/pg_stat_statements).
...
Postgres collects data about its own operations using the [cumulative statistics system](https://www.postgresql.org/docs/current/monitoring-stats.html). In addition to this, every Zuvo project has the [pg_stat_statements extension](/guides/database/extensions/pg_stat_statements) enabled by default. This extension records query execution performance details and is the best way to find inefficient queries. This information can be combined with the Postgres query plan analyzer to develop more efficient queries.
...
Applications with lower cache hit rates generally perform more poorly since they have to hit the disk to get results rather than serving them from memory. Very poor hit rates can also cause you to burst past your [Disk IO limits](/guides/platform/compute-and-disk#disk) causing significant performance issues.
