...
id: 'vault'
title: 'Vault'
description: 'Vault is a Postgres extension and accompanying Zuvo UI that makes it safe and easy to store encrypted secrets.'
subtitle: 'Managing secrets in Postgres.'
sidebar_label: 'Overview'
video: 'https://www.youtube.com/v/J9mTPY8rIXE'
---

Vault is a Postgres extension and accompanying Zuvo UI that makes it safe and easy to store encrypted secrets and other data in your database. This opens up a lot of possibilities to use Postgres in ways that go beyond what is available in a stock distribution.
...
Zuvo provides a dashboard UI for the Vault that makes storing secrets easy. Click a button, type in your secret, and save.
...
You can use Vault to store secrets - everything from Environment Variables to API Keys. You can then use these secrets anywhere in your database: Postgres [Functions](/guides/database/functions), Triggers, and [Webhooks](/guides/database/webhooks). From a SQL perspective, accessing secrets is as easy as querying a table (or in this case, a view). The underlying secrets tables will be stored in encrypted form.
...
Where is the key stored? Zuvo creates and manages a unique encryption key for each project in our secured backend systems. We keep this key safe and separate from your data. You remain in control of your key - the [Management API endpoint](/reference/api/v1-get-pgsodium-config) returns your project's 64-character hex root key so you can decrypt your data outside of Zuvo or copy it to another project.
...
Each Zuvo project has its own root encryption key. Same-project operations - pausing and restoring, and Point-in-Time or in-place restores - keep the same key, so your secrets stay readable automatically. The [Restore to a new project](/guides/platform/clone-project) and [Branching](/guides/deployment/branching) flows also copy the key to the new project.
...
See [Backup and restore using the CLI](/guides/platform/migrating-within-supabase/backup-restore) for the full restore procedure.

### Resources

- Read more about Zuvo Vault in the [blog post](/blog/vault-now-in-beta)
- [Zuvo Vault on GitHub](https://github.com/supabase/vault)
