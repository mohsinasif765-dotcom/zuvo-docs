...
description: 'How to manage schema migrations for your Zuvo project.'
subtitle: 'How to manage schema migrations for your Zuvo project.'
...
You will need to [install](/guides/local-development#quickstart) the Zuvo CLI and start the local development stack.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Create your table from the Dashboard">
      ...
      Then generate a [schema diff](/reference/cli/supabase-db-diff).
    </StepHikeCompact.Details>
...
You've been developing your project locally, making changes to your tables via migrations. It's time to deploy your project to the Zuvo Platform and start scaling up to millions of users!

Head over to [Zuvo](/dashboard) and create a new project to deploy to.

<StepHikeCompact>

  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Log in to the Zuvo CLI">
      [Login](/reference/cli/supabase-login) to the Zuvo CLI using an auto-generated Personal Access Token.
    </StepHikeCompact.Details>
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={2}>
    <StepHikeCompact.Details title="Link your project">
      [Link](/reference/cli/supabase-link) to your remote project by selecting from the on-screen prompt.
    </StepHikeCompact.Details>
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={3}>
    <StepHikeCompact.Details title="Deploy database migrations">
      [Push](/reference/cli/supabase-db-push) your migrations to the remote database.
    </StepHikeCompact.Details>
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={4}>
    <StepHikeCompact.Details title="Deploy database seed data (optional)">
      [Push](/reference/cli/supabase-db-push) your migrations and seed the remote database.
    </StepHikeCompact.Details>
...
Visiting your live project on [Zuvo](/dashboard/project/_), you'll see a new `employees` table, complete with the `department` column you added in the second migration above.

## Working with a team

When multiple developers share a Zuvo project, a few rules keep migrations from getting out of sync.
...
For a more automated deployment approach, consider using [Zuvo Branching](/guides/deployment/branching) or a CI/CD pipeline that runs `supabase db push` on merge to your main branch.
...
Zuvo tracks which migrations have been applied on each database in a table called `supabase_migrations.schema_migrations`. When you run `supabase db push`, it compares your local `supabase/migrations` folder against that table and runs only the ones not yet applied, in order.

Git tracks your migration _files_. Zuvo tracks what's been _applied to each database_. These are two separate systems that need to stay in sync.
