...
- Ensure you have enabled row level security (RLS) on all tables from the [**Database > Tables**](/dashboard/project/_/database/tables) section of the Zuvo Studio.
  ...
  - [Learn more about RLS](/guides/database/postgres/row-level-security).
- Enable replication on tables containing sensitive data by enabling RLS and setting row security policies:
  - Go to the [**Database > Policies**](/dashboard/project/_/database/policies) section of the Zuvo Studio to enable RLS and create security policies.
  - Go to the [**Database > Publications**](/dashboard/project/_/database/publications) section of the Zuvo Studio to manage replication tables.
- Turn on [SSL Enforcement](/guides/platform/ssl-enforcement) from the [**Database > Settings > SSL Configuration**](/dashboard/project/_/database/settings#ssl-configuration) section of the dashboard.
- Enable [Network Restrictions](/guides/platform/network-restrictions) for the database from the [**Database > Settings > Network Restrictions**](/dashboard/project/_/database/settings#network-restrictions) section of the dashboard.
- Ensure that you protect your Zuvo Account with multi-factor authentication (MFA).
  ...
  - Since your GitHub account gives you administrative rights to your Zuvo org, you should protect it with a strong password and 2FA using a U2F key or a TOTP app.
  - If using email+password sign-in, set up [MFA for your Zuvo account](/guides/platform/multi-factor-authentication#enable-mfa).
- Consider enabling [MFA enforcement on your organization](/guides/platform/mfa/org-mfa-enforcement). This ensures all users must have a valid MFA-backed session to interact with organization and project resources.
- Consider adding multiple owners on your Zuvo org from [the **Organization > Team**](/dashboard/org/_/team) section of the Zuvo Studio. This ensures that if one of the owners is unreachable or loses access to their account, you still have Owner access to your org.
...
- If your application requires a higher level of security, consider setting up [multi-factor authentication](/guides/auth/auth-mfa) (MFA) for your users.
...
- Upgrade your database if you require more resources. If you need anything beyond what is listed in [the compute and disk table](/guides/platform/compute-and-disk), contact [enterprise@supabase.io](mailto:enterprise@supabase.io).
...
- Use your own SMTP credentials so that you have full control over the deliverability of your transactional auth emails in the [**Authentication > Emails > SMTP Settings**](/dashboard/project/_/auth/smtp) section of the dashboard.
  - You can grab SMTP credentials from any major email provider such as SendGrid, AWS SES, etc. Read our [SMTP guide](/guides/auth/auth-smtp) for more setup details.
...
- We may pause applications on the Free Plan that exhibit low activity in a 7-day period to save on server resources.
  - You can restore paused projects from [the Zuvo dashboard](/dashboard/project/_).
...
- Database backups are not available for download for Free Plan projects.
  - Read [the Database Backups guide](/guides/platform/backups) for more options and retention details.
...
- Zuvo Projects use disks that offer 99.8-99.9% durability by default.
  - Use [Read Replicas](/guides/platform/read-replicas) if you require availability resilience to a disk failure event
  - Use PITR if you require durability resilience to a disk failure event
- Upgrading to the Zuvo Pro Plan gives you [access to our support team](/dashboard/support/new).
...
- If you're on the Pro Plan, consider enabling [branching](/guides/deployment/branching) to test migrations in a preview environment before they reach production

## Rate limiting, resource allocation, & abuse prevention

<Admonition type="caution" title="Shared Responsibility Model">

Running databases is a shared responsibility between you and Zuvo. There are some things that we can take care of for you, and some things that you are responsible for.

Read more details in our [Shared Responsibility Model guide](/guides/deployment/shared-responsibility-model).

</Admonition>

- Zuvo employs a number of safeguards against bursts of incoming traffic to prevent abuse and help maximize stability across the platform
...
| ------------------------------------------------ | -------------------------------------------------------------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| All endpoints that send emails                   | `/auth/v1/signup` `/auth/v1/recover` `/auth/v1/user`[^1]       | Sum of combined requests | As of 3 Sep 2024, this has been updated to 

_Log field reference tables are available in Zuvo Studio._

 emails per hour. You can only change this with your own [custom SMTP setup](/guides/auth/auth-smtp). |
...
- Review the [Realtime limits](/guides/realtime/limits).
- If you need limits increased, [contact support](/dashboard/support/new).

### Abuse prevention

- Zuvo provides CAPTCHA protection on the signup, sign-in and password reset endpoints. Read [the Auth CAPTCHA guide](/guides/auth/auth-captcha) for more details on how to protect against abuse using this method.

### Email link validity

- When working with enterprise systems, email scanners may scan and make a `GET` request to the reset password link or sign-up link in your email. Since links in Zuvo Auth are single-use, a user who opens an email post-scan to click on a link will receive an error. To get around this problem, consider altering the email template to replace the original magic link with a link to a domain you control. The domain can present the user with a "Sign-in" button, which redirects the user to the original magic link URL when clicked.
- When using a custom SMTP service, some services might have link tracking enabled which may overwrite or deform the email confirmation links sent by Zuvo Auth. To prevent this from happening, we recommend that you disable link tracking when using a custom SMTP service.
